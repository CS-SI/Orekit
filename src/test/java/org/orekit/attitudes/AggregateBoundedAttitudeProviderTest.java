...
class AggregateBoundedAttitudeProviderTest {
    ...
    void testEmptyList() {
        try {
            ...
            assertEquals(OrekitMessages.NOT_ENOUGH_ATTITUDE_PROVIDERS, oe.getSpecifier());
        }
    }
    ...
    void testAEM() {
        ...
        // Verify dates
        assertEquals(0.0, provider.getMinDate().durationFrom(ephemeris.getStart()), 1.0e-10);
        assertEquals(0.0, provider.getMaxDate().durationFrom(ephemeris.getStop()),  1.0e-10);
        assertEquals(0.0, provider.getMinDate().durationFrom(ephemeris.getSegments().get(0).getStart()), 1.0e-10);
        assertEquals(0.0, provider.getMaxDate().durationFrom(ephemeris.getSegments().get(1).getStop()), 1.0e-10);
        ...
        assertEquals(0.45652, rotation.getQ0(), 0.00001);
        assertEquals(-0.84532, rotation.getQ1(), 0.00001);
        assertEquals(0.26974, rotation.getQ2(), 0.00001);
        assertEquals(-0.06532, rotation.getQ3(), 0.00001);
    ...
    private <T extends CalculusFieldElement<T>> void doTestFieldAEM(final Field<T> field) {
        ...
        assertEquals(0.45652, rotation.getQ0().getReal(), 0.00001);
        assertEquals(-0.84532, rotation.getQ1().getReal(), 0.00001);
        assertEquals(0.26974, rotation.getQ2().getReal(), 0.00001);
        assertEquals(-0.06532, rotation.getQ3().getReal(), 0.00001);

        // Verify getAttitudeRotation
        FieldRotation<T> rotation2 = provider.getAttitudeRotation(null, date, null);
        assertEquals(0., Rotation.distance(rotation.toRotation(), rotation2.toRotation()));
    }
