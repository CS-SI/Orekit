...
class AbstractExtendedPositionProviderTest {
    ...
    void testGetPVCoordinates() {
        // GIVEN
        final Orbit orbit = TestUtils.getDefaultOrbit(AbsoluteDate.ARBITRARY_EPOCH);
        final TestPositionProvider positionProvider = new TestPositionProvider(orbit);
    ...
    private static class TestPositionProvider extends AbstractExtendedPositionProvider<Orbit> {

        protected TestPositionProvider(Orbit provider) {
            super(provider);
        }

        @Override
        protected <S extends CalculusFieldElement<S>> FieldPVCoordinatesProvider<S> getFieldProvider(Field<S> field) {
            return new FieldCartesianOrbit<>(field, getProvider());
        }
    }
