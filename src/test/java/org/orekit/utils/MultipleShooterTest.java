...
public class MultipleShooterTest {
    ...
    public void testHaloAllFree() {
        ...
        for (int i = 0; i < actualSol.size(); i++) {
            ...
            Assert.assertEquals(0.0, expPva.getDate().durationFrom(actPva.getDate()), 1e-2);
            Assert.assertEquals(expPva.getPosition().getX(), actPva.getPosition().getX(), 100.);
            Assert.assertEquals(expPva.getPosition().getY(), actPva.getPosition().getY(), 100.);
            Assert.assertEquals(expPva.getPosition().getZ(), actPva.getPosition().getZ(), 100.);
            Assert.assertEquals(expPva.getVelocity().getX(), actPva.getVelocity().getX(), 1e-4);
            Assert.assertEquals(expPva.getVelocity().getY(), actPva.getVelocity().getY(), 1e-4);
            Assert.assertEquals(expPva.getVelocity().getZ(), actPva.getVelocity().getZ(), 1e-4);
        }
    ...
    public void testDROFix() {
        ...
        for (int i = 0; i < actualSol.size(); i++) {
            actPva = actualSol.get(i).getAbsPVA();
            expPva = expectedSol.get(i).getAbsPVA();
            Assert.assertEquals(0.0, expPva.getDate().durationFrom(actPva.getDate()), 1e-1);
            Assert.assertEquals(expPva.getPosition().getX(), actPva.getPosition().getX(), 100.);
            Assert.assertEquals(expPva.getPosition().getY(), actPva.getPosition().getY(), 100.);
            Assert.assertEquals(expPva.getPosition().getZ(), actPva.getPosition().getZ(), 100.);
            Assert.assertEquals(expPva.getVelocity().getX(), actPva.getVelocity().getX(), 1e-4);
        ...
        Assert.assertEquals(et0[0] * tc, actualSol.get(0).getDate().durationFrom(AbsoluteDate.J2000_EPOCH), 1e-15);
        Assert.assertEquals(et0[9] * tc, actualSol.get(9).getDate().durationFrom(AbsoluteDate.J2000_EPOCH), 1e-15);
        Assert.assertEquals(x0[0][0] * lc, actualSol.get(0).getAbsPVA().getPosition().getX(), 1e-15);
        Assert.assertEquals(x0[0][1] * lc, actualSol.get(0).getAbsPVA().getPosition().getY(), 1e-15);
        Assert.assertEquals(x0[0][2] * lc, actualSol.get(0).getAbsPVA().getPosition().getZ(), 1e-15);
        Assert.assertEquals(x0[5][0] * lc, actualSol.get(5).getAbsPVA().getPosition().getX(), 1e-15);
        Assert.assertEquals(x0[5][1] * lc, actualSol.get(5).getAbsPVA().getPosition().getY(), 1e-15);
        Assert.assertEquals(x0[5][2] * lc, actualSol.get(5).getAbsPVA().getPosition().getZ(), 1e-15);
    ...
    public void testMultipleShooting() {
        ...
        final AbsoluteDate initialDate =
                        new AbsoluteDate(2000, 1, 1, 0, 0, 00.000,
    ...
    public void testMultipleShootingWithEstimatedParameters() {
        ...
        MultipleShooter multipleShooting = new MultipleShooter(correctedList, propagatorList, epochEquations, tolerance, 10);
        multipleShooting.setPatchPointComponentFreedom(1, 0, false);
        multipleShooting.setPatchPointComponentFreedom(1, 1, false);
        multipleShooting.setPatchPointComponentFreedom(1, 2, false);
        multipleShooting.setPatchPointComponentFreedom(1, 3, false);
    ...
    private static List<SpacecraftState> generatePatchPointsEphemeris(final CelestialBody primary, final CelestialBody secondary,
        ...
        for(int i = 0; i < narcs; i++) {
            ...
            currentDate = currentDate.shiftedBy(arcDuration);
    ...
    private static List<EpochDerivativesEquations> addDerivativesProviders(List<NumericalPropagator> propagatorList){
        ...
        for (NumericalPropagator propagator : propagatorList) {
            additionalEquations.add(new EpochDerivativesEquations("derivatives", propagator));
        }
