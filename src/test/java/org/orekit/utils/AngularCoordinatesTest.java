...
public class AngularCoordinatesTest {
    ...
    public void testAccelerationIssue() {
        final AngularCoordinates ac1 = new AngularCoordinates(new Rotation(0.8246393871437729,
                                                                           0.5319174399092428,
                                                                           -0.02269027802554014,
                                                                           -0.19109910929846488,
                                                                           false),
                                                              new Vector3D(6.3218563598750865,
                                                                           -0.5007359008927147,
                                                                           -2.1123869368031607),
                                                              new Vector3D(0.3525826586113047,
                                                                           -2.6708413581899695,
                                                                           1.5827902198374986));
       ...
       final FieldRotation<UnivariateDerivative2> r =
        ...
        double[][] d = {
            { 0.12999999999999998, 0.907186187414    , 0.3958327681464356 , -0.011910174147792139, -0.1420837386141021 , -1.4052318348831165, 2.847569430595228 , -0.18200785571235467, -1.0238882836528809 },
            { 0.13999999999999999, 0.8926307720109505, 0.424077978332774  , -0.013798160151021924, -0.15224251682375572, -1.505577803242234 , 2.8009325153797664, -0.19557294808967285, -1.0076865162109465 },
            { 0.15               , 0.8770802897009355, 0.45184069719235226, -0.015821289790368016, -0.1622338946339563 , -1.6042263223519142, 2.751079334796485 , -0.20903503166234183, -0.990412511764557  },
            { 0.17               , 0.8430653941300765, 0.5057915371967585 , -0.020268573736076614, -0.18167236179227658, -1.795982832779695 , 2.641930383943672 , -0.2356117380716437 , -0.9527559895089874 },
            { 0.18000000000000002, 0.8246393871437729, 0.5319174399092428 , -0.02269027802554014 , -0.19109910929846488, -1.8888713746829562, 2.5827461082239274, -0.24870610720044461, -0.9324314745011075 },
            { 0.19000000000000003, 0.8052950872419431, 0.5574364812121066 , -0.02524221744666447 , -0.20031780334301627, -1.9796238071353143, 2.5205684969331736, -0.26165693135611795, -0.9111506251515042 },
            { 0.21000000000000005, 0.7639401855988004, 0.6065361715452814 , -0.030730722391568755, -0.2180937599653455 , -2.1543044363680237, 2.3874925076919045, -0.28708259117660795, -0.8658519636977071 },
            { 0.22000000000000006, 0.7419764342774693, 0.6300595893112064 , -0.0336639573671692  , -0.2266332342534722 , -2.238030506466714 , 2.316731806079262 , -0.29953360158883624, -0.8419035228183022 },
            { 0.24000000000000007, 0.6956008884247272, 0.6749158452209764 , -0.03989914460215165 , -0.24297338157174894, -2.3977675494976265, 2.1671329114383253, -0.3238489270862786 , -0.7915912460247911 }
        };
        for (int i = 5; i < 9; ++i) {
            System.out.println(  -3 / (840 * 0.01) * (d[8][i] - d[0][i]) +
                                 32 / (840 * 0.01) * (d[7][i] - d[1][i]) +
                               -168 / (840 * 0.01) * (d[6][i] - d[2][i]) +
                                673 / (840 * 0.01) * (d[5][i] - d[3][i]));
        }
        ...
        final double aM4H = rz1(shift(r, -4 * dt));
        final double aM3H = rz1(shift(r, -3 * dt));
        final double aM2H = rz1(shift(r, -2 * dt));
        final double aM1H = rz1(shift(r, -1 * dt));
        final double aP1H = rz1(shift(r, +1 * dt));
        final double aP2H = rz1(shift(r, +2 * dt));
        final double aP3H = rz1(shift(r, +3 * dt));
        final double aP4H = rz1(shift(r, +4 * dt));
        final double finiteDifferencesAz =
                        MathArrays.linearCombination(  -3 * c, aP4H - aM4H,
                                                       32 * c, aP3H - aM3H,
                                                     -168 * c, aP2H - aM2H,
                                                      673 * c, aP1H - aM1H);
    ...
    private double rz1(final FieldRotation<UnivariateDerivative2> r) {
        return 2 * MathArrays.linearCombination(-r.getQ3().getValue(), r.getQ0().getFirstDerivative(),
                                                r.getQ2().getValue(), r.getQ1().getFirstDerivative(),
                                               -r.getQ1().getValue(), r.getQ2().getFirstDerivative(),
                                                r.getQ0().getValue(), r.getQ3().getFirstDerivative());
    }

    private double rz2(final FieldRotation<UnivariateDerivative2> r) {
        return 2 * MathArrays.linearCombination(-r.getQ3().getValue(), r.getQ0().getSecondDerivative(),
                                                r.getQ2().getValue(), r.getQ1().getSecondDerivative(),
                                               -r.getQ1().getValue(), r.getQ2().getSecondDerivative(),
                                                r.getQ0().getValue(), r.getQ3().getSecondDerivative());
    }
