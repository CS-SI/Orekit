...
class FieldSignalEmissionConditionTest {

    @Test
    void testToEmissionCondition() {
        // GIVEN
        final Frame frame = mock();
        final SignalEmissionCondition expectedCondition = new SignalEmissionCondition(AbsoluteDate.ARBITRARY_EPOCH,
                Vector3D.ZERO, frame);
        final FieldSignalEmissionCondition<Binary64> fieldCondition = new FieldSignalEmissionCondition<>(Binary64Field.getInstance(),
                expectedCondition);
        // WHEN
