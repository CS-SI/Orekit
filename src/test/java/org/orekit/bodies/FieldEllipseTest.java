...
class FieldEllipseTest {
    ...
    private <T extends CalculusFieldElement<T>> void doTestMeridianShape(final Field<T> field) {
        ...
        assertEquals(Constants.WGS84_EARTH_EQUATORIAL_RADIUS,
                                e.getA().getReal(),
                                1.0e-15 * Constants.WGS84_EARTH_EQUATORIAL_RADIUS);
        assertEquals(Constants.WGS84_EARTH_EQUATORIAL_RADIUS * (1 - Constants.WGS84_EARTH_FLATTENING),
                                e.getB().getReal(),
                                1.0e-15 * Constants.WGS84_EARTH_EQUATORIAL_RADIUS);
        assertEquals(0.5 * FastMath.PI, FieldVector3D.angle(Vector3D.PLUS_J, e.getU()).getReal(), 1.0e-15);
        assertEquals(0.5 * FastMath.PI, FieldVector3D.angle(Vector3D.PLUS_K, e.getU()).getReal(), 1.0e-15);
        assertEquals(0.5 * FastMath.PI, FieldVector3D.angle(Vector3D.PLUS_I, e.getV()).getReal(), 1.0e-15);
        assertEquals(0.5 * FastMath.PI, FieldVector3D.angle(Vector3D.PLUS_J, e.getV()).getReal(), 1.0e-15);
    }
