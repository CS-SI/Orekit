...
public class OMMParserTest {
    ...
    public void testParseOMM1()
        ...
        Assert.assertTrue(file.getData().getTLERelatedParametersComment().isEmpty());
        ...
        Assert.assertEquals(file.getData().getMeanMotion(), 1.00273272 * FastMath.PI / 43200.0 , 1e-10);
        Assert.assertEquals(file.getData().getE(), 0.0005013, 1e-10);
        Assert.assertEquals(file.getData().getI(), FastMath.toRadians(3.0539), 1e-10);
        Assert.assertEquals(file.getData().getRaan(), FastMath.toRadians(81.7939), 1e-10);
        Assert.assertEquals(file.getData().getPa(), FastMath.toRadians(249.2363), 1e-10);
        Assert.assertEquals(file.getData().getAnomaly(), FastMath.toRadians(150.1602), 1e-10);
        Assert.assertEquals(file.getData().getMu(), 398600.8 * 1e9, 1e-10);


        // Check TLE Related Parameters data block;

        Assert.assertEquals(0, file.getData().getEphemerisType());
        Assert.assertEquals('U', file.getData().getClassificationType());
        int[] noradIDExpected = new int[23581];
        int[] noradIDActual = new int[file.getData().getNoradID()];
        Assert.assertEquals(noradIDExpected[0], noradIDActual[0]);
        Assert.assertEquals("0925", file.getData().getElementSetNumber());
        int[] revAtEpochExpected = new int[4316];
        int[] revAtEpochActual = new int[file.getData().getRevAtEpoch()];
        Assert.assertEquals(1.00273272 * FastMath.PI / 43200.0, file.getData().getMeanMotion(), 1e-10);
        Assert.assertEquals(revAtEpochExpected[0], revAtEpochActual[0]);
        Assert.assertEquals(file.getData().getBStar(), 0.0001, 1e-10);
        Assert.assertEquals(file.getData().getMeanMotionDot(), -0.00000113 * FastMath.PI / 1.86624e9, 1e-12);
        Assert.assertEquals(file.getData().getMeanMotionDotDot(), 0.0 * FastMath.PI / 5.3747712e13, 1e-10);
    ...
    public void testParseOMM2()
        ...
        Assert.assertEquals(3.0, file.getHeader().getFormatVersion(), 1.0e-10);
        Assert.assertEquals(1.00273272, Constants.JULIAN_DAY * file.getData().getMeanMotion() / MathUtils.TWO_PI, 1e-10);
        try {
            file.getData().getMass();
            Assert.fail("an exception should have been thrown");
        } catch (OrekitException oe) {
            Assert.assertEquals(OrekitMessages.CCSDS_UNKNOWN_SPACECRAFT_MASS, oe.getSpecifier());
        }
        Assert.assertEquals(FramesFactory.getTEME(), file.getData().getCovRefFrame());
        Assert.assertTrue(file.getData().hasCovarianceMatrix());
        Assert.assertEquals(6, file.getData().getCovarianceMatrix().getRowDimension());
        Assert.assertEquals(6, file.getData().getCovarianceMatrix().getColumnDimension());
        Assert.assertEquals(1995, file.getMetadata().getLaunchYear());
        Assert.assertEquals(25, file.getMetadata().getLaunchNumber());
        Assert.assertEquals("A", file.getMetadata().getLaunchPiece());
        file.generateKeplerianOrbit();
    ...
    public void testParseOMM3()
        ...
        Assert.assertEquals(6800e3, file.getData().getA(), 1e-10);
        Assert.assertEquals(300, file.getData().getMass(), 1e-10);
        Assert.assertEquals(5, file.getData().getSolarRadArea(), 1e-10);
        Assert.assertEquals(0.001, file.getData().getSolarRadCoeff(), 1e-10);
        Assert.assertEquals(null, file.getData().getCovRefFrame());
        Assert.assertEquals(LOFType.TNW, file.getData().getCovRefLofType());
        ...
        Assert.assertEquals(userDefinedParameters, file.getData().getUserDefinedParameters());
        Assert.assertTrue(file.getData().hasCovarianceMatrix());
        ...
        Assert.assertEquals(dataSpacecraftComment, file.getData().getSpacecraftComment());
        ArrayList<String> dataCovarianceComment = new ArrayList<String>();
        dataCovarianceComment.add("Covariance matrix");
        Assert.assertEquals(dataCovarianceComment, file.getData().getCovarianceComment());
        Assert.assertEquals(1995, file.getMetadata().getLaunchYear());
        Assert.assertEquals(25, file.getMetadata().getLaunchNumber());
        Assert.assertEquals("A", file.getMetadata().getLaunchPiece());
        file.generateSpacecraftState();
