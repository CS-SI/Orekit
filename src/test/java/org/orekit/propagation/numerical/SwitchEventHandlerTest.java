...
class SwitchEventHandlerTest {
    ...
    void testResetState() {
        ...
        RealMatrix expectedStm = updateStm(stateAtSwitch, stm, fieldDetector, derivativesBefore, false);
        ...
        expectedStm = updateStm(stateAtSwitch, expectedStm, fieldDetector, derivativesAfter, true);
        for (int i = 0; i < 7; i++) {
    ...
    private static RealMatrix updateStm(final SpacecraftState state, final RealMatrix stm,
                                                 ...
                                                 final double[] derivatives, final boolean flip) {
        ...
        final FieldVector3D<Gradient> fieldPosition = new FieldVector3D<>(dt.multiply(derivatives[0]),
                dt.multiply(derivatives[1]), dt.multiply(derivatives[2]))
                .add(new FieldVector3D<>(new Gradient(position.getX(), augmentedStm.getRow(0)),
                        new Gradient(position.getY(), augmentedStm.getRow(1)),
                        new Gradient(position.getZ(), augmentedStm.getRow(2))));
        final FieldVector3D<Gradient> fieldVelocity = new FieldVector3D<>(dt.multiply(derivatives[3]),
                dt.multiply(derivatives[4]), dt.multiply(derivatives[5]))
                .add(new FieldVector3D<>(new Gradient(velocity.getX(), augmentedStm.getRow(3)),
                        new Gradient(velocity.getY(), augmentedStm.getRow(4)),
                        new Gradient(velocity.getZ(), augmentedStm.getRow(5))));
        final FieldAbsolutePVCoordinates<Gradient> fieldAbsolutePVCoordinates = new FieldAbsolutePVCoordinates<>(
                state.getFrame(), new FieldAbsoluteDate<>(dt.getField(), state.getDate()).shiftedBy(dt),
                fieldPosition, fieldVelocity);
        final Gradient fieldMass = new Gradient(state.getMass(), augmentedStm.getRow(6)).add(dt.multiply(derivatives[6]));
        ...
        for (int i = 0; i < 7; i++) {
            lhs.setEntry(i, 7, flip ? -derivatives[i] : derivatives[i]);
        }
