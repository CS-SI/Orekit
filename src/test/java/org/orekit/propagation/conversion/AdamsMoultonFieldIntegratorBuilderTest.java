...
class AdamsMoultonFieldIntegratorBuilderTest {
    ...
    void testGetTolerances() {
        ...
        final int nSteps = 2;
        // WHEN
        final AdamsMoultonFieldIntegratorBuilder<Complex> builder = new AdamsMoultonFieldIntegratorBuilder<>(nSteps, minStep,
                maxStep, dP, dV);
        final double[][] actualTolerances = builder.getTolerances(orbit, orbit.getType());
        // THEN
        final AdamsMoultonFieldIntegratorBuilder<Complex> builder2 = new AdamsMoultonFieldIntegratorBuilder<>(nSteps, minStep,
                maxStep, dP);
