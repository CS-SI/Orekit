...
public class BrouwerLyddaneStateTransitionMatrixTest {
    ...
    public void testStateJacobian() {
        ...
        for (int i = 0; i < 6; ++i) {
            propagator2 = new BrouwerLyddanePropagator(shiftState(initialState, OrbitParamsType.CARTESIAN, -4 * steps[i], i).getOrbit(), provider, M2);
            SpacecraftState sM4h = propagator2.propagate(target);
            propagator2 = new BrouwerLyddanePropagator(shiftState(initialState, OrbitParamsType.CARTESIAN, -3 * steps[i], i).getOrbit(), provider, M2);
            SpacecraftState sM3h = propagator2.propagate(target);
            propagator2 = new BrouwerLyddanePropagator(shiftState(initialState, OrbitParamsType.CARTESIAN, -2 * steps[i], i).getOrbit(), provider, M2);
            SpacecraftState sM2h = propagator2.propagate(target);
            propagator2 = new BrouwerLyddanePropagator(shiftState(initialState, OrbitParamsType.CARTESIAN, -1 * steps[i], i).getOrbit(), provider, M2);
