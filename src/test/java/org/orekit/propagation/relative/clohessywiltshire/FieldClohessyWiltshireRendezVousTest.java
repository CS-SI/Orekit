...
public class FieldClohessyWiltshireRendezVousTest {
    ...
    public void testTransferComputation() {
        ...
        final Binary64 rTarget = new Binary64(FastMath.pow(Constants.EIGEN5C_EARTH_MU/(n.getReal()*n.getReal()),1./3.));
        ...
        TimeStampedFieldPVCoordinates<Binary64> pvtChaserInitial = new TimeStampedFieldPVCoordinates<>(epoch, new FieldPVCoordinates<>(new FieldVector3D<>(field, new Vector3D(20.0e3, 20.0e3, 20.0e3)),new FieldVector3D<>(field, new Vector3D(-0.02e3, 0.02e3, -0.005e3))));
        TimeStampedFieldPVCoordinates<Binary64> pvtChaserFinal = new TimeStampedFieldPVCoordinates<>(rdvDate, new FieldPVCoordinates<>(new FieldVector3D<>(field,Vector3D.ZERO),new FieldVector3D<>(field,Vector3D.ZERO)));

        // Compute rendez-vous transfer using the Clohessy_Wiltshire equations
        final FieldTwoImpulseTransfer<Binary64> rdvSolution = (new FieldClohessyWiltshireRendezVous<Binary64>()).computeRendezVous(pvtChaserInitial,pvtChaserFinal,targetLof,targetOrbit);
        ...
        Assertions.assertEquals(pvt1Ref.getDate().toDouble(), rdvSolution.getPvt1().getDate().toAbsoluteDate().toDouble(),NUMERICAL_TOLERANCE);
        TestUtils.validateFieldVector3D(pvt1Ref.getPosition(),rdvSolution.getPvt1().getPosition(),NUMERICAL_TOLERANCE);
        TestUtils.validateFieldVector3D(pvt1Ref.getVelocity(),rdvSolution.getPvt1().getVelocity(),NUMERICAL_TOLERANCE);

        Assertions.assertEquals(pvt2Ref.getDate().toDouble(),rdvSolution.getPvt2().getDate().toAbsoluteDate().toDouble(),NUMERICAL_TOLERANCE);
        TestUtils.validateFieldVector3D(pvt2Ref.getPosition(),rdvSolution.getPvt2().getPosition(),NUMERICAL_TOLERANCE);
        TestUtils.validateFieldVector3D(pvt2Ref.getVelocity(),rdvSolution.getPvt2().getVelocity(),NUMERICAL_TOLERANCE);
