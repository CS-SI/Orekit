...
public class ClohessyWiltshireRendezVousTest {
    ...
    public void testMatrixComputation() {
        ...
        TestUtils.validateRealMatrix(MatrixUtils.createRealMatrix(new double[][] {{    4.97868541238670,    0.00000000000000,    0.00000000000000},{   -194.242457498099,    1.00000000000000,    0.00000000000000},{    0.00000000000000,    0.00000000000000,  -0.326228470795567}}), matrices.getPhiRR(), NUMERICAL_TOLERANCE);
        TestUtils.validateRealMatrix(MatrixUtils.createRealMatrix(new double[][] {{    817.081897951423,    2292.70633171889,    0.00000000000000},{   -2292.70633171889,   -83131.6724081943,    0.00000000000000},{    0.00000000000000,    0.00000000000000,    817.081897951423}}), matrices.getPhiRV(), NUMERICAL_TOLERANCE);
        TestUtils.validateRealMatrix(MatrixUtils.createRealMatrix(new double[][] {{ 0.00328085221475134,    0.00000000000000,    0.00000000000000},{-0.00920596902838442,    0.00000000000000,    0.00000000000000},{    0.00000000000000,    0.00000000000000,-0.00109361740491711}}), matrices.getPhiVR(), NUMERICAL_TOLERANCE);
        TestUtils.validateRealMatrix(MatrixUtils.createRealMatrix(new double[][] {{  -0.326228470795567,    1.89058190496195,    0.00000000000000},{   -1.89058190496195,   -4.30491388318227,    0.00000000000000},{    0.00000000000000,    0.00000000000000,  -0.326228470795567}}), matrices.getPhiVV(), NUMERICAL_TOLERANCE);
    }

    /**
     * Tests a transfer computation using the Clohessy-Wiltshire equations.
     * Source for numerical values : Orbital Mechanics for Engineering Students, Curtis
     */
    @Test
    public void testTransferComputation() {
        ...
        // Target's QSW LOF
        final LocalOrbitalFrame targetLof = new LocalOrbitalFrame(targetOrbit.getFrame(), LOFType.QSW, new KeplerianPropagator(targetOrbit), "QSW LOF target");

        // Start and end conditions of the transfer, expressed in the target's LOF
        TimeStampedPVCoordinates pvtChaserInitial = new TimeStampedPVCoordinates(epoch, new Vector3D(20.0e3, 20.0e3, 20.0e3), new Vector3D(-0.02e3, 0.02e3, -0.005e3));
        ...
        final TwoImpulseTransfer rdvSolution = ClohessyWiltshireRendezVous.computeRendezVous(
            pvtChaserInitial,
            pvtChaserFinal,
            targetLof,
            targetOrbit);
        ...
        Assertions.assertEquals(pvt2Ref.getDate().toDouble(), rdvSolution.getPvt2().getDate().toDouble(),NUMERICAL_TOLERANCE);
