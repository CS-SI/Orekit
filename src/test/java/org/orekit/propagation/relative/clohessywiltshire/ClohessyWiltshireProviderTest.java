...
/**
 * Comparison of chaser state propagation using Clohessy-Wiltshire provider and a classic Keplerian Propagator. As
 * Clohessy-Wiltshire model diverges promptly with time of simulation and for low altitude orbit, test is done for a GEO
 * target, in close proximity for a short propagation duration.
 */

class ClohessyWiltshireProviderTest {
    ...
    @Test
    void testClohessyWiltshireProvider() {
        ...
        final KeplerianOrbit targetOrbit = new KeplerianOrbit(orbitRadius,
                                                              0,
                                                              0,
                                                              0,
                                                              0,
        ...
        final KeplerianOrbit chaserOrbit =
                        new KeplerianOrbit(initialChaserPV, eme2000, date, Constants.EIGEN5C_EARTH_MU);
        ...
        final ClohessyWiltshireProvider cwProvider =
                        new ClohessyWiltshireProvider(targetOrbit, chaserTimePV, eme2000, "Clohessy-Wiltshire");
        ...
        final TimeStampedPVCoordinates chaserInertialWithExtract =
                        cwProvider.extractChaserPVT(finalTargetState, eme2000);

        // Assess PV coordinates of the chaser are equals in inertial after propagation and lof transformation
        final double dp = 2.915e-7;
        final double dv = 5.831e-9;
        TestUtils.validateVector3D(finalChaserState.getPosition(), inertialPVCW.getPosition(), dp);
        TestUtils.validateVector3D(finalChaserState.getVelocity(), inertialPVCW.getVelocity(), dv);

        // Assess PV coordinates of the chase are equals in inertial after propagation and using
        // extractChaserPVT method from the provider.
        TestUtils.validateVector3D(finalChaserState.getPosition(), chaserInertialWithExtract.getPosition(), dp);
        TestUtils.validateVector3D(finalChaserState.getVelocity(), chaserInertialWithExtract.getVelocity(), dv);
    }
    ...
    void testGetInitialChaserPVTLof() {
        ...
        final KeplerianOrbit            targetOrbit         = new KeplerianOrbit(10000,
                                                                                 0,
                                                                                 0,
                                                                                 0,
                                                                                 0,
        ...
        final ClohessyWiltshireProvider provider2 =
                        new ClohessyWiltshireProvider(targetOrbit,
                                                      new TimeStampedPVCoordinates(date,
                                                                                   new Vector3D(1, 2, 3),
                                                                                   new Vector3D(4,5, 6)));
        final TimeStampedPVCoordinates  initialChaserPVTLof = provider.getInitialChaserPVTLof();
        TestUtils.validateVector3D(Vector3D.ZERO, initialChaserPVTLof.getPosition(), 0);
        TestUtils.validateVector3D(Vector3D.ZERO, initialChaserPVTLof.getVelocity(), 0);
        TestUtils.validateVector3D(new Vector3D(1, 2, 3),
                                   provider2.getInitialChaserPVTLof().getPosition(),
                                   0);
        TestUtils.validateVector3D(new Vector3D(4, 5, 6),
                                   provider2.getInitialChaserPVTLof().getVelocity(),
                                   0);
    }
    ...
    void testSetInitialChaserPVTLof() {
        ...
        final ClohessyWiltshireProvider provider2   =
                        new ClohessyWiltshireProvider(targetOrbit, pvt1, "Clohessy-Wiltshire");
        ...
        TestUtils.validateVector3D(pvt2.getPosition(),
                                   provider.getInitialChaserPVTLof().getPosition(),
                                   0);
        TestUtils.validateVector3D(pvt2.getVelocity(),
                                   provider.getInitialChaserPVTLof().getVelocity(),
                                   0);
        TestUtils.validateVector3D(pvt2.getPosition(),
                                   provider2.getInitialChaserPVTLof().getPosition(),
                                   0);
