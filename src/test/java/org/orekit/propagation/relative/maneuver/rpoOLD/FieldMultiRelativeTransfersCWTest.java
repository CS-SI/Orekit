...
public class FieldMultiRelativeTransfersCWTest {
    ...
    public void FieldMultiLinearTransfersTest(){
        ...
        final List<TimeStampedFieldPVCoordinates<Binary64>> waypoints = RPOModel.CW.computeLinearWaypoints(initialPVT, finalPVT,11);
        // Computation of the transfers along the linear path.
        final List<FieldTwoImpulseTransfer<Binary64>> transfers = (new FieldMultiRelativeTransfersCW<>(waypoints, targetOrbit)).computeMultiRelativeTransfers();
    ...
    public void FieldForcedCircularTransfersTest(){
        ...
        final List<TimeStampedFieldPVCoordinates<Binary64>> waypoints = RPOModel.CW.computeForcedCircularMotionWaypoints(initialPVT,new FieldVector3D<>(field,Vector3D.ZERO),new Binary64(100),field.getZero(),field.getZero(),new Binary64(1000),10,1,11,new Binary64(1000));
        // Computation of the transfers to perform the forced circular trajectory.
        final List<FieldTwoImpulseTransfer<Binary64>> transfers = (new FieldMultiRelativeTransfersCW<>(waypoints,targetOrbit)).computeMultiRelativeTransfers();
        // Assert start and end point of the transfers are the same as the corresponding waypoints.
        for (int i = 0; i<transfers.size(); i++) {
            TestUtils.validateVector3D(transfers.get(i).getPvt1BeforeMan().getPosition().toVector3D(),waypoints.get(i).getPosition().toVector3D(),NUMERICAL_TOLERANCE);
    ...
    @Test
    public void FieldLinearTransferPropagationTest() {
        final Binary64Field field = Binary64Field.getInstance();
        final Binary64 n = new Binary64(0.0011569);// Mean motion of target's orbit
        final Binary64 rTarget = (new Binary64(Constants.EIGEN5C_EARTH_MU)).divide(n.multiply(n)).pow(1./3.);// Target orbit's radius
        final FieldAbsoluteDate<Binary64> epoch = new FieldAbsoluteDate<>(field,AbsoluteDate.J2000_EPOCH);
        ...
        final List<TimeStampedFieldPVCoordinates<Binary64>> waypoints = RPOModel.CW.computeLinearWaypoints(pvtChaserInitial, pvtChaserFinal, 6);
        ...
        final List<FieldClohessyWiltshireManeuver<Binary64>> maneuvers = (new FieldMultiRelativeTransfersCW<>(waypoints,targetOrbit)).computeRelativeManeuvers(cwProvider);
    ...
    public void FieldForcedCircularPropagationTest() {
        ...
        final List<TimeStampedFieldPVCoordinates<Binary64>> waypoints = RPOModel.CW.computeForcedCircularMotionWaypoints(pvtChaserInitial,new FieldVector3D<>(field, Vector3D.ZERO),new Binary64(100),new Binary64(0),new Binary64(0),new Binary64(1000),10,3,6,new Binary64(500));
        // Definition of the propagator.
        final FieldKeplerianPropagator<Binary64> propagator = new FieldKeplerianPropagator<>(targetOrbit);
        propagator.addAdditionalDataProvider(cwProvider);
        // Creation of the maneuvers corresponding to the impulses of the transfers.
    ...
    public void FieldNaturalCircumnavigationCWTest() {
        ...
        final List<TimeStampedFieldPVCoordinates<Binary64>> waypoints = RPOModel.CW.computeCircularNaturalCircumnavigationWaypoints(
        ...
        Assertions.assertEquals(-2 * relativeSemiMinorAxis.getReal(), chaserFinal[1].getReal(), NUMERICAL_TOLERANCE);
    ...
    // Test if the chaser position at the end of the propagation (one target orbit's period) is the same as the injection point.
    @Test
    public void FieldNaturalCircularMotionCWTest() {
        final Binary64Field field = Binary64Field.getInstance();
        final Binary64 n = new Binary64(0.0011569);// Mean motion of target's orbit
        final Binary64 rTarget = (new Binary64(Constants.EIGEN5C_EARTH_MU)).divide(n.multiply(n)).pow(1./3.);// Target orbit's radius
        final FieldAbsoluteDate<Binary64> epoch = new FieldAbsoluteDate<>(field,AbsoluteDate.J2000_EPOCH);
        ...
        final List<TimeStampedFieldPVCoordinates<Binary64>> waypoints = RPOModel.CW.computeNaturalCircularWaypoints(
        ...
        Assertions.assertEquals(0, chaserFinal[0].getReal(), NUMERICAL_TOLERANCE);
        Assertions.assertEquals(-radius.getReal(), chaserFinal[1].getReal(), NUMERICAL_TOLERANCE);
        Assertions.assertEquals(0, chaserFinal[2].getReal(), NUMERICAL_TOLERANCE);
    }
    ...
    public void getWaypointsTest() {
        ...
        // Definition of the linear path.
        final List<TimeStampedFieldPVCoordinates<Binary64>> waypoints = RPOModel.CW.computeLinearWaypoints(pvtChaserInitial, pvtChaserFinal, 6);
        final FieldMultiRelativeTransfersCW<Binary64> transfers = new FieldMultiRelativeTransfersCW<>(waypoints,targetOrbit);
    ...
    public void computeMultiRelativeTransferFromOtherFrame() {
        ...
        final List<TimeStampedFieldPVCoordinates<Binary64>> waypoints = RPOModel.CW.computeLinearWaypoints(pvtChaserInitial, pvtChaserFinal, 6);
        // Computation of the transfers along the linear path.
        final List<FieldTwoImpulseTransfer<Binary64>> transfersFromQSW = (new FieldMultiRelativeTransfersCW<>(waypoints,targetOrbit)).computeMultiRelativeTransfers();
        // Transform the waypoints in the LVLH frame and compute the transfers.
        final List<TimeStampedFieldPVCoordinates<Binary64>> waypointsLVLH = new ArrayList<>();
        ...
        final List<FieldTwoImpulseTransfer<Binary64>> transfersFromLVLH = (new FieldMultiRelativeTransfersCW<>(waypointsLVLH,targetOrbit)).computeMultiRelativeTransfers(lvlh);
        // Assert start and end point of the transfers are the same as the corresponding waypoints.
        for (int i = 0; i<transfersFromQSW.size(); i++) {
            TestUtils.validateVector3D(transfersFromQSW.get(i).getPvt1BeforeMan().getPosition().toVector3D(),transfersFromLVLH.get(i).getPvt1BeforeMan().getPosition().toVector3D(),NUMERICAL_TOLERANCE);
            TestUtils.validateVector3D(transfersFromQSW.get(i).getPvt1BeforeMan().getVelocity().toVector3D(),transfersFromLVLH.get(i).getPvt1BeforeMan().getVelocity().toVector3D(),NUMERICAL_TOLERANCE);
            TestUtils.validateVector3D(transfersFromQSW.get(i).getPvt2AfterMan().getPosition().toVector3D(),transfersFromLVLH.get(i).getPvt2AfterMan().getPosition().toVector3D(),NUMERICAL_TOLERANCE);
            TestUtils.validateVector3D(transfersFromQSW.get(i).getPvt2AfterMan().getVelocity().toVector3D(),transfersFromLVLH.get(i).getPvt2AfterMan().getVelocity().toVector3D(),NUMERICAL_TOLERANCE);
            TestUtils.validateVector3D(transfersFromQSW.get(i).getDeltaV1().toVector3D(),transfersFromLVLH.get(i).getDeltaV1().toVector3D(),NUMERICAL_TOLERANCE);
            TestUtils.validateVector3D(transfersFromQSW.get(i).getDeltaV2().toVector3D(),transfersFromLVLH.get(i).getDeltaV2().toVector3D(),NUMERICAL_TOLERANCE);
        }
    }
}
