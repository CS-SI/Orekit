...
import org.orekit.control.relative.FieldTeardropCircularWaypointCalculator;
...
public class FieldTeardropCircularWaypointCalculatorTest {
    ...
    public void computeTearDropWaypointTestCW() {
        ...
        final FieldTeardropCircularWaypointCalculator<Binary64> tearDropCalculator = new FieldTeardropCircularWaypointCalculator<>(targetMeanMotion, turnAroundDistance, maneuverDistance, numberOfOrbits);
        final List<TimeStampedFieldPVCoordinates<Binary64>> tearDropWaypoints = tearDropCalculator.computeTearDropWaypoints(epoch.shiftedBy(1000.));
        final List<TimeStampedFieldPVCoordinates<Binary64>> tearDropWaypointsFromRPO = RPOModel.CW.computeTeardropWaypoints(epoch.shiftedBy(1000), targetOrbit, turnAroundDistance,maneuverDistance, numberOfOrbits);
        ...
        for (int i = 0; i < tearDropWaypoints.size(); i++) {
            ...
            Assertions.assertEquals(tearDropWaypoint.getDate().toAbsoluteDate().toDouble(), tearDropWaypointRPO.getDate().toAbsoluteDate().toDouble(), NUMERICAL_TOLERANCE);
            TestUtils.validateVector3D(tearDropWaypoint.getPosition().toVector3D(), tearDropWaypointRPO.getPosition().toVector3D(), NUMERICAL_TOLERANCE);
            TestUtils.validateVector3D(tearDropWaypoint.getVelocity().toVector3D(), tearDropWaypointRPO.getVelocity().toVector3D(), NUMERICAL_TOLERANCE);
        }
    }
    ...
    public void computeTearDropWaypointTestYA() {
        ...
        for (int i = 0; i < tearDropWaypointsCW.size(); i++) {
            ...
            final FieldVector3D<Binary64> positionRef = new FieldVector3D<>(positionCW.getY(), positionCW.getZ().negate(), positionCW.getX().negate());
            final FieldVector3D<Binary64> velocityRef = new FieldVector3D<>(velocityCW.getY(), velocityCW.getZ().negate(), velocityCW.getX().negate());

            Assertions.assertEquals(tearDropWaypointCW.getDate().toAbsoluteDate().toDouble(), tearDropWaypointYA.getDate().toAbsoluteDate().toDouble(), NUMERICAL_TOLERANCE);
            TestUtils.validateVector3D(positionRef.toVector3D(), positionYA.toVector3D(), NUMERICAL_TOLERANCE);
            TestUtils.validateVector3D(velocityRef.toVector3D(), velocityYA.toVector3D(), NUMERICAL_TOLERANCE);
        }
    }
