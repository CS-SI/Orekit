...
/**
 * Comparison of chaser state propagation using Yamanaka-Ankersen provider and a classic Keplerian Propagator. As
 * Yamanaka-Ankersen model diverges promptly with time of simulation and for low altitude orbit, test is done for a GEO
 * target, in close proximity for a short propagation duration.
 */
class YamanakaAnkersenProviderTest {
    ...
    void yaProviderTest() {
        ...
        final KeplerianOrbit targetOrbit =
                        new KeplerianOrbit(sma, 0.3, 0, 0, 0, 0, PositionAngleType.TRUE, eme2000,
        ...
        final YamanakaAnkersenProvider yaProvider =
                        new YamanakaAnkersenProvider(targetOrbit, chaserTimePV, eme2000, "Yamanaka-Ankersen equation");
        ...
        final TimeStampedPVCoordinates chaserInertialWithExtract =
                        yaProvider.extractChaserPVT(finalTargetState, eme2000);

        TestUtils.validateVector3D(finalChaserState.getPosition(), inertialPVYA.getPosition(), 8.7e-7);
        TestUtils.validateVector3D(finalChaserState.getVelocity(), inertialPVYA.getVelocity(), 1.8e-8);
        TestUtils.validateVector3D(inertialPVYA.getPosition(), chaserInertialWithExtract.getPosition(), 0);
        TestUtils.validateVector3D(inertialPVYA.getVelocity(), chaserInertialWithExtract.getVelocity(), 0);
    }
    ...
    void CWYAComparison() {
        ...
        final KeplerianOrbit targetOrbit =
                        new KeplerianOrbit(6671000, 0, 40 * FastMath.PI / 180, 0, 20 * FastMath.PI / 180,
                                           60 * FastMath.PI / 180, PositionAngleType.TRUE, eme2000, date,
        ...
        final TimeStampedPVCoordinates chaserTimePV =
                        new TimeStampedPVCoordinates(date, new Vector3D(chaserInitialPosition),
                                                     new Vector3D(chaserInitialVelocity));
        ...
        final PVCoordinates inertialPVcw = targetLofCW.getTransformTo(eme2000, date.shiftedBy(5572.8))
                                                      .transformPVCoordinates(
                                                                      new PVCoordinates(new Vector3D(localPosCW),
                                                                                        new Vector3D(localVelCW)));
        final PVCoordinates inertialPVya = targetLofYA.getTransformTo(eme2000, date.shiftedBy(5572.8))
                                                      .transformPVCoordinates(
                                                                      new PVCoordinates(new Vector3D(localPosYA),
                                                                                        new Vector3D(localVelYA)));
    ...
    void testSetInitialChaserPVTLof() {
        ...
        final Frame frame = FramesFactory.getEME2000();
        final KeplerianOrbit targetOrbit = new KeplerianOrbit(10000, 0, 0, 0, 0, 0,
                                                              PositionAngleType.TRUE, frame, date,
                                                              Constants.EIGEN5C_EARTH_MU);
        ...
        TestUtils.validateVector3D(pvt2.getPosition(), provider.getInitialChaserPVTLof().getPosition(), 0);
        TestUtils.validateVector3D(pvt2.getVelocity(), provider.getInitialChaserPVTLof().getVelocity(), 0);
    }
}
