...
class FieldCircularParametersTest {
    ...
    private void templateTestBackAndForthElliptical(final PositionAngleType initialType,
        ...
        // GIVEN
        final CircularParameters elements = new CircularParameters(1., 0.5, 0.5, 0.5, 0.5, 0.5, initialType);
        final FieldCircularParameters<Binary64> expectedElements = new FieldCircularParameters<>(Binary64Field.getInstance(), elements);
        // WHEN
        final FieldCircularParameters<Binary64> converted = expectedElements.withPositionAngleType(intermediateType);
        final FieldCircularParameters<Binary64> actualElements = converted.withPositionAngleType(initialType);
        // THEN
        ...
        assertEquals(expectedElements.latitudeArgument().getReal(), actualElements.latitudeArgument().getReal(), 1e-12);
    }

    @Test
    void testToCircularElements() {
        // GIVEN
        final CircularParameters expectedElements = new CircularParameters(1., 0.5, 0.5, 0.5, 0.5, 0.5,
                PositionAngleType.TRUE);
        // WHEN
        final FieldCircularParameters<Binary64> fieldElements = new FieldCircularParameters<>(Binary64Field.getInstance(),
                expectedElements);
        final CircularParameters actualElements = fieldElements.toCircularElements();
        // THEN
