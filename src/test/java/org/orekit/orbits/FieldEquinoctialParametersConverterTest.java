...
class FieldEquinoctialParametersConverterTest {

    @ParameterizedTest
    @ValueSource(doubles = {-0.2, 0., 0.1, 0.5})
    void testBackAndForthEccentric(final double vx) {
        ...
        final FieldEquinoctialParametersConverter<Gradient> converter = new FieldEquinoctialParametersConverter<>(field.getOne());
        // WHEN
        final FieldEquinoctialParameters<Gradient> fieldElements = converter.toParameters(fieldPV, PositionAngleType.TRUE);
    ...
    void testToParametersEccentric(final PositionAngleType positionAngleType) {
        ...
        final FieldEquinoctialParametersConverter<Binary64> converter = new FieldEquinoctialParametersConverter<>(mu);
        // WHEN
        final FieldEquinoctialParameters<Binary64> fieldElements = converter.toParameters(fieldPV, positionAngleType);
        // THEN
        final EquinoctialParameters elements = new EquinoctialParametersConverter(mu.getReal()).toParameters(pv, positionAngleType);
        assertEquals(elements, fieldElements.toEquinoctialElements());
    }
    ...
    void testToCartesianElliptic(final PositionAngleType positionAngleType) {
        // GIVEN
        final Binary64Field field = Binary64Field.getInstance();
        final EquinoctialParameters elements = new EquinoctialParameters(1., 0.1, 2, 3, 4, 5, positionAngleType);
        final FieldEquinoctialParameters<Binary64> fieldElements = new FieldEquinoctialParameters<>(field, elements);
        final Binary64 mu = field.getOne();
        final FieldEquinoctialParametersConverter<Binary64> converter = new FieldEquinoctialParametersConverter<>(mu);
        // WHEN
        final FieldPVCoordinates<Binary64> fieldPV = converter.toCartesian(fieldElements);
        // THEN
        final PVCoordinates pv = new EquinoctialParametersConverter(mu.getReal()).toCartesian(elements);
