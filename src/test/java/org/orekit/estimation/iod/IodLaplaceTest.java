...
public class IodLaplaceTest extends AbstractIodTest {
    ...
    void testLaplaceKeplerian1() {
        ...
        assertEquals(0.0, error[0], 275.0);
        assertEquals(0.0, error[1], 0.8);
    }

    // Estimate the orbit of Galaxy 15 based on Keplerian motion
    @Test
    void testLaplaceKeplerian2() {
        final AbsoluteDate date = new AbsoluteDate(2019, 9, 29, 22, 0, 2.0, TimeScalesFactory.getUTC());
        ...
        assertEquals(0.0, error[0], 395.0);
        assertEquals(0.0, error[1], 0.03);
    }

    // Estimate the orbit of ISS (ZARYA) based on TLE propagation
    @Test
    void testLaplaceTLE1() {
        ...
        assertEquals(0.0, error[0], 5000.0);
        assertEquals(0.0, error[1], 10.0);
    }

    // Estimate the orbit of COSMOS 382 based on TLE propagation
    @Test
    void testLaplaceTLE2() {
    ...
    void testIssue753() {
        ...
        // Verify
        assertEquals(0.0, ref.getPosition().distance(orbit.getPosition()), 275.0);
        assertEquals(0.0, ref.getVelocity().distance(orbit.getPVCoordinates().getVelocity()), 0.8);
    ...
    void testLaplaceKeplerianWithAzEl() {
        ...
        assertEquals(0.0, Vector3D.distance(truth.getPosition(), estOrbit.getPosition()), 275.0);
        assertEquals(0.0, Vector3D.distance(truth.getVelocity(), estOrbit.getPVCoordinates().getVelocity()), 0.8);
    }
