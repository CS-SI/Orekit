...
public class OnBoardAntennaInterSatellitesPhaseModifierTest {
    ...
    public void testPreliminary() {
        ...
        // this test does not check OnBoardAntennaInterSatellitesPhaseModifier at all,
        // it just checks InterSatellitesPhaseMeasurementCreator behaves as necessary for the other test
        ...
        // create perfect inter-satellites phase measurements with antenna offset
        ...
        for (int i = 0; i < spacecraftCenteredMeasurements.size(); ++i) {
            ...
            Assert.assertTrue((ar.getObservedValue()[0] - sr.getObservedValue()[0]) * FREQUENCY.getWavelength() >= -1.0);
            Assert.assertTrue((ar.getObservedValue()[0] - sr.getObservedValue()[0]) * FREQUENCY.getWavelength() <= -0.36);
        }
    }
    ...
    public void testEffect() {
        ...
        OnBoardAntennaInterSatellitesPhaseModifier modifier = new OnBoardAntennaInterSatellitesPhaseModifier(apc1, apc2);
        for (int i = 0; i < spacecraftCenteredMeasurements.size(); ++i) {
            InterSatellitesPhase sr = (InterSatellitesPhase) spacecraftCenteredMeasurements.get(i);
            sr.addModifier(modifier);
            EstimatedMeasurement<InterSatellitesPhase> estimated = sr.estimate(0, 0, new SpacecraftState[] {
                                                                                         p3.propagate(sr.getDate()),
                                                                                         ephemeris.propagate(sr.getDate())
                                                                                     });
            InterSatellitesPhase ar = (InterSatellitesPhase) antennaCenteredMeasurements.get(i);
            Assert.assertEquals(0.0, sr.getDate().durationFrom(ar.getDate()), 2.0e-8);
            Assert.assertEquals(ar.getObservedValue()[0] * FREQUENCY.getWavelength(), estimated.getEstimatedValue()[0] * FREQUENCY.getWavelength(), 2.0e-5);
        }
