...
class TwoWaySignalTravelTimerTest {
    ...
    void testComputeNoVelocity() {
        ...
        final TwoWaySignalTravelTimer twoWaySignalTravelTimer = new TwoWaySignalTravelTimer(new SignalTravelTimeModel());
        ...
        // WHEN
        final double[] delays = twoWaySignalTravelTimer.computeDelays(frame, receiverPosition, date, relay,
                emitter);
        // THEN
    ...
    void testComputeField() {
        ...
        // WHEN
        final Gradient[] fieldDelays = twoWaySignalTravelTimer.computeDelays(frame, new FieldVector3D<>(field, receiverPosition),
                fieldDate, new FieldAbsolutePVCoordinates<>(field, relay), new FieldAbsolutePVCoordinates<>(field, emitter));
        // THEN
        final double[] delays = twoWaySignalTravelTimer.computeDelays(frame, receiverPosition, date, relay, emitter);
