...
class EstimationsProviderTest {
    ...
    void testGetRejectionIndicesNone(final EstimatedMeasurementBase.Status status) {
        ...
        when(provider.getProcessedIndices()).thenCallRealMethod();
        when(provider.getNumber()).thenReturn(10);
        final EstimatedMeasurement mockedMeasurement = mockMeasurement(status);
        for (int index = 0; index < provider.getNumber(); index++) {
            when(provider.getEstimatedMeasurement(index)).thenReturn(mockedMeasurement);
        }
        // WHEN
        final int[] indices = provider.getProcessedIndices();
        // THEN
        switch (status) {
            case PROCESSED:
