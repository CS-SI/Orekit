...
class EarthBasedStationTest {
    ...
    @Test
    void testGetEstimatedEarthFrame() {
        // GIVEN
        ...
        final EarthBasedStation earthBasedStation = new EarthBasedStation(topocentricFrame);
        // WHEN
        final Frame provider = earthBasedStation.getEstimatedEarthFrame();
        // THEN
        Assertions.assertInstanceOf(EstimatedEarthFrameProvider.class, provider.getTransformProvider());
    }

    @Test
    void getPVCoordinatesProviderVersusGroundStation() {
        // GIVEN
        EstimationTestUtils.eccentricContext("regular-data:potential:tides");
        final Frame ecef = FramesFactory.getGTOD(true);
        final GeodeticPoint geodeticPoint = new GeodeticPoint(1., 2., 3.);
        ...
        earthBasedStation.getParametersDrivers().forEach(driver -> driver.setReferenceDate(date));
        earthBasedStation.getEastOffsetDriver().setValue(0.1);
        earthBasedStation.getNorthOffsetDriver().setValue(-0.1);
        earthBasedStation.getZenithOffsetDriver().setValue(0.2);
        // WHEN
        final PVCoordinatesProvider actualProvider = earthBasedStation.getPVCoordinatesProvider();
        // THEN
        final Frame gcrf = FramesFactory.getGCRF();
        final GroundStation groundStation = new GroundStation(topocentricFrame);
        groundStation.getParametersDrivers().forEach(driver -> driver.setReferenceDate(date));
        groundStation.getEastOffsetDriver().setValue(earthBasedStation.getEastOffsetDriver().getValue());
        groundStation.getNorthOffsetDriver().setValue(earthBasedStation.getNorthOffsetDriver().getValue());
        groundStation.getZenithOffsetDriver().setValue(earthBasedStation.getZenithOffsetDriver().getValue());
        final PVCoordinatesProvider expectedProvider = groundStation.getPVCoordinatesProvider();
        Assertions.assertArrayEquals(expectedProvider.getPosition(date, gcrf).toArray(),
                actualProvider.getPosition(date, gcrf).toArray(), 1e-6);
        Assertions.assertArrayEquals(expectedProvider.getVelocity(date, gcrf).toArray(),
                actualProvider.getVelocity(date, gcrf).toArray(), 1e-4);
    }
    ...
    void getPVCoordinatesProvider() {
        ...
        final TopocentricFrame topocentricFrame = new TopocentricFrame(new OneAxisEllipsoid(Constants.WGS84_EARTH_EQUATORIAL_RADIUS,
                Constants.WGS84_EARTH_FLATTENING, ecef), geodeticPoint, "");
        final EarthBasedStation station = new EarthBasedStation(topocentricFrame);
        final AbsoluteDate date = AbsoluteDate.ARBITRARY_EPOCH;
    ...
    void testEstimateStationPosition() {
        ...
        final EarthBasedStation moved = new EarthBasedStation(new TopocentricFrame(parent,
    ...
    void testEstimateEOP() {
        ...
        final NumericalPropagatorBuilder linearPropagatorBuilder =
                        linearEOPContext.createNumerical(OrbitParamsType.KEPLERIAN, PositionAngleType.TRUE, true,
                                              1.0e-6, 60.0, 0.001);
        ...
        for (final ObservedMeasurement<?> linearMeasurement : linearMeasurements) {
            ...
            for (final EarthBasedStation station : zeroEOPContext.stations) {
        ...
        EarthBasedStation station = zeroEOPContext.stations.getFirst();
...
//         Assertions.assertEquals(ypDot, computedYpDot, 3.8e-8);
    ...
    void testNoReferenceDateGradient() {
        ...
        final EarthBasedStation station = new EarthBasedStation(new TopocentricFrame(earth,
    ...
    private UnivariateDifferentiableVectorFunction differentiatedStationPV(final EarthBasedStation station,
                                                                           final Frame eme2000,
    ...
    private UnivariateDifferentiableVectorFunction differentiatedTransformAngular(final EarthBasedStation station,
                                                                                  final Frame eme2000,
                                                                                  final AbsoluteDate date,
                                                                                  final ParameterDriver driver,
                                                                                  final double stepFactor) {
    ...
    private ParameterDriver[] selectAllDrivers(final EarthBasedStation station) {
