...
public class CssiSpaceWeatherLoaderTest {
    ...
    private CssiSpaceWeatherLoader loadCswl() {
        CssiSpaceWeatherLoader cswl = new CssiSpaceWeatherLoader("SpaceWeather-All-v1.2_snapshot_20200224.txt");
        DataProvidersManager manager = DataContext.getDefault().getDataProvidersManager();
        manager.feed(cswl.getSupportedNames(), cswl);
        return cswl;
    }

    @Test
    public void testMinDate() {
        CssiSpaceWeatherLoader cswl = loadCswl();
    ...
    public void testThreeHourlyKpObserved() {
        CssiSpaceWeatherLoader cswl = loadCswl();
        AbsoluteDate date = new AbsoluteDate(1957, 10, 3, 17, 17, 0.0, utc);
        final double kp = cswl.getThreeHourlyKP(date);
        assertThat(kp, closeTo(4.7, 1e-10));
    }
    ...
    public void testThreeHourlyApObservedSecondDay() {
        CssiSpaceWeatherLoader cswl = loadCswl();
        AbsoluteDate date = new AbsoluteDate(1957, 10, 2, 5, 17, 0.0, utc);
        try {
            final double[] ap = cswl.getAp(date);
            Assert.fail("an exception should have been thrown");
        } catch (OrekitException oe) {
    ...
    public void testThreeHourlyApObservedThirdDay() {
        CssiSpaceWeatherLoader cswl = loadCswl();
        AbsoluteDate date = new AbsoluteDate(1957, 10, 3, 17, 17, 0.0, utc);
        final double[] apExpected = new double[] { 19, 39, 22, 18, 5, 9.125, 18.875 };
        final double[] ap = cswl.getAp(date);
        for (int i = 0; i < 7; i++) {
