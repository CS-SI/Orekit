...
public class MariniMurrayModelTest {
    ...
    public void testSameDelay() {
        ...
        final double delay1 = model.pathDelay(FastMath.toRadians(elevation), new GeodeticPoint(latitude, longitude, height), model.getParameters(), AbsoluteDate.J2000_EPOCH);
        ...
        final double[] zenith  = model.computeZenithDelay(new GeodeticPoint(latitude, longitude, height), model.getParameters(), AbsoluteDate.J2000_EPOCH);
        final double[] mapping = model.mappingFactors(elevation, new GeodeticPoint(latitude, longitude, height), model.getParameters(), AbsoluteDate.J2000_EPOCH);
        // Delay
    ...
    private <T extends RealFieldElement<T>> void doTestFieldSameDelay(final Field<T> field) {
        ...
        // Compute delay using pathDelay method
        final T delay1 = model.pathDelay(elevation, new FieldGeodeticPoint<>(zero.add(latitude), zero.add(longitude), height), model.getParameters(field), FieldAbsoluteDate.getJ2000Epoch(field));

        // Compute delay using zenith delay and mapping factors
        // For Marini Murray model, the delay is not split into hydrostatic and non-hydrostatic parts
        // In that respect, mapping function is equal to 1.0 for for both components and for any elevation angle
        final T[] zenith  = model.computeZenithDelay(new FieldGeodeticPoint<>(zero.add(latitude), zero.add(longitude), height), model.getParameters(field), FieldAbsoluteDate.getJ2000Epoch(field));
        final T[] mapping = model.mappingFactors(elevation, new FieldGeodeticPoint<>(zero.add(latitude), zero.add(longitude), height), model.getParameters(field), FieldAbsoluteDate.getJ2000Epoch(field));
        // Delay
