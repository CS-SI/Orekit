...
class Rtcm1066Test {
    ...
    void testPerfectValue() {
        ...
        // Verify size
        assertEquals(1,                            rtcm1066.getData().size());

        // Verify header
        assertEquals(1066,                         rtcm1066.getTypeCode());
        assertEquals(64711.0,                      rtcm1066.getHeader().getEpochTime1s(), eps);
        assertEquals(30.0,                         rtcm1066.getHeader().getSsrUpdateInterval().getUpdateInterval(), eps);
        assertEquals(0,                            rtcm1066.getHeader().getMultipleMessageIndicator());
        assertEquals(7,                            rtcm1066.getHeader().getIodSsr());
        assertEquals(3951,                         rtcm1066.getHeader().getSsrProviderId());
        assertEquals(1,                            rtcm1066.getHeader().getSsrSolutionId());
        assertEquals(1,                            rtcm1066.getHeader().getNumberOfSatellites());

        // Verify data for satellite R01
        final RtcmCombinedCorrectionData r01 = rtcm1066.getDataMap().get("R01").get(0);
        assertEquals(1,                            r01.getSatelliteID());
        assertEquals(132,                          r01.getGnssIod());
        assertEquals(18.0095,                      r01.getOrbitCorrection().getDeltaOrbitRadial(),        eps);
        assertEquals(122.8668,                     r01.getOrbitCorrection().getDeltaOrbitAlongTrack(),    eps);
        assertEquals(122.8668,                     r01.getOrbitCorrection().getDeltaOrbitCrossTrack(),    eps);
        assertEquals(0.090047,                     r01.getOrbitCorrection().getDotOrbitDeltaRadial(),     eps);
        assertEquals(0.614332,                     r01.getOrbitCorrection().getDotOrbitDeltaAlongTrack(), eps);
        assertEquals(0.614332,                     r01.getOrbitCorrection().getDotOrbitDeltaCrossTrack(), eps);
        assertEquals(96.6527,                      r01.getClockCorrection().getDeltaClockC0(),            eps);
        assertEquals(0.483263,                     r01.getClockCorrection().getDeltaClockC1(),            eps);
        assertEquals(0.61857734,                   r01.getClockCorrection().getDeltaClockC2(),            eps);
    }
