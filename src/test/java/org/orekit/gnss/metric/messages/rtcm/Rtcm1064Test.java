...
public class Rtcm1064Test {
    ...
    public void testPerfectValue() {
        final Rtcm1064 rtcm1064 = (Rtcm1064) new RtcmMessagesParser(messages).parse(message, false);

        // Verify size
        Assert.assertEquals(1,                            rtcm1064.getData().size());

        // Verify header
        Assert.assertEquals(1064,                         rtcm1064.getTypeCode());
        Assert.assertEquals(64711.0,                      rtcm1064.getHeader().getEpochTime1s(), eps);
        Assert.assertEquals(30.0,                         rtcm1064.getHeader().getSsrUpdateInterval().getUpdateInterval(), eps);
        Assert.assertEquals(0,                            rtcm1064.getHeader().getMultipleMessageIndicator());
        Assert.assertEquals(7,                            rtcm1064.getHeader().getIodSsr());
        Assert.assertEquals(3951,                         rtcm1064.getHeader().getSsrProviderId());
        Assert.assertEquals(1,                            rtcm1064.getHeader().getSsrSolutionId());
        Assert.assertEquals(1,                            rtcm1064.getHeader().getNumberOfSatellites());
