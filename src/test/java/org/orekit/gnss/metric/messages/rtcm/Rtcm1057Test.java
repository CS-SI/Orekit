...
public class Rtcm1057Test {
    ...
    public void testPerfectValue() {
        ...
        // Verify size
        Assert.assertEquals(1,                            rtcm1057.getData().size());

        // Verify header
        Assert.assertEquals(1057,                         rtcm1057.getTypeCode());
        Assert.assertEquals(517695.0,                     rtcm1057.getHeader().getEpochTime1s(), eps);
        Assert.assertEquals(30.0,                         rtcm1057.getHeader().getSsrUpdateInterval().getUpdateInterval(), eps);
        Assert.assertEquals(0,                            rtcm1057.getHeader().getMultipleMessageIndicator());
        Assert.assertEquals(7,                            rtcm1057.getHeader().getIodSsr());
        Assert.assertEquals(3951,                         rtcm1057.getHeader().getSsrProviderId());
        Assert.assertEquals(1,                            rtcm1057.getHeader().getSsrSolutionId());
        Assert.assertEquals(1,                            rtcm1057.getHeader().getNumberOfSatellites());
