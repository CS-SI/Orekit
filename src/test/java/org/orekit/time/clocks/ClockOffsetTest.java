...
class ClockOffsetTest {
    ...
    void testConstructorException() {
        Assertions.assertThrows(OrekitException.class, () ->
                new ClockOffset(AbsoluteDate.ARBITRARY_EPOCH, new double[4]));
    }
    ...
    void testGetValue() {
        final ClockOffset clockOffset = new ClockOffset(AbsoluteDate.ARBITRARY_EPOCH, 1.0, 2.0,  3.0);
        final AbsoluteDate date = clockOffset.getDate().shiftedBy(1.0);
        final double offset = clockOffset.getValue(date);
        Assertions.assertEquals(6.0 , offset, 1.0e-15);
    }
}
