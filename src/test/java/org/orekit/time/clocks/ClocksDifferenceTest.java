...
public class ClocksDifferenceTest {
    ...
    public void testDouble() {
        ...
        final ClockModel clockModel = new ClocksDifference(new ConstantClockModel(1.5) {
            ...
            new PolynomialClockModel(t0, "dummy", 0.0, -1.0, -0.25) {
            @Override
            public AbsoluteDate getValidityStart() {
    ...
    public <T extends CalculusFieldElement<T>> void doTestField(final Field<T> field) {
        final AbsoluteDate t0 = AbsoluteDate.ARBITRARY_EPOCH;
        final ClocksDifference clockModel = new ClocksDifference(new ConstantClockModel(1.5),
                                                                 new PolynomialClockModel(t0, "dummy",
                                                                                          0.0, -1.0, -0.25));
        final FieldClocksDifference<T> fieldClockModel = clockModel.toField(v -> field.getZero().newInstance(v));
    ...
    void testGetFieldModel() {
        ...
        // WHEN & THEN
        final FieldClocksDifference<Gradient> g = clockModel.toGradient(4, new HashMap<>());
        final FieldAbsoluteDate<Gradient> gDate = FieldAbsoluteDate.getArbitraryEpoch(GradientField.getField(4));
        Assertions.assertEquals(4, g.getOffset(gDate).getBias().getFreeParameters());
    }
