...
class BDSScaleTest {
    ...
    void testT0() {
        ...
        assertEquals("BDT", scale.toString());
        AbsoluteDate t0 =
            new AbsoluteDate(new DateComponents(2006, 1, 1), new TimeComponents(0, 0, 13), scale);
        assertEquals(new AbsoluteDate(new DateComponents(2006, 1, 1), new TimeComponents(0, 0, 13),
    ...
    void testAwayFromGPS() {
        ...
        for (double dt = -10000; dt < 10000; dt += 123.456789) {
            ...
            assertEquals(gps.offsetFromTAI(date) - 14, bdt.offsetFromTAI(date), 1.0e-15);
        }
    }
