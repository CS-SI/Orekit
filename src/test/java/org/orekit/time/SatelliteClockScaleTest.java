...
class SatelliteClockScaleTest {
    ...
    @Test
    void testZeroAtEpoch() {
        final double    offset = 0.0;
        final double    drift  = 0.001;
        final TimeScale sclk   = new SatelliteClockScale("SCLK", epoch, utc, offset, drift);
        assertEquals("SCLK", sclk.toString());
        assertEquals(-25.0, sclk.offsetFromTAI(epoch), 1.0e-12);
        assertEquals(-24.0, sclk.offsetFromTAI(epoch.shiftedBy(1000)), 1.0e-12);
    }

    @Test
    void testNonZeroAtEpoch() {
        final double    offset = 325.0;
        final double    drift  = 0.002;
        final TimeScale sclk   = new SatelliteClockScale("SCLK", epoch, utc, offset, drift);
        assertEquals("SCLK", sclk.toString());
        assertEquals(300.0, sclk.offsetFromTAI(epoch), 1.0e-12);
        assertEquals(302.0, sclk.offsetFromTAI(epoch.shiftedBy(1000)), 1.0e-12);
    }
    ...
    void testField() {
        ...
        assertEquals(-24.0, sclk.offsetFromTAI(date).getReal(), 1.0e-12);
    }
    ...
    void testParseEpoch() {
        ...
        final AbsoluteDate date   = new AbsoluteDate(dtc.getDate(),
                                               new TimeComponents(dtc.getTime().getSecondsInUTCDay() +
                                                                  offset),
                                               sclk);
        assertEquals(0.0, date.durationFrom(epoch), 2.0e-12);
    }
    ...
    void testParse() {
        ...
        final AbsoluteDate date   = new AbsoluteDate(dtc.getDate(),
                                               new TimeComponents(dtc.getTime().getSecondsInUTCDay() +
                                                                  offset + shift),
                                               sclk);
        assertEquals(shift / (1 + drift), date.durationFrom(epoch), 2.0e-12);
    }

    @Test
    void testSymmetryDrift() {
        final double       offset = 325.0;
        final double       drift  = 10.002;
        final TimeScale    sclk   = new SatelliteClockScale("SCLK", epoch, utc, offset, drift);
        for (double dt = -1000; dt < 1000; dt += 0.01) {
            ...
            assertEquals(0.0, rebuilt.durationFrom(ref), 6.0e-12);
        }
    }

    @Test
    void testCountSymmetry() {
        final double              offset = 325.0;
        final double              drift  = 10.002;
        ...
        for (double count = -1000; count < 1000; count += 0.01) {
            ...
            assertEquals(count, rebuilt, 2.0e-13);
        }
    }
    ...
    void testCount() {
        ...
        final double              drift  = 10.002;
        final SatelliteClockScale sclk   = new SatelliteClockScale("SCLK", epoch, utc, offset, drift);
        assertEquals(0.0,    sclk.dateAtCount(offset).durationFrom(epoch), 1.0e-15);
        assertEquals(offset, sclk.countAtDate(epoch),                      1.0e-15);
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            assertEquals(drift, deltaCount / deltaT - 1.0, 2.0e-12);
            previous = current;
        }
    }
