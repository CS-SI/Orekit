...
/**
 * This class represents the effect of the Moon as a third body.
...
public class ThirdBody_Moon extends ThirdBody{
   ...
   */
    public ThirdBody_Moon() {
        super(Constants.Moonradius, Constants.Moonmu);
    }
    
   /** Gets the position of the Moon wrt the central body.
   * @param date date
   * @return MoonBody the Moon's position vector3D
   */
    public Vector3D getPosition(RDate t) throws OrekitException {
       // computation of the Moon position wrt the central body (units: m)
       // ================================================================
      ...
      b = b - 6.0 * Math.sin(xlp - f + d + d) - 7.0 * Math.sin(xl + xl + f - d - d);
      ...
      Vector3D centralMoon = new Vector3D();
      centralMoon.setCoordinates(rx * cr + ry * sr, 
                               ry * cr - rx * sr, 
                               sb * ce + su * cb * se);

      double dasr = 5450.0 * Math.cos(xl) + 1002.0 * Math.cos(xl - d - d) + 
                    825.0 * Math.cos(d + d);
      ...
      dasr = dasr + 11.0 * Math.cos(xl + xl - 4.0 * d) + 18.0 * Math.cos(3.0 * xl);
      dasr = dasr - 9.0 * Math.cos(xlp + d + d) - 7.0 * Math.cos(xl - xlp - d - d);
      ...
      // units: km
      Vector3D moonBody = new Vector3D();
      double denom = 1.0 + 1.E-05 * dasr;
      if (denom < Constants.Epsilon) 
         {throw new OrekitException("variable 'factor' is equal to 0");}
      double factor = 1000.0 * 384389.3 / denom;
      moonBody.setCoordinates(factor * centralMoon.getX(),
                              factor * centralMoon.getY(),
                              factor * centralMoon.getZ());
