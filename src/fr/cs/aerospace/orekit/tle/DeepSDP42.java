 ...
 * @author F. Maussion
 */
class DeepSDP42 extends SDP42 {

  /** Constructor for a unique initial TLE.
   * @param initialTLE the TLE to propagate.
   */
  protected DeepSDP42(TLE initialTLE) {
    super(initialTLE);
  }
  
  /** Computes luni - solar terms from initial coordinates and epoch.
   * These are needed quantities for deep-space perturbations calculation.
   */
  protected void luniSolarTermsComputation() {
    ...
    zx = Math.atan2( zx, zy) + gam - xnodce; // TOTO atan
    zcosgl = Math.cos( zx);
    zsingl = Math.sin( zx);
  ...
  protected void deepPeriodicEffects(double t)  {
    ...
    xll += pl;
    omgadf += pgh;
//    if( tle.getI() >= 0.2)   { // lyddane modif : use perturbed inclination
//    if( xinc >= 0.2)   {
    if( xqncl >= 0.2)   {
      ...
      xnode = trimAngle(xnode, Math.PI); // TODO angles
      delta_xnode = Math.atan2(alfdp,betdp) - xnode;
      trimAngle(delta_xnode, 0);
...
//System.out.println(" delta before : " + delta_xnode);
...
//      System.out.println(" delta after : " + delta_xnode);
...
//      *derivs++ = term1b + term2b + term3b; 
      ...
      double sin_omi = Math.sin( xomi);
      double cos_omi = Math.cos( xomi);
