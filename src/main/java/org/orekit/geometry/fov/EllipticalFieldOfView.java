 ...
 * x^2 + y^2 + z^2 &amp;= 1
 * \end{align*}\right.\]
 * which shows that the previous definition is also equivalent to define a first a
...
public class EllipticalFieldOfView extends SmoothFieldOfView {
    ...
    public EllipticalFieldOfView(final Vector3D center, final Vector3D primaryMeridian,
        ...
        if (halfApertureAlongX >= halfApertureAlongY) {
            ...
            focus1  = new Vector3D(FastMath.sqrt(1 - cos * cos), 0.0, cos);
            focus2  = new Vector3D(-focus1.getX(), 0.0, focus1.getZ());
            sumDist = 2 * halfApertureAlongX;
        } else {
            final double cos = FastMath.cos(halfApertureAlongY) / FastMath.cos(halfApertureAlongX);
            focus1  = new Vector3D(0.0, FastMath.sqrt(1 - cos * cos), cos);
            focus2  = new Vector3D(0.0, -focus1.getY(), focus1.getZ());
            sumDist = 2 * halfApertureAlongY;
        }
    ...
    public double offsetFromBoundary(final Vector3D lineOfSight, final double angularRadius,
        ...
        // TODO compute offset as an exact angle
        final Vector3D losE = new Vector3D(Vector3D.dotProduct(lineOfSight, getX()),
                                           Vector3D.dotProduct(lineOfSight, getY()),
                                           Vector3D.dotProduct(lineOfSight, getZ()));
        final double rawOffset = Vector3D.angle(focus1, losE) + Vector3D.angle(focus2, losE) - sumDist;
    ...
    protected Vector3D directionAt(final double angle) {
        ...
        final double   a   = FastMath.tan(halfApertureAlongX);
        final double   b   = FastMath.tan(halfApertureAlongY);
        final Vector3D d   = new Vector3D(a * sce.cos(), b * sce.sin(), 1.0).normalize();
