...
class UTCTAIBulletinAFilesLoader implements UTCTAILoader {
    ...
    */
    ...
    public SortedMap<DateComponents, Integer> loadTimeSteps() throws OrekitException {
        ...
        final SortedMap<DateComponents, Integer> timeSteps = new TreeMap<DateComponents, Integer>();
        ...
        if (!taiUtc.isEmpty()) {
            ...
            for (final int leapMJD : leapDays) {
                timeSteps.put(new DateComponents(DateComponents.MODIFIED_JULIAN_EPOCH, leapMJD), ++offset);
            }
            ...
            for (final Map.Entry<Integer, Integer> refTaiMUtc : taiUtc.entrySet()) {
                ...
                if (!before.isEmpty()) {
                    ...
                    final int expected = timeSteps.get(timeStepDC);
                    if (refTaiMUtc.getValue() != expected) {
            ...
            if (timeSteps.isEmpty()) {
                timeSteps.put(new DateComponents(1972, 1, 1), taiUtc.get(taiUtc.firstKey()));
            } else {
                if (timeSteps.firstKey().getYear() > 1972) {
                    timeSteps.put(new DateComponents(1972, 1, 1), timeSteps.get(timeSteps.firstKey()) - 1);
                }
            }
        }
