...
public interface LOF {
    ...
    default <T extends CalculusFieldElement<T>> FieldTransform<T> transformFromInertial(final FieldAbsoluteDate<T> date,
        ...
        if (isQuasiInertial()) {
            ...
            final T durationFrom = date.durationFrom(nonFieldDate);
            final FieldUnivariateDerivative2<T> shift = new FieldUnivariateDerivative2<>(durationFrom, field.getOne(),
                field.getZero());
            final FieldAbsoluteDate<FieldUnivariateDerivative2<T>> fud2Date = new FieldAbsoluteDate<>(fud2Field,
                nonFieldDate).shiftedBy(shift);
            ...
            final FieldAngularCoordinates<T> fieldAngularCoordinates = new FieldAngularCoordinates<T>(fud2Rotation);
    ...
    default Transform transformFromInertial(final AbsoluteDate date, final PVCoordinates pv) {
        if (isQuasiInertial()) {
            ...
            final FieldAbsoluteDate<UnivariateDerivative2> ud2Date = new FieldAbsoluteDate<>(ud2Field, date);
            // create PV with independent variable
