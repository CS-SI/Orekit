...
public class Fieldifier {
    ...
    public static <T extends CalculusFieldElement<T>> FieldOrbit<T> fieldify(final Field<T> field, final Orbit orbit) {
        ...
        final FieldAbsoluteDate<T>  orbitDate = new FieldAbsoluteDate<>(field, orbit.getDate());

        return new FieldCartesianOrbit<>(orbitPV, orbit.getFrame(), orbitDate, field.getOne().multiply(orbit.getMu()));
    }
