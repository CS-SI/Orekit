...
public class Omm extends NdmConstituent<OdmHeader, Segment<OmmMetadata, OmmData>> implements TimeStamped {
    ...
    /** Default value for missing optional values. */
    private static final double DEFAULT_MISSING_OPTIONAL_VALUE = 0.0;
    ...
    public KeplerianOrbit generateKeplerianOrbit() {
        return getData().getKeplerianElementsBlock().generateKeplerianOrbit(getMetadata().getFrame(), mu);
    }
    ...
    public TLE generateTLE() {
        ...
        final OmmTle            tle = getData().getTLEBlock().orElseThrow(() -> new OrekitException(OrekitMessages.CCSDS_MISSING_OPTIONAL_VALUE));
        return new TLE(tle.getNoradID(), tle.getClassificationType(),
                       ...
                       kep.getMeanMotion().orElse(DEFAULT_MISSING_OPTIONAL_VALUE),
                       tle.getMeanMotionDot().orElse(DEFAULT_MISSING_OPTIONAL_VALUE) / 2,
                       tle.getMeanMotionDotDot().orElse(DEFAULT_MISSING_OPTIONAL_VALUE) / 6,
                       kep.getE(), kep.getI(), kep.getPa(), kep.getRaan(),
                       kep.getAnomaly(), tle.getRevAtEpoch(),
                       tle.getBStar().orElse(DEFAULT_MISSING_OPTIONAL_VALUE), getDataContext().getTimeScales().getUTC());
    }
