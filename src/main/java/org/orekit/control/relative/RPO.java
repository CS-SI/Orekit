 ...
 *     <li>Forced Circular Motion,</li>
 *     <li>Natural Circumnavigation.</li>
 * </ul>
 * The computeWaypoints are default methods shared by the different models.
 * The only differences between two models in the computation of these waypoints come from the local orbital frame
 * used. This is handled by the getters defined in enum:{@link RPOModel RPOModel }.
 *
...
public interface RPO {
    ...
    /**
     * Get the rotation for the waypoint in the local orbital frame to be used in the
     * computeForcedCircularMotionWaypoints method.
     *
    ...
    default Rotation rotationInLof(final double inclination, final double raan) {
        ...
        final Rotation inclinationRotation =
                        new Rotation(vBar.scalarMultiply(-1), inclination, RotationConvention.VECTOR_OPERATOR);
    ...
    default <T extends CalculusFieldElement<T>> FieldRotation<T> rotationInLof(final T inclination, final T raan) {
        ...
        final FieldVector3D<T> localVerticalDirection =
                        new FieldVector3D<>(inclination.getField(), getOutOfPlaneDirection());

        // Create rotation for the waypoints.
        final FieldRotation<T> inclinationRotation =
                        new FieldRotation<>(vBar.scalarMultiply(-1), inclination, RotationConvention.VECTOR_OPERATOR);
        final FieldRotation<T> raanRotation =
                        new FieldRotation<>(localVerticalDirection, raan, RotationConvention.VECTOR_OPERATOR);
        return raanRotation.compose(inclinationRotation, RotationConvention.VECTOR_OPERATOR);
    }
    ...
    default Vector3D circularPosition(final double radius, final double angle) {
        ...
        final Vector3D vBar = getVBarDirection();
        return vBar.scalarMultiply(-radius * FastMath.cos(angle))
                   .add(rBar.scalarMultiply(radius * FastMath.sin(angle)));
    }
    ...
    default <T extends CalculusFieldElement<T>> FieldVector3D<T> circularPosition(final T radius, final T angle) {
        ...
        return vBar.scalarMultiply(radius.negate().multiply(angle.cos()))
                   .add(rBar.scalarMultiply(radius.multiply(angle.sin())));
    }
    ...
    default List<TimeStampedPVCoordinates> computeForcedLinearWaypoints(final TimeStampedPVCoordinates initialPVT,
        ...
        if (numberOfPoints > 1) {
            ...
            for (int i = 1; i < numberOfPoints - 1; i++) {
                ...
                final Vector3D hopLength = finalPVT.getPosition().subtract(initialPVT.getPosition())
                                                   .scalarMultiply(1. / (numberOfPoints - 1));
                final TimeStampedPVCoordinates waypoint =
                                new TimeStampedPVCoordinates(initialPVT.getDate().shiftedBy(i * timeStep),
                                                             initialPVT.getPosition().add(hopLength.scalarMultiply(i)),
                                                             Vector3D.ZERO);
     ...
     * @param centerOffset        Offset of the forced circular orbit center relative to the target. If no Offset, the
     *                            center is the target position.
     * @param radius              Radius of the forced circular orbit.
     * @param inclination         Inclination of the relative orbit.
     * @param raan                Relative Longitude of ascending node of the relative circular orbit in radians. If
     *                            raan = 0, the line of apsides is along the positive RBar direction (X+ axis in QSW
     *                            frame, Z- in LVLH CCSDS frame).
     * @param orbitDuration       Relative orbital Period, the time it takes to the chaser to perform a complete force
     *                            circular orbit.
     ...
     * @param retrograde          Make the relative circular motion retrograde (clockwise viewed from the target's
    ...
    default List<TimeStampedPVCoordinates> computeForcedCircularMotionWaypoints(final AbsoluteDate startDate,
        ...
        for (int i = 0; i < points.size(); i++) {
            final TimeStampedPVCoordinates pvt =
                            new TimeStampedPVCoordinates(startDate.shiftedBy(timeStep * i), points.get(i));
            waypoints.add(pvt);
        }
        ...
        if (numberOfRevolutions > 1) {
            ...
            for (int i = 1; i < numberOfRevolutions; i++) {
                for (TimeStampedPVCoordinates revolutionWaypoint : revolutionWaypoints) {
                    ...
                    final TimeStampedPVCoordinates waypoint =
                                    new TimeStampedPVCoordinates(waypointDate, revolutionWaypoint.getPosition(),
                                                                 revolutionWaypoint.getVelocity());
    ...
    /**
     * Computes the injection PVT for a natural circumnavigation around a target in a <b>circular</b> orbit.
    ...
    default TimeStampedPVCoordinates computeNaturalCircumnavigationInjectionCircular(final AbsoluteDate startDate,
        ...
        final Vector3D rBar = getRBarDirection();
        final Vector3D vBar = getVBarDirection();
        final TimeStampedPVCoordinates injectionPoint =
                        new TimeStampedPVCoordinates(startDate, vBar.scalarMultiply(-distanceAlongVBar), Vector3D.ZERO);
     ...
     */
    List<TimeStampedPVCoordinates> computeTeardropWaypoints(AbsoluteDate injectionDate, Orbit targetOrbit,
                                                            double turnAroundDistance, double maneuverDistance,
                                                            int numberOfTeardrops);
     ...
     * @return list of waypoints along the linear path.
     */
    default <T extends CalculusFieldElement<T>> List<TimeStampedFieldPVCoordinates<T>> computeForcedLinearWaypoints(
                    final TimeStampedFieldPVCoordinates<T> initialPVT, final TimeStampedFieldPVCoordinates<T> finalPVT,
                    final int numberOfPoints) {
        // Create a list of waypoints.
        ...
        if (numberOfPoints > 1) {
            ...
            final FieldVector3D<T> zeroVector = FieldVector3D.getZero(finalPVT.getDate().getField());

            // Compute the waypoints along the linear path.
            for (int i = 1; i < numberOfPoints - 1; i++) {

                final FieldVector3D<T> hopLength = finalPVT.getPosition().subtract(initialPVT.getPosition())
                                                           .scalarMultiply(1. / (numberOfPoints - 1));
                final TimeStampedFieldPVCoordinates<T> waypoint =
                                new TimeStampedFieldPVCoordinates<>(initialPVT.getDate().shiftedBy(timeStep.multiply(i)),
                                                                    new FieldPVCoordinates<>(
                                                                                    initialPVT.getPosition()
                                                                                              .add(hopLength.scalarMultiply(i)),
                                                                                    zeroVector));
                waypointsList.add(waypoint);
            }
        }
     ...
     * @return list of waypoints.
     */
    default <T extends CalculusFieldElement<T>> List<TimeStampedFieldPVCoordinates<T>> computeForcedCircularMotionWaypoints(
                    final FieldAbsoluteDate<T> startDate, final FieldVector3D<T> centerOffset, final T radius,
                    final T inclination, final T raan, final T orbitDuration, final int pointsOnOrbit,
                    final int numberOfRevolutions, final T startAngle, final boolean retrograde) {
        ...
        // Reorder the waypoints on the circular path. The first waypoint is the previous closest waypoint.
        for (int i = 0; i < points.size(); i++) {
            final TimeStampedFieldPVCoordinates<T> pvt =
                            new TimeStampedFieldPVCoordinates<>(startDate.shiftedBy(timeStep.multiply(i)),
                                                                points.get(i));
            waypoints.add(pvt);
        }
        ...
        if (numberOfRevolutions > 1) {
            ...
            for (int i = 1; i < numberOfRevolutions; i++) {
                for (TimeStampedFieldPVCoordinates<T> revolutionWaypoint : revolutionWaypoints) {
                    final FieldAbsoluteDate<T> waypointDate =
                                    revolutionWaypoint.getDate().shiftedBy(orbitDuration.multiply(i));
                    final TimeStampedFieldPVCoordinates<T> waypoint = new TimeStampedFieldPVCoordinates<>(waypointDate,
                                                                                                          new FieldPVCoordinates<>(
                                                                                                                          revolutionWaypoint.getPosition(),
                                                                                                                          revolutionWaypoint.getVelocity()));
                    revPoints.add(waypoint);
                }
            }
    ...
    default <T extends CalculusFieldElement<T>> TimeStampedFieldPVCoordinates<T> computeNaturalCircumnavigationInjectionCircular(
                    final FieldAbsoluteDate<T> startDate, final T distanceAlongVBar, final T inclination,
        ...
        final TimeStampedFieldPVCoordinates<T> injectionPoint =
                        new TimeStampedFieldPVCoordinates<>(startDate,
                                                            new FieldPVCoordinates<>(
                                                                            vBar.scalarMultiply(
                                                                                            distanceAlongVBar.negate()),
        ...
        if (inclination.getReal() != 0) {
            ...
            final FieldRotation<T> rotation =
                            new FieldRotation<>(vBar, inclination, RotationConvention.VECTOR_OPERATOR);

            // Compute the rotation of the inclination around the vBar axis of the target's LOF
            // The scale factor 1/cos(i) ensures that the starting distance is the one specified when the object was constructed
            velocityAfterMan = rotation.applyTo(velocityAfterMan).scalarMultiply(field.getOne().divide(sci.cos()));
        }
        return new TimeStampedFieldPVCoordinates<>(startDate, new FieldPVCoordinates<>(injectionPoint.getPosition(),
                                                                                       velocityAfterMan));
    }
     ...
     *                           (in between the planet and the target), while positive means above the target (target
     *                           is in between the chaser and the planet).
     ...
     * @return List of waypoints in time. Date, position, and velocity are non-zero.
     */
    <T extends CalculusFieldElement<T>> List<TimeStampedFieldPVCoordinates<T>> computeTeardropWaypoints(
                    FieldAbsoluteDate<T> injectionDate, FieldOrbit<T> targetOrbit, T turnAroundDistance,
                    T maneuverDistance, int numberOfTeardrops);

    /**
     * Compute relative maneuvers to realize a forced trajectory defined by the waypoints
     * (ForcedLinear/ForcedCircular).
     *
     ...
     * <p><b>Warning</b>: EventDetector of the maneuvers must be DateDetector.</p>
     *
     ...
     */
    default List<ImpulseManeuver> convertToImpulseManeuver(final List<RelativeManeuver> maneuvers,
                                                           final Orbit targetOrbit, final double isp) {
        ...
        for (RelativeManeuver maneuver : maneuvers) {
            final AbsoluteDate maneuverDate = ((DateDetector) maneuver.getDetector()).getDate();
            final PVCoordinates pvTarget = targetPropagator.propagate(maneuverDate).getPVCoordinates();
            final Transform lofToInertial = getLOFType().transformFromInertial(maneuverDate, pvTarget).getInverse();
            final Vector3D deltaVInertial = lofToInertial.transformVector(maneuver.getDeltaV());
            impulseManeuvers.add(new ImpulseManeuver(maneuver.getDetector(), deltaVInertial, isp));
        }
    ...
    default <T extends CalculusFieldElement<T>> List<FieldImpulseManeuver<T>> convertToImpulseManeuver(
        ...
        for (FieldRelativeManeuver<T> maneuver : maneuvers) {
            final FieldAbsoluteDate<T> maneuverDate = ((FieldDateDetector<T>) maneuver.getDetector()).getDate();
            ...
            final FieldTransform<T> lofToInertial =
                            getLOFType().transformFromInertial(maneuverDate, pvTarget).getInverse();
            final FieldVector3D<T> deltaVInertial = lofToInertial.transformVector(maneuver.getDeltaV());
            impulseManeuvers.add(new FieldImpulseManeuver<>(maneuver.getDetector(), deltaVInertial, isp));
        }
        return impulseManeuvers;
    }
