...
public enum RPOModel implements RPO {
    ...
    CW {
        ...
        public List<TimeStampedPVCoordinates> computeTeardropWaypoints(final AbsoluteDate injectionDate,
                                                                       final Orbit targetOrbit,
                                                                       final double turnAroundDistance,
                                                                       final double maneuverDistance,
                                                                       final int numberOfTeardrops) {
            return new TeardropCircularWaypointCalculator(targetOrbit.getKeplerianMeanMotion(), turnAroundDistance,
                                                          maneuverDistance, numberOfTeardrops).computeTearDropWaypoints(
                            injectionDate);
        }
         ...
         */
        @Override
        public <T extends CalculusFieldElement<T>> List<TimeStampedFieldPVCoordinates<T>> computeTeardropWaypoints(
                        final FieldAbsoluteDate<T> injectionDate, final FieldOrbit<T> targetOrbit,
                        final T turnAroundDistance, final T maneuverDistance, final int numberOfTeardrops) {
            return new FieldTeardropCircularWaypointCalculator<T>(targetOrbit.getKeplerianMeanMotion(),
                                                                  turnAroundDistance, maneuverDistance,
                                                                  numberOfTeardrops).computeTearDropWaypoints(
                            injectionDate);
        }
         ...
         */
        @Override
        public List<RelativeManeuver> computeForcedManeuvers(final List<TimeStampedPVCoordinates> waypoints,
                                                             final Vector3D initialVelocity, final Orbit targetOrbit,
            ...
            for (int i = 0; i < waypoints.size() - 1; i++) {
                ...
                final Vector3D velocityAfterManeuver =
                                ClohessyWiltshireRendezVous.computeRendezVous(currentWaypoint, nextWaypoint, targetLof,
                                                                              targetOrbit).getPvt1().getVelocity();
                ...
                final ClohessyWiltshireManeuver maneuver = new ClohessyWiltshireManeuver(firstImpulseTrigger, deltaV,
                                                                                         (ClohessyWiltshireProvider) cwProvider);
                ...
                final ClohessyWiltshireMatrices matrices = ClohessyWiltshireEquations.computeMatrices(transferDuration,
                                                                                                      targetOrbit.getKeplerianMeanMotion());
                velocityBeforeManeuver = new Vector3D(matrices.getPhiVR().operate(MatrixUtils.createRealVector(
                                                                              currentWaypoint.getPosition().toArray())).add(matrices.getPhiVV()
                                                                                                                                    .operate(MatrixUtils.createRealVector(
                                                                                                                                                    velocityAfterManeuver.toArray())))
                                                              .toArray());
            }
        ...
        public <T extends CalculusFieldElement<T>> List<FieldRelativeManeuver<T>> computeForcedManeuvers(
            ...
            for (int i = 0; i < waypoints.size() - 1; i++) {
                ...
                // Compute the velocity after the maneuver to arrive at the following waypoint at the correct date using RendezVous.
                final FieldVector3D<T> velocityAfterManeuver =
                                (new FieldClohessyWiltshireRendezVous<T>()).computeRendezVous(currentWaypoint,
                                                                                              nextWaypoint, targetLof,
                                                                                              targetOrbit).getPvt1()
                                                                           .getVelocity();
                ...
                // Create the Clohessy-Wiltshire maneuver and add it to the list.
                final FieldClohessyWiltshireManeuver<T> maneuver =
                                new FieldClohessyWiltshireManeuver<>(firstImpulseTrigger, deltaV,
                                                                     (FieldClohessyWiltshireProvider<T>) cwProvider);
                maneuvers.add(maneuver);

                // Compute the velocity before the maneuver at the next waypoint.
                final T transferDuration = nextWaypoint.getDate().durationFrom(currentWaypoint.getDate());
                final FieldClohessyWiltshireMatrices<T> matrices =
                                (new FieldClohessyWiltshireEquations<T>()).computeMatrices(transferDuration,
                                                                                           targetOrbit.getKeplerianMeanMotion());
                final FieldVector<T> position = MatrixUtils.createFieldVector(currentWaypoint.getPosition().toArray());
                final FieldVector<T> velocity = MatrixUtils.createFieldVector(velocityAfterManeuver.toArray());
                velocityBeforeManeuver =
                                new FieldVector3D<>(matrices.getPhiVR().operate(position).
                                                add(matrices.getPhiVV().operate(velocity)).toArray());
            }
            return new ArrayList<>(maneuvers);
        }
         ...
         */
        @Override
        public List<RelativeManeuver> computeTeardropManeuvers(final List<TimeStampedPVCoordinates> waypoints,
                                                               final RelativeProvider cwProvider) {
            ...
            for (int i = 1; i < waypoints.size(); i++) {
                ...
                final TimeStampedPVCoordinates pvtBeforeMan =
                                new TimeStampedPVCoordinates(maneuverWaypoint.getDate(), maneuverWaypoint.getPosition(),
                                                             new Vector3D(-maneuverVelocity.getX(),
                                                                          maneuverVelocity.getY(),
                                                                          maneuverVelocity.getZ()));
                final Vector3D deltaV = maneuverVelocity.subtract(pvtBeforeMan.getVelocity());
                final RelativeManeuver maneuver =
                                new ClohessyWiltshireManeuver(maneuverDate, deltaV,
                                                              (ClohessyWiltshireProvider) cwProvider);
                maneuvers.add(maneuver);
            }
            return maneuvers;
        ...
        public <T extends CalculusFieldElement<T>> List<FieldRelativeManeuver<T>> computeTeardropManeuvers(
                        final List<TimeStampedFieldPVCoordinates<T>> waypoints,
                        final FieldRelativeProvider<T> cwProvider) {
            ...
            for (int i = 1; i < waypoints.size(); i++) {
                ...
                final TimeStampedFieldPVCoordinates<T> pvtBeforeMan =
                                new TimeStampedFieldPVCoordinates<>(maneuverWaypoint.getDate(),
                                                                    new FieldPVCoordinates<>(
                                                                                    maneuverWaypoint.getPosition(),
                                                                                    new FieldVector3D<>(
                                                                                                    maneuverVelocity.getX()
                                                                                                                    .negate(),
                                                                                                    maneuverVelocity.getY(),
                                                                                                    maneuverVelocity.getZ())));
                final FieldVector3D<T> deltaV = maneuverVelocity.subtract(pvtBeforeMan.getVelocity());
                final FieldRelativeManeuver<T> maneuver =
                                new FieldClohessyWiltshireManeuver<>(maneuverDate, deltaV,
                                                                     (FieldClohessyWiltshireProvider<T>) cwProvider);
                maneuvers.add(maneuver);
            }
            return maneuvers;
    ...
    YA {
        ...
        public List<TimeStampedPVCoordinates> computeTeardropWaypoints(final AbsoluteDate injectionDate,
            ...
            if (targetOrbit.getE() >= 1e-4) {
                ...
                final List<TimeStampedPVCoordinates> waypointsQSW =
                                new TeardropCircularWaypointCalculator(targetOrbit.getKeplerianMeanMotion(),
                                                                       turnAroundDistance, maneuverDistance,
                                                                       numberOfTeardrops).computeTearDropWaypoints(
                                                injectionDate);
                final List<TimeStampedPVCoordinates> waypointsLVLH = new ArrayList<>();
                for (TimeStampedPVCoordinates waypoint : waypointsQSW) {
                    final TimeStampedPVCoordinates waypointLVLH =
                                    LOFType.LVLH_CCSDS.transformFromLOF(LOFType.QSW, waypoint.getDate(), waypoint)
                                                      .transformPVCoordinates(waypoint);
        ...
        public <T extends CalculusFieldElement<T>> List<TimeStampedFieldPVCoordinates<T>> computeTeardropWaypoints(
            ...
            if (targetOrbit.getE().getReal() >= 1e-4) {
                ...
                final List<TimeStampedFieldPVCoordinates<T>> waypointsQSW =
                                new FieldTeardropCircularWaypointCalculator<>(targetOrbit.getKeplerianMeanMotion(),
                                                                              turnAroundDistance, maneuverDistance,
                                                                              numberOfTeardrops)
                                                .computeTearDropWaypoints(injectionDate);
                ...
                for (TimeStampedFieldPVCoordinates<T> waypoint : waypointsQSW) {
                    final TimeStampedFieldPVCoordinates<T> waypointLVLH =
                                    LOFType.LVLH_CCSDS.transformFromLOF(LOFType.QSW, waypoint.getDate(), waypoint)
                                                      .transformPVCoordinates(waypoint);
                    waypointsLVLH.add(waypointLVLH);
                }
        ...
        public List<RelativeManeuver> computeForcedManeuvers(final List<TimeStampedPVCoordinates> waypoints,
            ...
            for (int i = 0; i < waypoints.size() - 1; i++) {
                ...
                // Compute the velocity after the maneuver to arrive at the following waypoint at the correct date using RendezVous.
                final Vector3D velocityAfterManeuver =
                                YamanakaAnkersenRendezVous.computeRendezVous(currentWaypoint, nextWaypoint, lofUpdated,
                                                                             orbit, targetPropagator).getPvt1()
                                                          .getVelocity();
                ...
                final RelativeManeuver maneuver = new YamanakaAnkersenManeuver(firstImpulseTrigger, deltaV,
                                                                               (YamanakaAnkersenProvider) yaProvider);
                maneuvers.add(maneuver);

                // Create a Yamanaka-Ankersen provider to propagate to the next waypoint.
                final YamanakaAnkersenProvider yaProviderManeuver = new YamanakaAnkersenProvider(orbit,
                ...
                orbit = new KeplerianOrbit(orbit.getA(), orbit.getE(), orbit.getI(), orbit.getPeriapsisArgument(),
                                           ...
                                           orbit.getMu());
                velocityBeforeManeuver =
                                new Vector3D(Arrays.copyOfRange(yaProviderManeuver.getAdditionalData(propagated), 3,
                                                                6));
            }
            return maneuvers;
        }
        ...
        public <T extends CalculusFieldElement<T>> List<FieldRelativeManeuver<T>> computeForcedManeuvers(
            ...
            for (int i = 0; i < waypoints.size() - 1; i++) {
                ...
                // Compute the velocity after the maneuver to arrive at the following waypoint at the correct date using RendezVous.
                final FieldVector3D<T> velocityAfterManeuver =
                                (new FieldYamanakaAnkersenRendezVous<T>()).computeRendezVous(currentWaypoint,
                                                                                             nextWaypoint, lofUpdated,
                                                                                             orbit, targetPropagator)
                                                                          .getPvt1().getVelocity();
                // Compute the impulsion at the current waypoint to reach the next waypoint.
                final FieldVector3D<T> deltaV = velocityAfterManeuver.subtract(velocityBeforeManeuver);
                ...
                // Create the Yamanaka-Ankersen maneuver and add it to the list.
                final FieldRelativeManeuver<T> maneuver =
                                new FieldYamanakaAnkersenManeuver<>(firstImpulseTrigger, deltaV,
                                                                    (FieldYamanakaAnkersenProvider<T>) yaProvider);
                maneuvers.add(maneuver);

                // Create a Yamanaka-Ankersen provider to propagate to the next waypoint.
                final FieldYamanakaAnkersenProvider<T> yaProviderManeuver =
                                new FieldYamanakaAnkersenProvider<>(orbit,
                ...
                orbit = new FieldKeplerianOrbit<>(orbit.getA(), orbit.getE(), orbit.getI(),
                                                  ...
                                                  orbit.getMu());
                velocityBeforeManeuver = new FieldVector3D<>(
                                Arrays.copyOfRange(yaProviderManeuver.getAdditionalData(propagated), 3, 6));
            }
            return maneuvers;
        }
                ...
                final TimeStampedPVCoordinates pvtBeforeMan =
                                new TimeStampedPVCoordinates(maneuverWaypoint.getDate(), maneuverWaypoint.getPosition(),
                                                             new Vector3D(maneuverVelocity.getX(),
                                                                          ...
                                                                          -maneuverVelocity.getZ()));
                final Vector3D deltaV = maneuverVelocity.subtract(pvtBeforeMan.getVelocity());
                final RelativeManeuver maneuver = new YamanakaAnkersenManeuver(maneuverDate, deltaV,
                                                                               (YamanakaAnkersenProvider) yaProvider);
                maneuvers.add(maneuver);
            }
            return maneuvers;
        ...
        public <T extends CalculusFieldElement<T>> List<FieldRelativeManeuver<T>> computeTeardropManeuvers(
                        final List<TimeStampedFieldPVCoordinates<T>> waypoints,
                        final FieldRelativeProvider<T> yaProvider) {
            ...
            for (int i = 1; i < waypoints.size(); i++) {
                ...
                final FieldVector3D<T> deltaV = maneuverVelocity.subtract(pvtBeforeMan.getVelocity());
                final FieldRelativeManeuver<T> maneuver =
                                new FieldYamanakaAnkersenManeuver<>(maneuverDate, deltaV,
                                                                    (FieldYamanakaAnkersenProvider<T>) yaProvider);
                maneuvers.add(maneuver);
            }
            return maneuvers;
