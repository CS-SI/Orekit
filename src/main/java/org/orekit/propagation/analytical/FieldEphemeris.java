...
public class FieldEphemeris<T extends RealFieldElement<T>>
    extends
     ...
     * <p>
     * This constructor allows extrapolating outside of the states time span by
     * up to the 1ms {@link #DEFAULT_EXTRAPOLATION_THRESHOLD_SEC default
     ...
     * @exception MathIllegalArgumentException if the number of states is
     *            smaller than the number of points to use in interpolation
     ...
     * @param states tabulates states
     * @param interpolationPoints number of points to use in interpolation
     * @param extrapolationThreshold the largest time difference in seconds
     *        between the start or stop boundary of the ephemeris bounds to be
     *        doing extrapolation
     * @exception MathIllegalArgumentException if the number of states is
     *            smaller than the number of points to use in interpolation
    ...
    public FieldEphemeris(final Field<T> field, final List<SpacecraftState> states,
        ...
        maxDate =
            new FieldAbsoluteDate<>(field,
                                    states.get(states.size() - 1).getDate());
        frame = s0.getFrame();
        ...
        pvProvider =
            new LocalPVProvider<>(states, interpolationPoints,
                                  extrapolationThreshold);

        // user needs to explicitly set attitude provider if they want to use
        // one
        setAttitudeProvider(null);

        // set up cache
        cache =
            new ImmutableTimeStampedCache<SpacecraftState>(interpolationPoints,
                                                           states);
    ...
    public FieldSpacecraftState<T>
        ...
        final List<FieldSpacecraftState<T>> neighbors =
            cache.getNeighbors(central.toAbsoluteDate())
                .map(state -> new FieldSpacecraftState<>(date.getField(),
                                                         state))
        ...
        if (attitudeProvider == null) {
            ...
            final FieldAttitude<T> calculatedAttitude =
                attitudeProvider.getAttitude(pvProvider, date,
                                             evaluatedState.getFrame());

            // Verify if orbit is defined
            if (evaluatedState.isOrbitDefined()) {
                return new FieldSpacecraftState<T>(evaluatedState.getOrbit(),
                                                   calculatedAttitude,
                                                   evaluatedState.getMass(),
                                                   evaluatedState
                                                       .getAdditionalStates());
            } else {
                return new FieldSpacecraftState<T>(evaluatedState.getAbsPVA(),
                                                   calculatedAttitude,
                                                   evaluatedState.getMass(),
                                                   evaluatedState
                                                       .getAdditionalStates());
    ...
    /** {@inheritDoc} */
    @Override
    public TimeStampedFieldPVCoordinates<T>
        getPVCoordinates(final FieldAbsoluteDate<T> date, final Frame f) {
    ...
    public boolean isAdditionalStateManaged(final String name) {
        ...
        // the additional state may be managed by a specific provider in the
        // base class
    ...
    public String[] getManagedAdditionalStates() {
        ...
        final String[] managed =
            new String[upperManaged.length + additional.length];
        System.arraycopy(upperManaged, 0, managed, 0, upperManaged.length);
        System.arraycopy(additional, 0, managed, upperManaged.length,
                         additional.length);
        return managed;
    }
    ...
    private static class LocalPVProvider<T extends RealFieldElement<T>>
        ...
        public TimeStampedFieldPVCoordinates<T>
            ...
            if (FastMath.abs(dt).getReal() > closeEnoughTimeInSec) {
                ...
                // used in case of attitude transition, the attitude computed is
                // not at the
                // current date.
                final FieldEphemeris<T> ephemeris =
                    new FieldEphemeris<>(date.getField(), states,
                                         interpolationPoints,
