 ...
 * <p>The Eckstein-Hechler model is suited for near circular FieldOrbit<T>s
 ...
 * FieldOrbit<T>s, and it was fixed as of version 7.0 of Orekit, with a visible side effect.
 * The problems is that if the circular parameters produced by the Eckstein-Hechler
 * model are used to build an FieldOrbit<T> considered to be osculating, the velocity deduced
 * from this FieldOrbit<T> was <em>inconsistent with the position evolution</em>! The reason is
 ...
 * considered important, the propagator now generates {@link CartesianFieldOrbit<T> Cartesian
 * FieldOrbit<T>s} which are built in a special way to ensure consistency throughout propagation.
 * A side effect is that if circular parameters are rebuilt by user from these propagated
 * Cartesian FieldOrbit<T>, the circular parameters will generally <em>not</em> match the initial
 * FieldOrbit<T> (differences in semi-major axis can exceed 120 m). The position however <em>will</em>
 ...
 * of the initialization has been assessed and is good, as it allows the subsequent FieldOrbit<T>
 * to remain close to a numerical reference FieldOrbit<T>.
 * </p>
 * <p>
 ...
 * sample instead of just a single initial FieldOrbit<T>.
 * </p>
 * @see FieldOrbit<T>
