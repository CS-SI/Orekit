...
public class FieldSmallManeuverAnalyticalModel<T extends RealFieldElement<T>>
	 ...
	 */
	public FieldSmallManeuverAnalyticalModel(Field<T> field, final FieldSpacecraftState<T> state0,
			final FieldVector3D<T> dV, final T isp) {
		this(field, state0, state0.getFrame(), state0.getAttitude().getRotation().applyInverseTo(dV), isp);
	}
	 ...
	 * @param isp    engine specific impulse (s)
	 */
	public FieldSmallManeuverAnalyticalModel(Field<T> field, final FieldSpacecraftState<T> state0, final Frame frame,
			final FieldVector3D<T> dV, final T isp) {
		final T zero = field.getZero();
		this.state0 = state0;
		this.massRatio = FastMath.exp(zero.subtract(dV.getNorm()).divide(isp.multiply(Constants.G0_STANDARD_GRAVITY)));

		// use equinoctial orbit type if possible, Keplerian if nearly hyperbolic orbits
		type = (state0.getE().getReal() < 0.9) ? OrbitType.EQUINOCTIAL : OrbitType.KEPLERIAN;
		...
		ksi = zero.add(-1.5).multiply(FastMath.sqrt(zero.add(mu).divide(a)).divide(a.multiply(a)));
	...
	public FieldSpacecraftState<T> apply(final FieldSpacecraftState<T> state1) {
		...
		return new FieldSpacecraftState<>(state1.getOrbit().getType().convertType(updateOrbit(state1.getOrbit())),
	 ...
	 * 
	 * @param orbit1 original orbit at t₁, without maneuver
	 * @return orbit at t₁, always taking the maneuver into account, always in the
	 *         internal type
	 */
	private FieldOrbit<T> updateOrbit(final FieldOrbit<T> orbit1) {
		...
		for (int i = 0; i < delta.length; ++i) {
			delta[i] = j0[i][0].multiply(x).add(j0[i][1].multiply(y).add(j0[i][2].multiply(z)));
		}
		...
		return type.mapArrayToOrbit(parameters, null, PositionAngle.MEAN, orbit1.getDate(), orbit1.getMu(),
				orbit1.getFrame());
	 ...
	 * Orbit.getJacobianWrtCartesian} method. Columns (0, 1, 2) correspond to the
	 * velocity increment coordinates (ΔV<sub>x</sub>, ΔV<sub>y</sub>,
	 * ΔV<sub>z</sub>) in the inertial frame returned by
	 * {@link #getInertialFrame()}, and column 3 corresponds to the maneuver date
	 * t₀.
	 * </p>
	 * 
	 * @param orbit1        original orbit at t₁, without maneuver
	 ...
	 *                      Jacobian, if matrix is larger than 6x4, only the 6x4
	 *                      upper left corner will be modified
	 * @see #apply(Orbit)
	 */
	public void getJacobian(final FieldOrbit<T> orbit1, final PositionAngle positionAngle, final T[][] jacobian) {
		...
		// derivatives of Keplerian/equinoctial elements with respect to velocity
		// increment
		final T x = inertialDV.getX();
		final T y = inertialDV.getY();
		final T z = inertialDV.getZ();
		...
		for (int j = 0; j < 3; ++j) {
			jacobian[5][j] = jacobian[5][j].add(ksi.multiply(dt).multiply(j0[0][j]));
		}

		// derivatives of Keplerian/equinoctial elements with respect to date
		evaluateJ0Dot();
		for (int i = 0; i < 6; ++i) {
			jacobian[i][3] = j0Dot[i][0].multiply(x).add(j0Dot[i][1].multiply(y)).add(j0Dot[i][2].multiply(z));
		}
		final T da = j0[0][0].multiply(x).add(j0[0][1].multiply(y)).add(j0[0][2].multiply(z));
		jacobian[5][3] = jacobian[5][3].add(ksi.multiply(jacobian[0][3].multiply(dt).subtract(da)));
		...
		if (orbit1.getType() != type || positionAngle != PositionAngle.MEAN) {
			...
			for (int i = 0; i < 6; ++i) {
				for (int j = 0; j < 4; ++j) {
					pvJacobian[i][j] = j2[i][0].multiply(jacobian[0][j]).add(j2[i][1].multiply(jacobian[1][j]))
							.add(j2[i][2].multiply(jacobian[2][j])).add(j2[i][3].multiply(jacobian[3][j]))
							.add(j2[i][4].multiply(jacobian[4][j])).add(j2[i][5].multiply(jacobian[5][j]));
				}
			}
			...
			for (int j = 0; j < 4; ++j) {
				for (int i = 0; i < 6; ++i) {
					jacobian[i][j] = j3[i][0].multiply(pvJacobian[0][j]).add(j3[i][1].multiply(pvJacobian[1][j]))
							.add(j3[i][2].multiply(pvJacobian[2][j])).add(j3[i][3].multiply(pvJacobian[3][j]))
							.add(j3[i][4].multiply(pvJacobian[4][j])).add(j3[i][5].multiply(pvJacobian[5][j]));
				}
			}
	...
	private void evaluateJ0Dot() {
		...
		if (j0Dot == null) {
			...
			final T dt = zero.add(1.0e-5).divide(state0.getOrbit().getKeplerianMeanMotion());
			...
			orbit.shiftedBy(dt.multiply(-1)).getJacobianWrtCartesian(PositionAngle.MEAN, j0m1);
			final T[][] j0p1 = MathArrays.buildArray(field, 6, 6);
			orbit.shiftedBy(dt.multiply(+1)).getJacobianWrtCartesian(PositionAngle.MEAN, j0p1);
			...
			for (int i = 0; i < j0Dot.length; ++i) {
				...
				for (int j = 0; j < 3; ++j) {
					j0DotRow[j] = (p1Row[j + 3].subtract(m1Row[j + 3])).divide(dt.multiply(2));
				}
			}
