...
public class TLE implements TimeStamped, Serializable {
	 ...
	 * 
	 * <p>
	 * The right ascension of ascending node Ω and the argument of perigee ω can be
	 * negative and are therefore normalized into the [0, 2π] interval. After that,
	 * a range check is performed on some of the orbital elements:
	 * 
	 * <pre>
	 *     meanMotion &gt;= 0
	 *     0 &lt;= i &lt;= π
    ...
    public TLE(final int satelliteNumber, final char classification,
        ...
        MathUtils.checkRangeInclusive(meanMotion, 0.0, Double.POSITIVE_INFINITY);
        ...
        MathUtils.checkRangeInclusive(i, 0, FastMath.PI);
        this.inclination = i;

        // Checking RAAN range
        MathUtils.checkRangeInclusive(raan, -2 * FastMath.PI, 2 * FastMath.PI);
        // Normalizing RAAN in [0,2pi] interval
        this.raan = MathUtils.normalizeAngle(raan, FastMath.PI);

        // Checking eccentricity range
        MathUtils.checkRangeInclusive(e, 0.0, 1.0);
        this.eccentricity = e;

        // Checking PA range
        MathUtils.checkRangeInclusive(pa, -2 * FastMath.PI, 2 * FastMath.PI);
        // Normalizing PA in [0,2pi] interval
        this.pa = MathUtils.normalizeAngle(pa, FastMath.PI);

        // Checking mean anomaly range
        MathUtils.checkRangeInclusive(meanAnomaly, 0.0, 2 * FastMath.PI);
