...
public class FieldGLONASSDate<T extends RealFieldElement<T>>
    implements
    Serializable,
    FieldTimeStamped<T> {
     ...
     */
    @DefaultDataContext
    public FieldGLONASSDate(final Field<T> field, final int na, final int n4,
                            final T secInNa) {
        this(field, na, n4, secInNa,
     ...
     */
    public FieldGLONASSDate(final Field<T> field, final int na, final int n4,
                            final T secInNa, final TimeScale glonass) {
        this.field = field;
        this.na = na;
        this.n4 = n4;
     ...
     */
    @DefaultDataContext
    public FieldGLONASSDate(final Field<T> field,
    ...
    public FieldGLONASSDate(final Field<T> field,
        ...
        final T zero = field.getZero();
        final T duration =
            zero.add(date.durationFrom(new FieldAbsoluteDate<T>(field, start, 1,
                                                                1, glonass))
                .getReal());
    ...
    /**
     * Compute the Greenwich Mean Sidereal Time using the current Julian date
     * JD0.
     *
     * @return the Greenwich Mean Sidereal Time (rad)
     */
    private T computeGMST() {
        ...
        final T era =
            zero.add(2. * GLONASSOrbitalElements.GLONASS_PI * 0.7790572732640)
                .add((jd0.subtract(ref)).multiply(1.00273781191135448));
        // Time from Epoch 2000 (1st January, 00:00 UTC) till current Epoch in
        // Julian
        // centuries
        ...
        // GMST computation
        final T gTime =
            era.add(7.03270726e-8).add(time.multiply(2.23603658710194e-2))
                .add(time2.add(6.7465784654e-6))
                .subtract(time3.multiply(2.1332e-12))
                .subtract(time4.multiply(1.452308e-10))
                .subtract(time5.multiply(1.784e-13));
        return gTime;
    }
    ...
    private FieldAbsoluteDate<T> computeDate(final TimeScale glonass) {
        ...
        return new FieldAbsoluteDate<T>(field,
                                        new DateComponents(year, month, day),
                                        new TimeComponents(secInNa.getReal()),
                                        glonass);
    }
    ...
    private class DataTransferObject
         ...
         * @param secInNa the number of seconds since na start
         */
        DataTransferObject(final Field<T> field, final int na, final int n4,
                           final T secInNa) {
            this.field = field;
            this.na = na;
            this.n4 = n4;
