 ...
 */
public class FieldGLONASSAnalyticalPropagator<T extends RealFieldElement<T>> extends FieldAbstractAnalyticalPropagator<T> {
     ...
     * <p>It implements the classical FieldBuilder pattern.</p>
     *
     */
    public class FieldBuilder {
        ...
        private final FieldGLONASSOrbitalElements<T> orbit;
         ...
         * {@code FieldBuilder(final GLONASSOrbitalElements gpsOrbElt, final DataContext dataContext)}</p>
         *
         ...
         */
        @DefaultDataContext
        public FieldBuilder(final Field<T> field, final FieldGLONASSOrbitalElements<T> glonassOrbElt) {
         ...
         * @return the updated FieldBuilder
         */
        public FieldBuilder attitudeProvider(final AttitudeProvider userProvider) {
         ...
         * @return the updated FieldBuilder
         */
        public FieldBuilder mass(final double userMass) {
            this.mass = userMass;
            return this;
        }
         ...
         * @return the updated FieldBuilder
         */
        public FieldBuilder eci(final Frame inertial) {
            this.eci = inertial;
            return this;
        }
         ...
         * @return the updated FieldBuilder
         */
        public FieldBuilder ecef(final Frame bodyFixed) {
            this.ecef = bodyFixed;
            return this;
        }
         ...
         * @return the updated FieldBuilder.
         */
        public FieldBuilder dataContext(final DataContext context) {
         ...
         */
        public FieldGLONASSAnalyticalPropagator<T> build() {
            return new FieldGLONASSAnalyticalPropagator<>(field, this);
        }
    ...
    /**
     * Private constructor.
     * @param FieldBuilder the FieldBuilder
     */
    private FieldGLONASSAnalyticalPropagator(final Field<T> field, final FieldBuilder FieldBuilder) {
        super(field, FieldBuilder.attitudeProvider);
        this.dataContext = FieldBuilder.dataContext;
        // Stores the GLONASS orbital elements
        this.glonassOrbit = FieldBuilder.orbit;
        // Sets the start date as the date of the orbital elements
        setStartDate(glonassOrbit.getDate());
        // Sets the mass
        this.mass = FieldBuilder.mass;
        // Sets the Earth Centered Inertial frame
        this.eci  = FieldBuilder.eci;
        // Sets the Earth Centered Earth Fixed frame
        this.ecef = FieldBuilder.ecef;
		this.field = null;
    }
    ...
    public FieldPVCoordinates<T> propagateInEcef(final FieldAbsoluteDate<T> date) {
        ...
        final FieldUnivariateDerivative2<T> coef    = tanPAo2.multiply(FastMath.sqrt(e.negate().add(1.0).divide(e.add(1.0))));
        ...
        return new FieldPVCoordinates<T>(new FieldVector3D<T>(positionwithDerivatives.getX().getValue(),
     ...
     */
    private FieldUnivariateDerivative2<T> getEccentricAnomaly(final FieldUnivariateDerivative2<T> mk, final FieldUnivariateDerivative2<T> e) {
        ...
        final boolean noCancellationRisk = (e1.getReal() + ek.getValue().getReal() * ek.getValue().getReal() / 6.0) >= 0.1;
     ...
     * @return E - e sin(E)
     */
    private FieldUnivariateDerivative2<T> eMeSinE(final FieldUnivariateDerivative2<T> E, final FieldUnivariateDerivative2<T> ecc) {
    ...
    */
    private FieldUnivariateDerivative2<T> getTrueAnomaly(final FieldUnivariateDerivative2<T> ek, final FieldUnivariateDerivative2<T> ecc) {
    ...
    private FieldUnivariateDerivative2<T> getdTpr(final FieldAbsoluteDate<T> date) {
        ...
        final FieldGLONASSDate<T> tEnd = new FieldGLONASSDate<T>(date, glonass);
        final FieldGLONASSDate<T> tSta = new FieldGLONASSDate<T>(glonassOrbit.getDate(), glonass);
        ...
        final FieldUnivariateDerivative2<T> ti = new FieldUnivariateDerivative2<T>(zero.add(tEnd.getSecInDay()), zero.add(1.0), zero.add(1.0));
    ...
    private FieldUnivariateDerivative2<T> computeSma(final FieldUnivariateDerivative2<T> tDR,
        ...
        if (Double.isNaN(tDR.getValue().getReal()) || Double.isNaN(i.getValue().getReal()) || Double.isNaN(e.getValue().getReal())) {
    ...
    private FieldUnivariateDerivative2<T> computePA(final FieldUnivariateDerivative2<T> dTpr,
        ...
        final FieldUnivariateDerivative2<T> precession = aeop2.multiply(n).multiply(cos2I.multiply(5.0).negate().add(1.0)).multiply(0.75 * GLONASS_J20);
     ...
     */
    private FieldUnivariateDerivative2<T>[] getParameterDifferentials(final FieldUnivariateDerivative2<T> a, final FieldUnivariateDerivative2<T> i,
                                                              final FieldUnivariateDerivative2<T> h, final FieldUnivariateDerivative2<T> l,
        ...
        final FieldUnivariateDerivative2<T> dakT2 = b.multiply(sinI2).multiply(hSinLk.multiply(0.5).subtract(lCosLk.multiply(0.5)).
                                                                     add(cos2Lk).add(lCos3Lk.multiply(3.5)).add(hSin3Lk.multiply(3.5)));
        final FieldUnivariateDerivative2<T> dak = dakT1.add(dakT2);

        // δh
        final FieldUnivariateDerivative2<T> dhkT1 = b.multiply(om3o2xSinI2).multiply(sinLk.add(lSin2Lk.multiply(1.5)).subtract(hCos2Lk.multiply(1.5)));
        final FieldUnivariateDerivative2<T> dhkT2 = b.multiply(sinI2).multiply(0.25).multiply(sinLk.subtract(sin3Lk.multiply(SEVEN_THIRD)).add(lSin2Lk.multiply(5.0)).
        ...
        // δl
        final FieldUnivariateDerivative2<T> dlkT1 = b.multiply(om3o2xSinI2).multiply(cosLk.add(lCos2Lk.multiply(1.5)).add(hSin2Lk.multiply(1.5)));
        final FieldUnivariateDerivative2<T> dlkT2 = b.multiply(sinI2).multiply(0.25).multiply(cosLk.negate().subtract(cos3Lk.multiply(SEVEN_THIRD)).subtract(hSin2Lk.multiply(5.0)).
        ...
        final FieldUnivariateDerivative2<T> dokT2 = lSinLk.multiply(3.5).subtract(hCosLk.multiply(2.5)).subtract(sin2Lk.multiply(0.5)).
        ...
        // δi
        final FieldUnivariateDerivative2<T> dik = b.multiply(sinI).multiply(cosI).multiply(0.5).
                        multiply(lCosLk.negate().add(hSinLk).add(cos2Lk).add(lCos3Lk.multiply(SEVEN_THIRD)).add(hSin3Lk.multiply(SEVEN_THIRD)));

        // δL
        final FieldUnivariateDerivative2<T> dLkT1 = b.multiply(2.0).multiply(om3o2xSinI2).multiply(lSinLk.multiply(1.75).subtract(hCosLk.multiply(1.75)));
        final FieldUnivariateDerivative2<T> dLkT2 = b.multiply(sinI2).multiply(3.0).multiply(hCosLk.multiply(SEVEN_24TH).negate().subtract(lSinLk.multiply(SEVEN_24TH)).
                                                                                   subtract(hCos3Lk.multiply(FN_72TH)).add(lSin3Lk.multiply(FN_72TH)).add(sin2Lk.multiply(0.25)));
        final FieldUnivariateDerivative2<T> dLkT3 = b.multiply(cosI2).multiply(lSinLk.multiply(3.5).subtract(hCosLk.multiply(2.5)).subtract(sin2Lk.multiply(0.5)).
                                                                     subtract(lSin3Lk.multiply(SEVEN_SIXTH)).add(hCos3Lk.multiply(SEVEN_SIXTH)));
    ...
    protected FieldOrbit<T> propagateOrbit(final FieldAbsoluteDate<T> date) {
        ...
        final T zero = date.getField().getZero();
        return new FieldCartesianOrbit<T>(pvaInECI, eci, date, zero.add(FieldGLONASSOrbitalElements.GLONASS_MU));
    }
