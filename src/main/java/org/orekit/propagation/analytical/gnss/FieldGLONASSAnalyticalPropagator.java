...
extends FieldAbstractAnalyticalPropagator<T> {
	 ...
	 * Default constructor.
	 * 
	 * <p>The Field GLONASS orbital elements is the only requested parameter to build a FieldGLONASSPropagator.</p>
	 ...
	 * {@code FieldGLONASSAnalyticalPropagator(final Field<T> field, final FieldGLONASSOrbitalElements<T> glonassOrbElt, final DataContext dataContext)}</p>
	 * 
	 * @param field
	 * @param glonassOrbElt the Field GLONASS orbital elements to be used by the Field GLONASS propagator.
	 */
	@DefaultDataContext
	public FieldGLONASSAnalyticalPropagator(final Field<T> field, final FieldGLONASSOrbitalElements<T> glonassOrbElt) {
	 ...
	 * {@code FieldGLONASSAnalyticalPropagator(final Field<T> field, final FieldGLONASSOrbitalElements<T> glonassOrbElt,
			final DataContext dataContext, final AttitudeProvider attitudeProvider, T mass, final Frame eci, final Frame ecef)}</p>
	 *
	 * @param field
	 * @param glonassOrbElt the Field GLONASS orbital elements to be used by the Field GLONASS propagator.
	 * @param dataContext the data
	 */
	public FieldGLONASSAnalyticalPropagator(final Field<T> field, final FieldGLONASSOrbitalElements<T> glonassOrbElt, final DataContext dataContext) {
		this(field, glonassOrbElt, dataContext, Propagator.getDefaultLaw(dataContext.getFrames()), field.getZero().add(DEFAULT_MASS),dataContext.getFrames().getEME2000(),
	...
	/**
	 * Gets the FieldPVCoordinates of the GLONASS SV in {@link #getECEF() ECEF frame}.
	 *
	 * <p>
	 ...
	 * @return the GLONASS SV FieldPVCoordinates in {@link #getECEF() ECEF frame}
	 */
	public FieldPVCoordinates<T> propagateInEcef(final FieldAbsoluteDate<T> date) {
		...
		final FieldUnivariateDerivative2<T> w = FastMath.floor(dTpr.divide(glonassOrbit.getDeltaT().add(GLONASS_MEAN_DRACONIAN_PERIOD)));

		// Current inclination
		final FieldUnivariateDerivative2<T> i = zero.add(zero.add(GLONASS_MEAN_INCLINATION / 180 * GLONASSOrbitalElements.GLONASS_PI).add(glonassOrbit.getDeltaI()));

		// Eccentricity
		final FieldUnivariateDerivative2<T> e = zero.add(glonassOrbit.getE());

		// Mean draconique period in orbite w+1 and mean motion
		final FieldUnivariateDerivative2<T> tDR = w.multiply(2.0).add(1.0).multiply(glonassOrbit.getDeltaTDot())
													.add(glonassOrbit.getDeltaT())
													.add(GLONASS_MEAN_DRACONIAN_PERIOD);
		final FieldUnivariateDerivative2<T> n = tDR.divide(2.0 * GLONASSOrbitalElements.GLONASS_PI).reciprocal();

		// Semi-major axis : computed by successive approximation
		final FieldUnivariateDerivative2<T> sma = computeSma(tDR, i, e);

		// (ae / p)^2 term
		final FieldUnivariateDerivative2<T> p = sma.multiply(e.multiply(e).negate().add(1.0));
		final FieldUnivariateDerivative2<T> aeop = p.divide(GLONASS_EARTH_EQUATORIAL_RADIUS).reciprocal();
		final FieldUnivariateDerivative2<T> aeop2 = aeop.multiply(aeop);

		// Current longitude of the ascending node
		final FieldUnivariateDerivative2<T> lambda = computeLambda(dTpr, n, aeop2, i);

		// Current argument of perigee
		final FieldUnivariateDerivative2<T> pa = computePA(dTpr, n, aeop2, i);

		// Mean longitude at the instant the spacecraft passes the current ascending
		// node
		final FieldUnivariateDerivative2<T> tanPAo2 = FastMath.tan(pa.divide(2.0));
		final FieldUnivariateDerivative2<T> coef = tanPAo2
				.multiply(FastMath.sqrt(e.negate().add(1.0).divide(e.add(1.0))));
		final FieldUnivariateDerivative2<T> e0 = FastMath.atan(coef).multiply(2.0).negate();
		final FieldUnivariateDerivative2<T> m1 = pa.add(e0).subtract(FastMath.sin(e0).multiply(e));

		// Current mean longitude
		final FieldUnivariateDerivative2<T> correction = dTpr
														.subtract(w.multiply(zero.add(GLONASS_MEAN_DRACONIAN_PERIOD).add(glonassOrbit.getDeltaT())))
		...
		final FieldSinCos<FieldUnivariateDerivative2<T>> scPa = FastMath.sinCos(pa);
		final FieldUnivariateDerivative2<T> h = e.multiply(scPa.sin());
		final FieldUnivariateDerivative2<T> l = e.multiply(scPa.cos());
		// δa1
		final FieldUnivariateDerivative2<T>[] d1 = getParameterDifferentials(sma, i, h, l, m1);
		// δa2
		final FieldUnivariateDerivative2<T>[] d2 = getParameterDifferentials(sma, i, h, l, m);
		// Apply corrections
		final FieldUnivariateDerivative2<T> smaCorr = sma.add(d2[0]).subtract(d1[0]);
		final FieldUnivariateDerivative2<T> hCorr = h.add(d2[1]).subtract(d1[1]);
		final FieldUnivariateDerivative2<T> lCorr = l.add(d2[2]).subtract(d1[2]);
		final FieldUnivariateDerivative2<T> lambdaCorr = lambda.add(d2[3]).subtract(d1[3]);
		final FieldUnivariateDerivative2<T> iCorr = i.add(d2[4]).subtract(d1[4]);
		final FieldUnivariateDerivative2<T> mCorr = m.add(d2[5]).subtract(d1[5]);
		final FieldUnivariateDerivative2<T> eCorr = FastMath.sqrt(hCorr.multiply(hCorr).add(lCorr.multiply(lCorr)));
		...
		if (eCorr.getValue().getReal() == 0.) {
			...
			} else if (lCorr.getValue().getReal() == -eCorr.getValue().getReal()) {
				paCorr = zero.add(-0.5 * FieldGLONASSOrbitalElements.GLONASS_PI);
			} else {
		...
		// Corrected Radius
		final FieldUnivariateDerivative2<T> pCorr = smaCorr.multiply(eCorr.multiply(eCorr).negate().add(1.0));
		final FieldUnivariateDerivative2<T> rk = pCorr.divide(eCorr.multiply(FastMath.cos(vk)).add(1.0));

		// Positions in orbital plane
		final FieldSinCos<FieldUnivariateDerivative2<T>> scPhik = FastMath.sinCos(phik);
		...
		final FieldSinCos<FieldUnivariateDerivative2<T>> scL = FastMath.sinCos(lambdaCorr);
		final FieldSinCos<FieldUnivariateDerivative2<T>> scI = FastMath.sinCos(iCorr);
	...
	private FieldUnivariateDerivative2<T> getEccentricAnomaly(final FieldUnivariateDerivative2<T> mk,
		...
		if (FastMath.abs(reducedM.getValue()).getReal() < 1.0 / 6.0) {
			...
			if (reducedM.getValue().getReal() < 0) {
				...
				ek = reducedM
						.add(w.multiply(-A).divide(w.subtract(B)).subtract(FastMath.PI).subtract(reducedM).multiply(e));
			} else {
				final FieldUnivariateDerivative2<T> minusW = reducedM.subtract(FastMath.PI);
				ek = reducedM
						.add(minusW.multiply(A).divide(minusW.add(B)).add(FastMath.PI).subtract(reducedM).multiply(e));
			}
		}
	...
	private FieldUnivariateDerivative2<T> eMeSinE(final FieldUnivariateDerivative2<T> E,
		...
		FieldUnivariateDerivative2<T> x = E.sin().multiply(ecc.negate().add(1.0));
	...
	private FieldUnivariateDerivative2<T> getTrueAnomaly(final FieldUnivariateDerivative2<T> ek,
			final FieldUnivariateDerivative2<T> ecc) {
		final FieldUnivariateDerivative2<T> svk = ek.sin().multiply(FastMath.sqrt(ecc.multiply(ecc).negate().add(1.0)));
	 ...
	 */
	private FieldUnivariateDerivative2<T> getdTpr(final FieldAbsoluteDate<T> date) {
		final TimeScale glonass = dataContext.getTimeScales().getGLONASS();
		final FieldGLONASSDate<T> tEnd = new FieldGLONASSDate<T>(date.getField(), date, glonass);
		final FieldGLONASSDate<T> tSta = new FieldGLONASSDate<T>(date.getField(), glonassOrbit.getDate(), glonass);
		...
		final FieldUnivariateDerivative2<T> ti = new FieldUnivariateDerivative2<T>(zero.add(tEnd.getSecInDay()),
				zero.add(1.0), zero);
	 ...
	 */
	private FieldUnivariateDerivative2<T> computeSma(final FieldUnivariateDerivative2<T> tDR,
			final FieldUnivariateDerivative2<T> i, final FieldUnivariateDerivative2<T> e) {

		// Zero
		final FieldUnivariateDerivative2<T> zero = tDR.getField().getZero();

		// If one of the input parameter is equal to Double.NaN, an infinite loop can
		// occur.
		// In that case, we do not compute the value of the semi major axis.
		// We decided to return a Double.NaN value instead.
		if (Double.isNaN(tDR.getValue().getReal()) || Double.isNaN(i.getValue().getReal())
		...
		final FieldUnivariateDerivative2<T> ome2 = e.multiply(e).negate().add(1.0);
		final FieldUnivariateDerivative2<T> ome2Pow3o2 = FastMath.sqrt(ome2).multiply(ome2);
		...
		final FieldUnivariateDerivative2<T> opecosPA = e.multiply(cosPA).add(1.0);
		final FieldUnivariateDerivative2<T> opecosPAPow2 = opecosPA.multiply(opecosPA);
		final FieldUnivariateDerivative2<T> opecosPAPow3 = opecosPAPow2.multiply(opecosPA);

		// Initial approximation
		FieldUnivariateDerivative2<T> tOCK = tDR;

		// Successive approximations
		// The process of approximation ends when fulfilling the following condition:
		// |a(n+1) - a(n)| < 1cm
		...
		while (!isLastStep) {
			...
			// a(n+1) computation
			final FieldUnivariateDerivative2<T> tOCKo2p = tOCK.divide(2.0 * GLONASSOrbitalElements.GLONASS_PI);
			final FieldUnivariateDerivative2<T> tOCKo2pPow2 = tOCKo2p.multiply(tOCKo2p);
			...
			final FieldUnivariateDerivative2<T> aeop = p.divide(GLONASS_EARTH_EQUATORIAL_RADIUS).reciprocal();
			final FieldUnivariateDerivative2<T> aeop2 = aeop.multiply(aeop);
			final FieldUnivariateDerivative2<T> term1 = aeop2.multiply(GLONASS_J20).multiply(1.5);
			final FieldUnivariateDerivative2<T> term2 = sin2I.multiply(2.5).negate().add(2.0);
			final FieldUnivariateDerivative2<T> term3 = ome2Pow3o2.divide(opecosPAPow2);
			final FieldUnivariateDerivative2<T> term4 = opecosPAPow3.divide(ome2);
	 ...
	 * @param aeop2 square of the ratio between the radius of the ellipsoid and p,
	 *              with p = sma * (1 - ecc²)
	 * @param i     inclination (rad)
	 * @return the current longitude of the ascending node (rad)
	 */
	private FieldUnivariateDerivative2<T> computeLambda(final FieldUnivariateDerivative2<T> dTpr,
			final FieldUnivariateDerivative2<T> n, final FieldUnivariateDerivative2<T> aeop2,
			final FieldUnivariateDerivative2<T> i) {
		final FieldUnivariateDerivative2<T> cosI = FastMath.cos(i);
		final FieldUnivariateDerivative2<T> precession = aeop2.multiply(n).multiply(cosI).multiply(1.5 * GLONASS_J20);
	 ...
	 */
	private FieldUnivariateDerivative2<T> computePA(final FieldUnivariateDerivative2<T> dTpr,
			final FieldUnivariateDerivative2<T> n, final FieldUnivariateDerivative2<T> aeop2,
			final FieldUnivariateDerivative2<T> i) {
		final FieldUnivariateDerivative2<T> cosI = FastMath.cos(i);
		final FieldUnivariateDerivative2<T> cos2I = cosI.multiply(cosI);
		final FieldUnivariateDerivative2<T> precession = aeop2.multiply(n)
				.multiply(cos2I.multiply(5.0).negate().add(1.0)).multiply(0.75 * GLONASS_J20);
	 ...
	 * <p>
	 * The value of i depends of the type of longitude (i = 2 for the current mean
	 * longitude; i = 1 for the mean longitude at the instant the spacecraft passes
	 * the current ascending node)
	 * </p>
	 * 
	 ...
	 */
	private FieldUnivariateDerivative2<T>[] getParameterDifferentials(final FieldUnivariateDerivative2<T> a,
			final FieldUnivariateDerivative2<T> i, final FieldUnivariateDerivative2<T> h,
			final FieldUnivariateDerivative2<T> l, final FieldUnivariateDerivative2<T> m) {

		// B constant
		final FieldUnivariateDerivative2<T> aeoa = a.divide(GLONASS_EARTH_EQUATORIAL_RADIUS).reciprocal();
		final FieldUnivariateDerivative2<T> aeoa2 = aeoa.multiply(aeoa);
		final FieldUnivariateDerivative2<T> b = aeoa2.multiply(1.5 * GLONASS_J20);

		// Commons Parameters
		final FieldSinCos<FieldUnivariateDerivative2<T>> scI = FastMath.sinCos(i);
		final FieldSinCos<FieldUnivariateDerivative2<T>> scLk = FastMath.sinCos(m);
		final FieldSinCos<FieldUnivariateDerivative2<T>> sc2Lk = FieldSinCos.sum(scLk, scLk);
		final FieldSinCos<FieldUnivariateDerivative2<T>> sc3Lk = FieldSinCos.sum(scLk, sc2Lk);
		final FieldSinCos<FieldUnivariateDerivative2<T>> sc4Lk = FieldSinCos.sum(sc2Lk, sc2Lk);
		...
		// 1 - (3 / 2)*sin²i
		final FieldUnivariateDerivative2<T> om3o2xSinI2 = sinI2.multiply(1.5).negate().add(1.0);

		// Compute Differentials
		// δa
		final FieldUnivariateDerivative2<T> dakT1 = b.multiply(2.0).multiply(om3o2xSinI2).multiply(lCosLk.add(hSinLk));
		final FieldUnivariateDerivative2<T> dakT2 = b.multiply(sinI2).multiply(hSinLk.multiply(0.5)
				.subtract(lCosLk.multiply(0.5)).add(cos2Lk).add(lCos3Lk.multiply(3.5)).add(hSin3Lk.multiply(3.5)));
		final FieldUnivariateDerivative2<T> dak = dakT1.add(dakT2);

		// δh
		final FieldUnivariateDerivative2<T> dhkT1 = b.multiply(om3o2xSinI2)
				.multiply(sinLk.add(lSin2Lk.multiply(1.5)).subtract(hCos2Lk.multiply(1.5)));
		final FieldUnivariateDerivative2<T> dhkT2 = b.multiply(sinI2).multiply(0.25)
				.multiply(sinLk.subtract(sin3Lk.multiply(SEVEN_THIRD)).add(lSin2Lk.multiply(5.0))
						.subtract(lSin4Lk.multiply(8.5)).add(hCos4Lk.multiply(8.5)).add(hCos2Lk));
		final FieldUnivariateDerivative2<T> dhkT3 = lSin2Lk.multiply(cosI2).multiply(b).multiply(0.5).negate();
		final FieldUnivariateDerivative2<T> dhk = dhkT1.subtract(dhkT2).add(dhkT3);

		// δl
		final FieldUnivariateDerivative2<T> dlkT1 = b.multiply(om3o2xSinI2)
				.multiply(cosLk.add(lCos2Lk.multiply(1.5)).add(hSin2Lk.multiply(1.5)));
		final FieldUnivariateDerivative2<T> dlkT2 = b.multiply(sinI2).multiply(0.25)
				.multiply(cosLk.negate().subtract(cos3Lk.multiply(SEVEN_THIRD)).subtract(hSin2Lk.multiply(5.0))
						.subtract(lCos4Lk.multiply(8.5)).subtract(hSin4Lk.multiply(8.5)).add(lCos2Lk));
		final FieldUnivariateDerivative2<T> dlkT3 = hSin2Lk.multiply(cosI2).multiply(b).multiply(0.5);
		final FieldUnivariateDerivative2<T> dlk = dlkT1.subtract(dlkT2).add(dlkT3);

		// δλ
		final FieldUnivariateDerivative2<T> dokT1 = b.negate().multiply(cosI);
		final FieldUnivariateDerivative2<T> dokT2 = lSinLk.multiply(3.5).subtract(hCosLk.multiply(2.5))
				.subtract(sin2Lk.multiply(0.5)).subtract(lSin3Lk.multiply(SEVEN_SIXTH))
				.add(hCos3Lk.multiply(SEVEN_SIXTH));
		final FieldUnivariateDerivative2<T> dok = dokT1.multiply(dokT2);

		// δi
		final FieldUnivariateDerivative2<T> dik = b.multiply(sinI).multiply(cosI).multiply(0.5).multiply(lCosLk.negate()
				.add(hSinLk).add(cos2Lk).add(lCos3Lk.multiply(SEVEN_THIRD)).add(hSin3Lk.multiply(SEVEN_THIRD)));

		// δL
		final FieldUnivariateDerivative2<T> dLkT1 = b.multiply(2.0).multiply(om3o2xSinI2)
				.multiply(lSinLk.multiply(1.75).subtract(hCosLk.multiply(1.75)));
		final FieldUnivariateDerivative2<T> dLkT2 = b.multiply(sinI2).multiply(3.0)
				.multiply(hCosLk.multiply(SEVEN_24TH).negate().subtract(lSinLk.multiply(SEVEN_24TH))
						.subtract(hCos3Lk.multiply(FN_72TH)).add(lSin3Lk.multiply(FN_72TH)).add(sin2Lk.multiply(0.25)));
		final FieldUnivariateDerivative2<T> dLkT3 = b.multiply(cosI2)
				.multiply(lSinLk.multiply(3.5).subtract(hCosLk.multiply(2.5)).subtract(sin2Lk.multiply(0.5))
		...
		// Final array
		final FieldUnivariateDerivative2<T>[] differentials = MathArrays.buildArray(a.getField(), 6);
	 ...
	 */
	public T getMU() {
		return field.getZero().add(GLONASSOrbitalElements.GLONASS_MU);
	}
	...
	protected FieldOrbit<T> propagateOrbit(FieldAbsoluteDate<T> date, T[] parameters) {
		...
		return new FieldCartesianOrbit<>(pvaInECI, eci, date, date.getField().getZero().add(FieldGLONASSOrbitalElements.GLONASS_MU));
	}
