 ...
 */
public class FieldGPSAlmanac<T extends RealFieldElement<T>>
    implements
    FieldGPSOrbitalElements<T> {
     ...
     * @see #FieldGPSAlmanac(String, int, int, int, T, T, T, T, T, T, T, T, T,
     *      T, int, int, int, FieldAbsoluteDate)
     */
    @DefaultDataContext
    public FieldGPSAlmanac(final String source, final int prn, final int svn,
                           final int week, final T toa, final T sqa,
                           final T ecc, final T inc, final T om0, final T dom,
                           final T aop, final T anom, final T af0, final T af1,
                           final int health, final int ura, final int config) {
        this(source, prn, svn, week, toa, sqa, ecc, inc, om0, dom, aop, anom,
             af0, af1, health, ura, config,
    ...
    /**
     * Constructor This constructor converts a GPSAlmanac into a FieldGPSAlmanac.
     *
     * @param field
     * @param almanac a GPSAlmanac
     */
    public FieldGPSAlmanac(final Field<T> field, final GPSAlmanac almanac) {
    ...
    public T getMeanMotion() {
        ...
        return FastMath.sqrt(absA.getField().getZero().add(GPS_MU).divide(absA))
            .divide(absA);
    }
