...
public abstract class FieldAbstractGNSSPropagator<T extends RealFieldElement<T>>
     ...
     *
     * @param field
     * @param gnssOrbit the common Field GNSS orbital elements to be used by the
     *        Field Abstract GNSS propagator
    ...
    protected FieldAbstractGNSSPropagator(final Field<T> field,
    ...
    public FieldPVCoordinates<T>
        ...
        final FieldUnivariateDerivative2<T> tk =
            new FieldUnivariateDerivative2<>(getTk(date), field.getOne(),
                                             field.getZero());
        // Mean anomaly
        final FieldUnivariateDerivative2<T> mk =
            tk.multiply(gnssOrbit.getMeanMotion()).add(gnssOrbit.getM0());
        // Eccentric Anomaly
        ...
        final FieldUnivariateDerivative2<T> dphik =
            c2phi.multiply(gnssOrbit.getCuc())
                .add(s2phi.multiply(gnssOrbit.getCus()));
        // Radius Correction
        final FieldUnivariateDerivative2<T> drk =
            c2phi.multiply(gnssOrbit.getCrc())
                .add(s2phi.multiply(gnssOrbit.getCrs()));
        // Inclination Correction
        final FieldUnivariateDerivative2<T> dik =
            c2phi.multiply(gnssOrbit.getCic())
        ...
        final FieldUnivariateDerivative2<T> rk =
            ek.cos().multiply((gnssOrbit.getE()).negate()).add(1)
                .multiply(gnssOrbit.getSma()).add(drk);
        // Corrected Inclination
        final FieldUnivariateDerivative2<T> ik =
            tk.multiply(gnssOrbit.getIDot()).add(gnssOrbit.getI0()).add(dik);
        ...
        final FieldUnivariateDerivative2<T> omk =
            tk.multiply((gnssOrbit.getOmegaDot()).subtract(av)).add(gnssOrbit
                .getOmega0().subtract(gnssOrbit.getTime().multiply(av)));
        ...
        final FieldVector3D<FieldUnivariateDerivative2<T>> positionwithDerivatives =
            new FieldVector3D<>(xk.multiply(comk)
                .subtract(yk.multiply(somk).multiply(cik)),
                                xk.multiply(somk)
                                    .add(yk.multiply(comk).multiply(cik)),
                                yk.multiply(ik.sin()));
    ...
    private FieldUnivariateDerivative2<T>
        ...
        if (FastMath.abs(reducedM.getValue().getReal()) < 1.0 / 6.0) {
            if (FastMath
                .abs(reducedM.getValue().getReal()) < Precision.SAFE_MIN) {
            ...
            if (reducedM.getValue().getReal() < 0) {
                ...
                ek =
                    reducedM.add(w.multiply(-A).divide(w.subtract(B))
                        .subtract(FastMath.PI).subtract(reducedM)
                        .multiply(gnssOrbit.getE()));
            } else {
                ...
                ek =
                    reducedM.add(minusW.multiply(A).divide(minusW.add(B))
                        .add(FastMath.PI).subtract(reducedM)
                        .multiply(gnssOrbit.getE()));
            }
        }
        ...
        for (int j = 0; j < 2; ++j) {
            ...
            final FieldUnivariateDerivative2<T> fdd =
                ek.sin().multiply(gnssOrbit.getE());
            final FieldUnivariateDerivative2<T> fddd =
                ek.cos().multiply(gnssOrbit.getE());
            if (noCancellationRisk) {
                ...
                fd =
                    s.multiply(s).multiply(gnssOrbit.getE().multiply(2.0))
                        .add(e1);
            }
    ...
    private FieldUnivariateDerivative2<T>
        ...
        FieldUnivariateDerivative2<T> x =
            E.sin().multiply(gnssOrbit.getE().negate().add(1));
    ...
    private FieldUnivariateDerivative2<T>
        ...
        final FieldUnivariateDerivative2<T> svk =
            ek.sin()
                .multiply(FastMath
                    .sqrt((gnssOrbit.getE().multiply(gnssOrbit.getE())).negate()
                        .add(1.0)));
        final FieldUnivariateDerivative2<T> cvk =
            ek.cos().subtract(gnssOrbit.getE());
