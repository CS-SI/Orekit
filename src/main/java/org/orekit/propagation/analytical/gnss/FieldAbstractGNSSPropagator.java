 ...
 * {@link FieldAbstractAnalyticalPropagator} methods for specific GNSS propagators.
 * </p>
 * 
...
public abstract class FieldAbstractGNSSPropagator<T extends RealFieldElement<T>>
    ...
    public FieldPVCoordinates<T> propagateInEcef(final FieldAbsoluteDate<T> date) {
        ...
        final FieldUnivariateDerivative2<T> tk = new FieldUnivariateDerivative2<>(getTk(date), field.getOne(), field.getZero());
        // Mean anomaly
        ...
        final FieldUnivariateDerivative2<T> dphik = c2phi.multiply(gnssOrbit.getCuc()).add(s2phi.multiply(gnssOrbit.getCus()));
        // Radius Correction
        final FieldUnivariateDerivative2<T> drk = c2phi.multiply(gnssOrbit.getCrc()).add(s2phi.multiply(gnssOrbit.getCrs()));
        // Inclination Correction
        final FieldUnivariateDerivative2<T> dik = c2phi.multiply(gnssOrbit.getCic()).add(s2phi.multiply(gnssOrbit.getCis()));
        // Corrected Argument of Latitude
        final FieldUnivariateDerivative2<T> uk = phik.add(dphik);
        // Corrected Radius
        final FieldUnivariateDerivative2<T> rk = ek.cos().multiply((gnssOrbit.getE()).negate()).add(1).multiply(gnssOrbit.getSma()).add(drk);
    ...
    private FieldUnivariateDerivative2<T> getTrueAnomaly(final FieldUnivariateDerivative2<T> ek) {
        final FieldUnivariateDerivative2<T> svk = ek.sin().multiply(FastMath.sqrt((gnssOrbit.getE().multiply(gnssOrbit.getE())).negate().add(1.0) ));
    ...
    protected List<ParameterDriver> getParametersDrivers() {
        // GNSS propagation model does not have parameter drivers.
