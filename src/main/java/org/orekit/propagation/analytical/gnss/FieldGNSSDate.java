 ...
 * {@link SatelliteSystem#GALILEO Galileo}, {@link SatelliteSystem#BEIDOU
 * BeiDou} and {@link SatelliteSystem#QZSS QZSS} dates.
 * </p>
 *
 ...
 */
public class FieldGNSSDate<T extends RealFieldElement<T>>
    implements
    Serializable,
    FieldTimeStamped<T> {
     ...
     * @since 9.3.1
     */
    private static AtomicReference<DateComponents> rolloverReference =
        new AtomicReference<DateComponents>(null);

    /** Week number since the GNSS reference epoch. */
    private final int weekNumber;

    /** Number of milliseconds since week start. */
    private final T milliInWeek;
     ...
     * <p>
     * GNSS dates are provided as a week number starting at the GNSS reference
     * epoch and as a number of milliseconds since week start.
     * </p>
     * <p>
     * Many interfaces provide week number modulo the constellation week cycle.
     * In order to cope with this, when the week number is smaller than the week
     * cycle, this constructor assumes a modulo operation has been performed and
     * it will fix the week number according to the reference date set up for
     * handling rollover (see {@link #setRolloverReference(DateComponents)
     * setRolloverReference(reference)}). If the week number is equal to the
     * week cycle or larger, it will be used without any correction.
     * </p>
     * <p>
     ...
     */
    @DefaultDataContext
    public FieldGNSSDate(final int weekNumber, final T milliInWeek,
                         final SatelliteSystem system) {
        this(weekNumber, milliInWeek, system,
     ...
     * @param milliInWeek number of milliseconds since week start
     * @param system satellite system to consider
     * @param timeScales the set of time scales. Used to retrieve the
     *        appropriate time scale for the given {@code system}.
     * @since 10.1
     */
    public FieldGNSSDate(final int weekNumber, final T milliInWeek,
                         final SatelliteSystem system,
        ...
        final int day =
            (int) FastMath.floor(milliInWeek.getReal() /
                                 (Constants.JULIAN_DAY * S_TO_MS));
        final double secondsInDay =
            milliInWeek.getReal() / S_TO_MS - day * Constants.JULIAN_DAY;

        int w = weekNumber;
        DateComponents dc =
            new DateComponents(getWeekReferenceDateComponents(system),
                               weekNumber * 7 + day);
        ...
        if (weekNumber < cycleW) {
            ...
            if (reference == null) {
                ...
                final UT1Scale ut1 =
                    timeScales.getUT1(IERSConventions.IERS_2010, true);
                final List<EOPEntry> eop = ut1.getEOPHistory().getEntries();
                final int lastMJD = eop.get(eop.size() - 1).getMjd();
                reference =
                    new DateComponents(DateComponents.MODIFIED_JULIAN_EPOCH,
                                       lastMJD);
        ...
        date =
            new FieldAbsoluteDate<T>(milliInWeek.getField(), dc,
                                     new TimeComponents(secondsInDay),
                                     getTimeScale(system, timeScales));
     ...
     */
    @DefaultDataContext
    public FieldGNSSDate(final FieldAbsoluteDate<T> date,
                         final SatelliteSystem system) {
    ...
    public FieldGNSSDate(final FieldAbsoluteDate<T> date,
        ...
        final AbsoluteDate epoch =
            getWeekReferenceAbsoluteDate(system, timeScales);
        this.weekNumber =
            (int) FastMath
                .floor(date.durationFrom(epoch).divide(WEEK_S).getReal());
        final AbsoluteDate weekStart =
            new AbsoluteDate(epoch, WEEK_S * weekNumber);
        this.milliInWeek = date.durationFrom(weekStart).multiply(S_TO_MS);
        this.date = date;
     ...
     * GNSSDate(weekNumber, milliInWeek, system)} constructor and with a week
     * number between 0 and the constellation week cycle (cycleW) after this
     * method has been called will fix the week number to ensure they correspond
     * to dates between {@code reference - cycleW / 2 weeks} and
     * {@code reference + cycleW / 2 weeks}.
     * </p>
     * <p>
     * If this method is never called, a default reference date for rollover
     * will be set using the date of the last known EOP entry retrieved from
     * {@link UT1Scale#getEOPHistory() UT1} time scale.
     * </p>
     *
    ...
    /**
     * Get the reference epoch of the week number for the given satellite
     * system.
     * <p>
    ...
    private AbsoluteDate
        getWeekReferenceAbsoluteDate(final SatelliteSystem satellite,
                                     final TimeScales timeScales) {
        switch (satellite) {
            case GPS:
    ...
    private DateComponents
        getWeekReferenceDateComponents(final SatelliteSystem satellite) {
        switch (satellite) {
            case GPS:
    ...
    private class DataTransferObject
         ...
         * @param system satellite system to consider
         */
        DataTransferObject(final int weekNumber, final T milliInWeek,
                           final SatelliteSystem system) {
        ...
        private Object readResolve() {
            return new FieldGNSSDate<T>(weekNumber, milliInWeek, system, null);
        }
    ...
    private enum GNSSDateType {
        ...
        private static final Map<SatelliteSystem, Integer> CYCLE_MAP =
            new HashMap<SatelliteSystem, Integer>();
        static {
        ...
        /**
         * Get the number of week in one rollover cycle for the given satellite
         * system.
         *
         * @param satellite satellite system
         * @return the number of week in one rollover cycle for the given
         *         satellite system
         */
