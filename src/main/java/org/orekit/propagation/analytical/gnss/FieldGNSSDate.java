 ...
 */
public class FieldGNSSDate<T extends RealFieldElement<T>> implements Serializable, FieldTimeStamped<T> {
     ...
     * @since 10.1
     */
    public FieldGNSSDate(final int weekNumber,
                    final T milliInWeek,
                    final SatelliteSystem system,
                    final TimeScales timeScales) {

        final int day = (int) FastMath.floor(milliInWeek.getReal() / (Constants.JULIAN_DAY * S_TO_MS));
        final double secondsInDay = milliInWeek.getReal() / S_TO_MS - day * Constants.JULIAN_DAY;
        ...
        date = new FieldAbsoluteDate<T>(milliInWeek.getField(), dc, new TimeComponents(secondsInDay), getTimeScale(system, timeScales));
     ...
     */
    @DefaultDataContext
    public FieldGNSSDate(final FieldAbsoluteDate<T> date, final SatelliteSystem system) {
    ...
    public FieldGNSSDate(final FieldAbsoluteDate<T> date,
        ...
        this.weekNumber  = (int) FastMath.floor(date.durationFrom(epoch).divide(WEEK_S).getReal());
