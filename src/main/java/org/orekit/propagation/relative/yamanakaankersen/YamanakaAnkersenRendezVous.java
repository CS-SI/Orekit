...
public class YamanakaAnkersenRendezVous {
    ...
    public static TwoImpulseTransfer computeRendezVous(final TimeStampedPVCoordinates chaserPVTInitial,
        ...
        final YamanakaAnkersenMatrices yaMatrices =
                        YamanakaAnkersenEquations.computeMatrices(deltaT, finalTargetOrbit.getA(), e,
        ...
        final double k2 = FastMath.sqrt(finalTargetOrbit.getMu() / (p * p * p));

        final SinCos sinCosNu = FastMath.sinCos(finalTrueAnomaly);
        final SinCos sinCosNu0 = FastMath.sinCos(initialTrueAnomaly);

        final double rho = 1 + e * sinCosNu.cos();
        final double rho0 = 1 + e * sinCosNu0.cos();

        final double k2rho = k2 * rho;
        final double k2rho0 = k2 * rho0;

        final double k2esinNu = k2 * e * sinCosNu.sin();

        final double esinNu0 = e * sinCosNu0.sin();

        // Compute outPlane transfer matrix for initialTrueAnomaly
        final double[][] outPlaneTransferMatrix = {
                {rho0, 0.}, {-esinNu0, 1 / k2rho0}
        };

        // Compute outPlane inverse transfer matrix for finalTrueAnomaly
        final double[][] outPlaneInvTransferMatrix = {
                {1 / rho, 0}, {k2esinNu, k2rho}
        };

        // Compute inPlane transfer matrix for initialTrueAnomaly
        final double[][] inPlaneTransferMatrix = {
                {rho0, 0., 0., 0.}, {0., rho0, 0., 0.}, {-esinNu0, 0., 1 / k2rho0, 0.},
                {0., -esinNu0, 0., 1 / k2rho0}
        };

        // Compute inPlane inverse transfer matrix for finalTrueAnomaly
        final double[][] inPlaneInvTransferMatrix = {
                {1 / rho, 0., 0., 0.}, {0., 1 / rho, 0., 0.}, {k2esinNu, 0., k2rho, 0.},
                {0., k2esinNu, 0., k2rho}
        };

        // Compute out of plane (y_axis) initial velocity required to reach desired final position at t = Δt.
        final double vy0 = k2rho0 * (rho * chaserPVTFinalLof.getPosition().getY() -
                                     (FastMath.cos(finalTrueAnomaly - initialTrueAnomaly) +
                                      e * sinCosNu.cos()) * chaserPVTInitialLof.getPosition().getY()) /
                           FastMath.sin(finalTrueAnomaly - initialTrueAnomaly);

        // Compute out of plane (y_axis) velocity at t = Δt on the transfer orbit.
        final RealVector yCoordsPostMan =
                        MatrixUtils.createRealVector(new double[] {chaserPVTInitialLof.getPosition().getY(), vy0});
        final double vyf =
                        MatrixUtils.createRealMatrix(outPlaneInvTransferMatrix).multiply(yaMatrices.getOutPlaneMatrix())
                                   .multiply(MatrixUtils.createRealMatrix(outPlaneTransferMatrix))
        ...
        final RealMatrix transferMatrix = MatrixUtils.createRealMatrix(inPlaneInvTransferMatrix)
        ...
        final double vx0 = (d14 * zf + (d24 * d11 - d21 * d14) * x0 + (d24 * d12 - d22 * d14) * z0 - d24 * xf) /
                           (d23 * d14 - d24 * d13);
        final double vz0 = (d23 * xf - d13 * zf + (d13 * d21 - d11 * d23) * x0 + (d13 * d22 - d12 * d23) * z0) /
                           (d23 * d14 - d24 * d13);
