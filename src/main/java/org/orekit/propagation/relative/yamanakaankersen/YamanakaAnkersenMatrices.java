...
public class YamanakaAnkersenMatrices {
     ...
     */
    public YamanakaAnkersenMatrices(final double timeSinceEpoch, final double targetTheta, final RealMatrix inPlaneMatrix, final RealMatrix outPlaneMatrix) {
     ...
     */
    public double getTargetTrueAnomaly() { return targetTheta; }

    /**
     * Get the in-plane matrix at current time.
     * @return In-plane matrix at current time.
     */
    public RealMatrix getInPlaneMatrix() { return inPlaneMatrix; }

    /**
     * Get the in-plane matrix at current time.
     * @return In-plane matrix at current time.
     */
    public RealMatrix getOutPlaneMatrix() { return outPlaneMatrix; }

    /**
     * Transforms the input initial PVT expressed in the target's LVLH CCSDS LOF to the PVT at the time and true anomaly associated encoded in the Yamanaka-Ankersen matrices contained in the object.
     * First transform LOF PV coordinates to ~ coordinates system as presented in the Yamanaka Ankersen paper.
     * Compute actualized coordinates in the ~ system
     * Retransform from transformed frame to LOF to get desired PVCoordinates in LOF.
     *
     ...
     */
    public TimeStampedPVCoordinates transform(final TimeStampedPVCoordinates pvt, final double initialTrueAnomaly, final double trueAnomaly, final double eccentricity, final double sma, final double mu) {
        return new TimeStampedPVCoordinates(pvt.getDate().shiftedBy(timeSinceEpoch), transform(new PVCoordinates(pvt.getPosition(), pvt.getVelocity()), initialTrueAnomaly, trueAnomaly, eccentricity, sma, mu));
    }
    ...
    public PVCoordinates transform(final PVCoordinates pv, final double initialTrueAnomaly, final double trueAnomaly, final double eccentricity, final double sma, final double mu) {
        ...
        final RealMatrix pos_tilde0 = pos0.scalarMultiply(1 + eccentricity * FastMath.cos(initialTrueAnomaly));
        final RealMatrix vel_tilde0 = pos0.scalarMultiply(-eccentricity * FastMath.sin(initialTrueAnomaly)).add(vel0.scalarMultiply(1 / (FastMath.pow(k, 2) * (1 + eccentricity * FastMath.cos(initialTrueAnomaly)))));

        // Reform the vectors [x,z,vx,vz] and [y,vy]
        final double[] xz_coord0 = {pos_tilde0.getEntry(0, 0), pos_tilde0.getEntry(2, 0), vel_tilde0.getEntry(0, 0), vel_tilde0.getEntry(2, 0)};
        ...
        // Reshape the vectors
        final RealVector pos_tilde = MatrixUtils.createRealVector(new double[] {xz_coord.getEntry(0), y_coord.getEntry(0), xz_coord.getEntry(1) });
        final RealVector vel_tilde = MatrixUtils.createRealVector(new double[] {xz_coord.getEntry(2), y_coord.getEntry(1), xz_coord.getEntry(3) });

        // Compute final vectors in LOF
        final RealVector pos = pos_tilde.mapDivide(1 + eccentricity * FastMath.cos(trueAnomaly));
        final RealVector vel = pos_tilde.mapMultiply(FastMath.pow(k, 2) * eccentricity * FastMath.sin(trueAnomaly)).add(vel_tilde.mapMultiply(FastMath.pow(k, 2) * (1 + eccentricity * FastMath.cos(trueAnomaly))));
