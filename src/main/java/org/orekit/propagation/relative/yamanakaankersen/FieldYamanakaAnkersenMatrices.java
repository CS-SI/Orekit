...
public class FieldYamanakaAnkersenMatrices<T extends CalculusFieldElement<T>> {
     ...
     */
    public FieldYamanakaAnkersenMatrices(final T timeSinceEpoch, final T targetTheta, final FieldMatrix<T> inPlaneMatrix, final FieldMatrix<T> outPlaneMatrix) {
     ...
     */
    public T getTargetTrueAnomaly() { return targetTheta; }

    /**
     * Get the in-plane matrix at current time.
     * @return In-plane matrix at current time.
     */
    public FieldMatrix<T> getInPlaneMatrix() { return inPlaneMatrix; }

    /**
     * Get the in-plane matrix at current time.
     * @return In-plane matrix at current time.
     */
    public FieldMatrix<T> getOutPlaneMatrix() { return outPlaneMatrix; }
     ...
     */
    public TimeStampedFieldPVCoordinates<T> transform(final TimeStampedFieldPVCoordinates<T> pvt, final T initialTrueAnomaly, final T trueAnomaly, final T eccentricity, final T sma, final T mu) {
        return new TimeStampedFieldPVCoordinates<>(pvt.getDate().shiftedBy(timeSinceEpoch), transform(new FieldPVCoordinates<>(pvt.getPosition(), pvt.getVelocity()), initialTrueAnomaly, trueAnomaly, eccentricity, sma, mu));
    }
    ...
    public FieldPVCoordinates<T> transform(final FieldPVCoordinates<T> pv, final T initialTrueAnomaly, final T trueAnomaly, final T eccentricity, final T sma, final T mu) {
        ...
        final FieldMatrix<T> pos_tilde0 = pos0.scalarMultiply(eccentricity.multiply(initialTrueAnomaly.cos()).add(1));
        final FieldMatrix<T> vel_tilde0 = pos0.scalarMultiply(eccentricity.multiply(-1.).multiply(initialTrueAnomaly.sin())).add(vel0.scalarMultiply(sma.getField().getOne().divide(k.pow(2).multiply(eccentricity.multiply(initialTrueAnomaly.cos()).add(1)))));
        ...
        final FieldVector<T> pos = pos_tilde.mapDivide(eccentricity.multiply(trueAnomaly.cos()).add(1));
        final FieldVector<T> vel = pos_tilde.mapMultiply(k.pow(2).multiply(eccentricity).multiply(trueAnomaly.sin())).add(vel_tilde.mapMultiply(k.pow(2).multiply(eccentricity.multiply(trueAnomaly.cos()).add(1))));
