...
public class FieldYamanakaAnkersenRendezVous<T extends CalculusFieldElement<T>> {
     ...
     * @return rho.
     */
    public T rho(final T theta, final T targetE) { return targetE.multiply(theta.cos()).add(1.); }
     ...
     */
    public FieldTwoImpulseTransfer<T> computeRendezVous(final TimeStampedFieldPVCoordinates<T> chaserPVTInitial, final TimeStampedFieldPVCoordinates<T> chaserPVTFinal, final Frame chaserPVFrame, final FieldOrbit<T> targetOrbit, final FieldPropagator<T> propagator) {
        // create LVLH CCSDS local orbital frame of the target
        final LocalOrbitalFrame targetLof = new LocalOrbitalFrame(targetOrbit.getFrame(), LOFType.LVLH_CCSDS, new KeplerianPropagator(targetOrbit.toOrbit()), "LVLH CCSDS LOF");
        ...
        // Compute duration of the transfer from the inertial and final states dates
        final T deltaT = chaserPVTFinalLof.getDate().durationFrom(chaserPVTInitialLof.getDate());

        final FieldKeplerianOrbit<T> initialTargetOrbit = (FieldKeplerianOrbit<T>) OrbitType.KEPLERIAN.convertType(targetOrbit);
        ...
        final FieldKeplerianOrbit<T> finalTargetOrbit = (FieldKeplerianOrbit<T>) OrbitType.KEPLERIAN.convertType(propagated.getOrbit());
        ...
        outPlaneTransferMatrix[0][0] = rho(initialTrueAnomaly, E);
        outPlaneTransferMatrix[0][1] = zero;
        outPlaneTransferMatrix[1][0] = E.multiply(-1.).multiply(initialTrueAnomaly.sin());
        outPlaneTransferMatrix[1][1] = one.divide(k.pow(2).multiply(rho(initialTrueAnomaly, E)));
        ...
        outPlaneInvTransferMatrix[0][0] = one.divide(rho(finalTrueAnomaly, E));
        outPlaneInvTransferMatrix[0][1] = zero;
        outPlaneInvTransferMatrix[1][0] = E.multiply(k.pow(2)).multiply(finalTrueAnomaly.sin());
        outPlaneInvTransferMatrix[1][1] = k.pow(2).multiply(rho(finalTrueAnomaly, E));

        // Compute inPlane transfer matrix for initialTrueAnomaly
        final T[][] inPlaneTransferMatrix = MathArrays.buildArray(E.getField(), 4, 4);
        inPlaneTransferMatrix[0][0] = rho(initialTrueAnomaly, E);
        inPlaneTransferMatrix[0][1] = zero;
        inPlaneTransferMatrix[0][2] = zero;
        inPlaneTransferMatrix[0][3] = zero;
        inPlaneTransferMatrix[1][0] = zero;
        inPlaneTransferMatrix[1][1] = rho(initialTrueAnomaly, E);
        inPlaneTransferMatrix[1][2] = zero;
        inPlaneTransferMatrix[1][3] = zero;
        inPlaneTransferMatrix[2][0] = E.multiply(-1.).multiply(initialTrueAnomaly.sin());
        inPlaneTransferMatrix[2][1] = zero;
        inPlaneTransferMatrix[2][2] = one.divide(k.pow(2).multiply(rho(initialTrueAnomaly, E)));
        inPlaneTransferMatrix[2][3] = zero;
        inPlaneTransferMatrix[3][0] = zero;
        inPlaneTransferMatrix[3][1] = E.multiply(-1.).multiply(initialTrueAnomaly.sin());
        inPlaneTransferMatrix[3][2] = zero;
        inPlaneTransferMatrix[3][3] = one.divide(k.pow(2).multiply(rho(initialTrueAnomaly, E)));
        ...
        inPlaneInvTransferMatrix[0][0] = one.divide(rho(finalTrueAnomaly, E));
        ...
        inPlaneInvTransferMatrix[1][1] = one.divide(rho(finalTrueAnomaly, E));
        inPlaneInvTransferMatrix[1][2] = zero;
        inPlaneInvTransferMatrix[1][3] = zero;
        inPlaneInvTransferMatrix[2][0] = E.multiply(k.pow(2).multiply(finalTrueAnomaly.sin()));
        inPlaneInvTransferMatrix[2][1] = zero;
        inPlaneInvTransferMatrix[2][2] = k.pow(2).multiply(rho(finalTrueAnomaly, E));
        inPlaneInvTransferMatrix[2][3] = zero;
        inPlaneInvTransferMatrix[3][0] = zero;
        inPlaneInvTransferMatrix[3][1] = E.multiply(k.pow(2).multiply(finalTrueAnomaly.sin()));
        inPlaneInvTransferMatrix[3][2] = zero;
        inPlaneInvTransferMatrix[3][3] = k.pow(2).multiply(rho(finalTrueAnomaly, E));

        // Compute out of plane (y_axis) initial velocity required to reach desired final position at t = Δt.
        final T vy0 = k.pow(2).multiply(rho(initialTrueAnomaly, E)).multiply(rho(finalTrueAnomaly, E).multiply(chaserPVTFinalLof.getPosition().getY()).subtract(finalTrueAnomaly.subtract(initialTrueAnomaly).cos().add(E.multiply(finalTrueAnomaly.cos())).multiply(chaserPVTInitialLof.getPosition().getY()))).divide((finalTrueAnomaly.subtract(initialTrueAnomaly)).sin());
        ...
        final T vyf = MatrixUtils.createFieldMatrix(outPlaneInvTransferMatrix).multiply(yaMatrices.getOutPlaneMatrix()).multiply(MatrixUtils.createFieldMatrix(outPlaneTransferMatrix)).operate(yCoordsPostMan).getEntry(1);

        // Compute in plane (x and z axis) initial velocities required to reach desired final position at t = Δt.
        final FieldMatrix<T> transferMatrix = MatrixUtils.createFieldMatrix(inPlaneInvTransferMatrix).multiply(yaMatrices.getInPlaneMatrix().multiply(MatrixUtils.createFieldMatrix(inPlaneTransferMatrix)));
        ...
        final T vx0 = d14.multiply(zf).add(d24.multiply(d11).subtract(d21.multiply(d14)).multiply(x0).add(d24.multiply(d12).subtract(d22.multiply(d14)).multiply(z0)).subtract(d24.multiply(xf))).divide(d23.multiply(d14).subtract(d24.multiply(d13)));
        final T vz0 = d23.multiply(xf).subtract(d13.multiply(zf)).add(d13.multiply(d21).subtract(d11.multiply(d23)).multiply(x0)).add(d13.multiply(d22).subtract(d12.multiply(d23)).multiply(z0)).divide(d23.multiply(d14).subtract(d24.multiply(d13)));
        ...
        final TimeStampedFieldPVCoordinates<T> chaserPVTFinalLofPreMan = new TimeStampedFieldPVCoordinates<>(chaserPVTFinalLof.getDate(), new FieldPVCoordinates<>(chaserPVTFinalLof.getPosition(), vfPreMan));
