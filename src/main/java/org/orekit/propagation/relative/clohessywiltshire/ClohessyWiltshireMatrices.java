...
public class ClohessyWiltshireMatrices {
     ...
     */
    public ClohessyWiltshireMatrices(final double timeSinceEpoch, final RealMatrix phi_rr, final RealMatrix phi_rv, final RealMatrix phi_vr, final RealMatrix phi_vv) {
    ...
    public TimeStampedPVCoordinates transform(final TimeStampedPVCoordinates pvt) {
        return new TimeStampedPVCoordinates(pvt.getDate().shiftedBy(timeSinceEpoch), transform(new PVCoordinates(pvt.getPosition(), pvt.getVelocity())));
    }
    ...
    public PVCoordinates transform(final PVCoordinates pv) {
        ...
        final Vector3D pos1 = new Vector3D(phi_rr.multiply(pos0).add(phi_rv.multiply(vel0)).getColumn(0));
        final Vector3D vel1 = new Vector3D(phi_vr.multiply(pos0).add(phi_vv.multiply(vel0)).getColumn(0));
