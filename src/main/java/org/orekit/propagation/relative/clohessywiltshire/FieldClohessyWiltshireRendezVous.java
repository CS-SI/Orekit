...
public class FieldClohessyWiltshireRendezVous<T extends CalculusFieldElement<T>> {
     ...
     * solution.
     * <p>
     *     Unlike its non-field, this method cannot be static because of the genericity introduced by the field. The field
     *     type T cannot be inferred from the method parameters, and must be defined at the class level.
     * </p>
     *
    ...
    public FieldTwoImpulseTransfer<T> computeRendezVous(final TimeStampedFieldPVCoordinates<T> chaserPVTInitial,
        ...
        final FieldClohessyWiltshireMatrices<T> cwMatrices =
                        (new FieldClohessyWiltshireEquations<T>()).computeMatrices(deltaT, meanMotion);
        final FieldMatrix<T> phi_rv = cwMatrices.getPhiRV();
        final FieldQRDecomposition<T> qrDecomposition = new FieldQRDecomposition<>(phi_rv);
        final FieldMatrix<T> phi_rv_inv = qrDecomposition.getSolver().getInverse();
        ...
        final FieldMatrix<T> initialChaserPos =
                        MatrixUtils.createColumnFieldMatrix(chaserPVTInitialLOF.getPosition().toArray());
        final FieldMatrix<T> finalChaserPos =
                        MatrixUtils.createColumnFieldMatrix(chaserPVTFinalLOF.getPosition().toArray());
        final FieldMatrix<T> subtract = finalChaserPos.subtract(cwMatrices.getPhiRR().multiply(initialChaserPos));
        final FieldVector3D<T> v0PostMan = new FieldVector3D<>(phi_rv_inv.multiply(subtract).getColumn(0));
        ...
        final FieldMatrix<T> chaserP =
                        MatrixUtils.createColumnFieldMatrix(chaserPVTInitialLOFPostMan.getPosition().toArray());
        final FieldMatrix<T> chaserV =
                        MatrixUtils.createColumnFieldMatrix(chaserPVTInitialLOFPostMan.getVelocity().toArray());
        final FieldVector3D<T> v1PreMan = new FieldVector3D<>(cwMatrices.getPhiVR().multiply(chaserP)
