...
public class FieldClohessyWiltshireEquations<T extends CalculusFieldElement<T>> {
    ...
    public FieldClohessyWiltshireMatrices<T> computeMatrices(final T timeSinceEpoch, final T meanMotion) {
        ...
        final FieldSinCos<T> sinCos_nt = FastMath.sinCos(nt);
        final T sin_nt = sinCos_nt.sin();
        final T cos_nt = sinCos_nt.cos();
        final T zero = meanMotion.getField().getZero();
        final T one = meanMotion.getField().getOne();
        ...
        phi_rv[0][1] = one.subtract(cos_nt).multiply(2).divide(meanMotion);
        phi_rv[0][2] = zero;
        ...
        return new FieldClohessyWiltshireMatrices<>(timeSinceEpoch, MatrixUtils.createFieldMatrix(phi_rr),
                                                    MatrixUtils.createFieldMatrix(phi_rv),
                                                    MatrixUtils.createFieldMatrix(phi_vr),
                                                    MatrixUtils.createFieldMatrix(phi_vv));
    }
}
