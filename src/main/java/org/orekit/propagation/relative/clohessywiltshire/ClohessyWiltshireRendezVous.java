...
public class ClohessyWiltshireRendezVous {
     ...
     */
    public static TwoImpulseTransfer computeRendezVous(final TimeStampedPVCoordinates chaserPVTInitial, final TimeStampedPVCoordinates chaserPVTFinal, final Frame chaserPVFrame, final Orbit targetOrbit) {
        ...
        final Vector3D v0PostMan = new Vector3D(MatrixUtils.inverse(cwMatrices.getPhiRV()).operate(MatrixUtils.createColumnRealMatrix(chaserPVTFinalLOF.getPosition().toArray()).subtract(
                        cwMatrices.getPhiRR().multiply(MatrixUtils.createColumnRealMatrix(chaserPVTInitialLOF.getPosition().toArray())))
                .getColumn(0)));
        ...
        final Vector3D v1PreMan = new Vector3D(cwMatrices.getPhiVR().multiply(MatrixUtils.createColumnRealMatrix(chaserPVTInitialLOFPostMan.getPosition().toArray()))
                .add(cwMatrices.getPhiVV().multiply(MatrixUtils.createColumnRealMatrix(chaserPVTInitialLOFPostMan.getVelocity().toArray()))).getColumn(0));

        // PVT of chaser before second maneuver
        final TimeStampedPVCoordinates chaserPVTFinalLOFPreMan = new TimeStampedPVCoordinates(chaserPVTFinalLOF.getDate(), chaserPVTFinalLOF.getPosition(), v1PreMan);
