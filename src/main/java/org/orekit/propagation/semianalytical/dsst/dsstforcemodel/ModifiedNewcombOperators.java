...
public class ModifiedNewcombOperators {
    ...
    private RecurrencePolynomialGenerator                      polynomialGenerator;

    /**
     * Create Modified Newcomb polynomials. From their definition, &sigma; <= &rho;.
     * 
     * @param orderMax
     *            maximum order of computation. This parameter set the &rho; value, and then every
     *            Newcomb polynomials are going to be computed until the element Y<sub>orderMax,
     *            orderMax</sub> is reached.
     */
    public ModifiedNewcombOperators(final int orderMax) {
        ...
        this.polynomialGenerator = new RecurrencePolynomialGenerator();
        buildPolynomial(orderMax);
    ...
    public double getValue(int rho,
        ...
        // Error if sigma > rho
        if (sigma > rho) {
            throw new OrekitException(OrekitMessages.DSST_NEWCOMB_OPERATORS_COMPUTATION, new String[] { "" + sigma,
                            "" + rho }, null);
        }
        // If order hasn't been computed yet, update the Newcomb polynomials
        if (rho > orderMax) {
            computeUpToDegree(rho, orderMax);
        }
        ...
        List<PolynomialFunction> poly = NEWCOMB_POLYNOMIALS.get(new Couple(rho, sigma));
     ...
     */
    public List<PolynomialFunction> getPolynomialList(final int rho,
                                                      final int sigma) {
        return NEWCOMB_POLYNOMIALS.get(new Couple(rho, sigma));
    }
     ...
     * </pre>
     * 
     * * where the P<sub>s<sub>j</sub></sub> and Q<sub>s<sub>k</sub></sub> are polynomials in s, s
     * being the index of the Y<sub>&rho;,&sigma;</sub><sup>n,s</sup> function
     * 
     * @param poly1
    ...
    /** Build the next polynomials if needed */
    private void buildPolynomial(final int degree) {
        final int maxDegree = (int) FastMath.floor(FastMath.sqrt(2 * NEWCOMB_POLYNOMIALS.size())) - 1;
        if (degree > maxDegree) {
    ...
    /**
     * Compute the Modified Newcomb Operators. The computation sequence is the following one (for
     * more clarity, only the subscript are written here.
     * 
     * <pre>
     * Y<sub>0, 0</sub>
     * Y<sub>1, 0</sub>
     ...
     */
    private void computeUpToDegree(int rho,
                                   int maxDegree) {
        ...
        for (int i = FastMath.min(maxDegree, rho) + 1; i < 2 * rho + 1; i++) {
            int k = i;
            int j = 0;
            ...
            while (sequenceRespected) {
                ...
                Couple couple = new Couple(k, j);
                // Get the coefficient from the recurrence relation
                map = polynomialGenerator.generateRecurrenceCoefficients(couple);
     ...
     */
    private List<PolynomialFunction> shiftList(final List<PolynomialFunction> polynomialList,
                                               final int shift) {
