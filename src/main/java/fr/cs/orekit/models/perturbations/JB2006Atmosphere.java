...
public class JB2006Atmosphere {
    ...
    public double getDensity(double dateMJD,double sunRA, double sunDecli,double satLon,
        ...
        final double DF = S + (C - S) * Math.pow(Math.abs(Math.cos(0.5 * tau)),3);
        ...
        final double Z1 = 90.;
        final double Z2 = Math.min(satAlt,105.);
        ...
        if(satAlt <= 105.) {
            ...
            // Equation (6)
            final double Z3 = Math.min(satAlt,500.);
            AL = Math.log(Z3/Z);
            ...
            final double Z4 = Math.max(satAlt,500.);
            AL = Math.log(Z4/Z);
            double R = R2;
            if (satAlt > 500.) {
                R = R3;
        ...
        final double TRASH = (dateMJD - 36204.) / 365.2422;
        final double CAPPHI = TRASH % 1;

        final int signum = (satLat >= 0) ? 1 : -1;

        final double DLRSL = 0.02 * (satAlt - 90.) *
        ...
        if ((satAlt >= 1000.) && (satAlt < 1500.)) {
            ...
            final double F15C   = CHT[1] + CHT[2]*f10B + CHT[3]*1500. + CHT[4]*f10B*1500.;
            final double F15C_ZETA = (CHT[3] + CHT[4]*f10B) * 500.;
            final double FEX2   = 3. * F15C - F15C_ZETA - 3.;
            final double FEX3   = F15C_ZETA - 2. * F15C + 2.;
    ...
    private static double dTc(double f10,double solTimeHour,double satLat,double satAlt) {
        ...
        double dTc = 0;
        final double tx  = solTimeHour/24.;
        ...
        if ((satAlt >= 120) && (satAlt <= 200)) {
            ...
            final double CC  = 3.*DTC200 - DTC200DZ;
            final double DD  = DTC200 - CC;
            final double ZP  = (satAlt-120.)/80.;
        ...
        if ((satAlt > 240.0) && (satAlt <= 300.0)) {
            ...
            final double CC = 3.*DTC300 - DTC300DZ - 3.*AA - 2.*BB;
            final double  DD = DTC300 - AA - BB - CC;
            final double ZP  = (satAlt-240.)/60.;
        ...
        if ((satAlt > 600.0) && (satAlt <= 800.0)) {
            ...
            final double BB  = BdtSub[13]*ycs +
                               BdtSub[14]*tx*ycs    + BdtSub[15]*tx2*ycs + BdtSub[16]*tx3*ycs +
                               BdtSub[17]*tx4*ycs + BdtSub[18]*tx5*ycs;
            final double CC  = -(3.*AA+4.*BB)/4.;
            final double DD  = (AA+BB)/4.;
            dTc = AA + BB*ZP + CC*ZP*ZP + DD*ZP*ZP*ZP;
        }
    ...
    private static double semian (double DAY,double HT,double F10BAR) {
        ...
        final double F10B = F10BAR;
        final double FB2  = F10BAR*F10BAR;

        final double HTZ = HT/1000.;
        ...
        final double TAU   = (DAY-1.)/365;
        final double SIN1P = Math.sin(TWOPI*TAU);
        final double COS1P = Math.cos(TWOPI*TAU);
        final double SIN2P = Math.sin(2.*TWOPI*TAU);
        final double COS2P = Math.cos(2.*TWOPI*TAU);
        final double SIN3P = Math.sin(3.*TWOPI*TAU);
        final double COS3P = Math.cos(3.*TWOPI*TAU);
        final double SIN4P = Math.sin(4.*TWOPI*TAU);
        final double COS4P = Math.cos(4.*TWOPI*TAU);
    ...
    private static double dayOfYear(double D1950) {
        ...
        int IYDAY = (int)D1950;
        final double FRACO = D1950 - IYDAY;
        IYDAY = IYDAY + 364;
