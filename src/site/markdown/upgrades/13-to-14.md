...
| [Use of Optional for optional fields in CCSDS](#Use_of_Optional_for_optional_fields_in_CCSDS)                                                                          | [issue 1103](https://gitlab.orekit.org/orekit/orekit/-/work_items/1103)                                                                          |                                                                            | [issue 1953](https://gitlab.orekit.org/orekit/orekit/-/issues/1953)                                                                      |
| [Added width check to FastLongFormatter](#Added_width_check_to_FastLongFormatter)                                                                                      | [issue 1975](https://gitlab.orekit.org/orekit/orekit/-/work_items/1975)                                                                          |                                                                            | [issue 1975](https://gitlab.orekit.org/orekit/orekit/-/issues/1975)                                                                      |
| [Optional participants pos-vel in measurements](#Optional_participants_pos_vel_in_measurements)                                                                        | [issue 1973](https://gitlab.orekit.org/orekit/orekit/-/work_items/1973)                                                                          |                                                                            | [issue 1953](https://gitlab.orekit.org/orekit/orekit/-/issues/1953)                                                                      |
| [Removed IOException from DataFilter.filter](#Removed_IOException_from_DataFilter_filter)                                                                              | [issue 1973](https://gitlab.orekit.org/orekit/orekit/-/work_items/1973)                                                                          |                                                                            | [issue 1978](https://gitlab.orekit.org/orekit/orekit/-/issues/1978)                                                                      |
| [Use EOPFittedModel instead of EOPFitter to build PredictedEOPHistory](#Use_EOPFittedModel_instead_of_EOPFitter_to_build_PredictedEOPHistory)                          | [issue 1973](https://gitlab.orekit.org/orekit/orekit/-/work_items/1973)                                                                          |                                                                            | [issue 1980](https://gitlab.orekit.org/orekit/orekit/-/issues/1980)                                                                      |
...
`{Field}KeplerianOrbit` that uses a dedicated frame build by freezing the Earth frame to the inertial frame at epoch.
This trick allows the *right ascension* of the ascending node (which corresponds to the `{Field}KeplerianOrbit` API)
is numerically equal to the *longitude* of the ascending node as used in GNSS world. In order to be able to build
this intermediate frame (and change it if time is changed), the factory is built with both the inertial frame and
the Earth frame. This additional frames are also required for building parsers like SEM parsers, YUMA parsers, and
Ntrip clients.
...
waiting for notifications when the messages are already built. They should not see the underlying factory that was
...
Up to 13.X, the `Propagator` interface had a `getInitialState` method to retrieve the initial state. This state was set
up either from implementations constructors or using the `resetInitialState` method. This could include some unmanaged
data in addition to orbit, attitude and mass.
...
has been added to the `Propagator` interface.

### how to adapt existing source code

Use code will mist probably not need any modification. If the side effect of having the additional data evaluated
(as the additional data providers are automatically called when `getInitialState` is called), then users can call
`getBaseInitialState()` instead.
...
Previous code checked for `null` or `Double.NaN`
```java
final SpacecraftParameter parameters = omm.getData().getSpacecraftParametersBlock();
final double dragCoefficient = Double.isNaN(parameters.getDragCoefficient()) ? 1.0 : parameters.getDragCoefficient();
```

was replaced by:
```java
final SpacecraftParameter parameters = omm.getData().getSpacecraftParametersBlock();
final double dragCoefficient = parameters.getDragCoefficient().orElse(1.0);
```
