...
| [remove field parameter of FieldDSSTPropagator and FieldNumericalPropagator](#Remove_field_parameter_of_FieldDSSTPropagator_and_FieldNumericalPropagator)              | [issue 1809](https://gitlab.orekit.org/orekit/orekit/-/issues/1809)                                                                      |
| [refactored (Field)AbsolutePVCoordinates and deleted (Field)ShiftingPVCoordinatesProvider](#Refactored_AbsolutePVCoordiates_and_deleted_ShiftingPVCoordinatesProvider) | [issue 1830](https://gitlab.orekit.org/orekit/orekit/-/issues/1830)                                                                      |
...
| [refactored counting handlers](#Refactored_counting_handlers)                                                                                                          | [issue 1842](https://gitlab.orekit.org/orekit/orekit/-/issues/1842)                                                                      |
...
| [Added getBaseInitialState](#Added-getBaseInitialState)                                                                                                                | [issue 1974](https://gitlab.orekit.org/orekit/orekit/-/work_items/1974)                                                                  |
| [Use of Optional for optional fields in CCSDS](#Use_of_Optional_for_optional_fields_in_CCSDS)                                                                          | [issue 1103](https://gitlab.orekit.org/orekit/orekit/-/work_items/1103)                                                                  |
| [Added width check to FastLongFormatter](#Added_width_check_to_FastLongFormatter)                                                                                      | [issue 1975](https://gitlab.orekit.org/orekit/orekit/-/work_items/1975)                                                                  |
| [Optional participants pos-vel in measurements](#Optional_participants_pos_vel_in_measurements)                                                                        | [issue 1973](https://gitlab.orekit.org/orekit/orekit/-/work_items/1973)                                                                  |
| [Removed IOException from DataFilter.filter](#Removed_IOException_from_DataFilter_filter)                                                                              | [issue 1973](https://gitlab.orekit.org/orekit/orekit/-/work_items/1973)                                                                  |
| [Use EOPFittedModel instead of EOPFitter to build PredictedEOPHistory](#Use_EOPFittedModel_instead_of_EOPFitter_to_build_PredictedEOPHistory)                          | [issue 1973](https://gitlab.orekit.org/orekit/orekit/-/work_items/1973)                                                                  |
| [Creation of ClockModel to generalize QuadraticClockModel](#Creation_of_ClockModel_to_generalize_QuadraticClockModel)                                                  | [issue 1924](https://gitlab.orekit.org/orekit/orekit/-/work_items/1924)                                                                  |                                                                                                                                              |
| [Added controlled loading of EOP data](#Added_controlled_loading_of_EOP_data)                                                                                          | [issue 1992](https://gitlab.orekit.org/orekit/orekit/-/work_items/1992)                                                                  |
| [Renaming of OrbitType](#Renaming_of_orbittype)                                                                                                                        | [issue 1792](https://gitlab.orekit.org/orekit/orekit/-/work_items/1792)                                                                  |
| [Added field version of ParameterDriver](#Added_field_version_of_ParameterDriver)                                                                                      | [issue 2000](https://gitlab.orekit.org/orekit/orekit/-/work_items/2000)                                                                  |
| [Removed time spans in ParameterDriver](#Removed_time_spans_in_ParameterDriver)                                                                                        | [issue 2001](https://gitlab.orekit.org/orekit/orekit/-/work_items/2001)                                                                  |
| [Revamped IsotropicDrag](#Revamped_IsotropicDrag)                                                                                                                      | [issue 2001](https://gitlab.orekit.org/orekit/orekit/-/work_items/2001)                                                                  |
...
So a new marker interface `OrbitalState` has been added, on top of the existing `Orbit`, `TLE`, and
`GNSSOrbitalElements` classes. A new `OrbitalStateFactory` interface has been added with a complete hierarchy
...
All propagator builders are now built using an `OrbitalStateFactory` implementation, which replaces both
...
hierarchy has been renamed `GNSSOrbitalElementsFactory` and now implements the `OrbitalStateFactory` interface
...
`getOrbitalStateFactory` method, and this factory provides the desired elements. Note that `getOrbitType` returns
...
`OrbitalStateFactory<TLE>` and contains both the template TLE and the driver for the B-star non-Keplerian
model parameter. There are still two implementations, one based on a fixed point method and one based on the least
squares method. The static `stateToTLE` methods in `{Field}TLE` now take `TleGenerationAlgorithm` instead of
a template TLE.
...
Classical propagator builders that previously required a template `Orbit` now require an `OrbitalStateFactory`
that generates objects belonging to the `Orbit` hierarchy. The `DSSTPropagatorBuilder` is special, though,
as it requires explicitly an `EquinoctialOrbitFactory`. GNSS propagators require an `OrbitalStateFactory` that
generates objects belonging to the `GNSSOrbitalElements` hierarchy. Beware that since GNSS orbital elements refer to
an Earth frame frozen at a specific date to become an inertial frame, the factory date <em>must</em> have been
...
As the builders now require an `OrbitalStateFactory` instance and as it is possible to create a factory
...
As the getters have been moved from the `PropagatorBuilder` interface to the `OrbitalStateFactory`
...
should be replaced by
```java
AbsoluteDate date = builder.getOrbitalStateFactory().getDate();
```
...
```java
Orbit orbit = builder.getOrbitalStateFactory().createFromDrivers();
```
...
Use of the state transition matrices and Jacobian matrices generated by GNSS propagators should also be adapted
as they are now based on Keplerian elements (they were based on Cartesian elements in previous versions).
...
As the getters for some parts of navigation messages are now available in the messages itself
rather than in the RTCM message, calls of the form:
...
The `getClockOffsetDriver()` method has been renamed `getBiasDriver()`.
...
Users calling the methods `getClockOffsetDriver()` or the driver names should use the new names.
...
Classes that used to hold `QuadraticClockModel`s now by default create `PolynomialClockModel` classes.
However, those clocks can be set to other clock models such as `ClockSum`, `PerfectClockModel`,
or user-defined clock models. The `PolynomialClockModel` requires a reference date if the
underlying polynomial is degree 1 or higher. This reference date has to be provided at construction
when building from simple primitive double coefficients, and should be present in the drivers
when building from `ParameterDriver` instances. When building from `ParameterDriver` instances,
they must follow a naming convention. The names should hve the form `{prefix}-clock-bias`,
`{prefix}-clock-drift`, `{prefix}-clock-acceleration`, and `{prefix}-clock-term-{i}`, where
`{prefix}` is a common prefix and `{i}` is the degree of the term, for degrees 3 and more.

The methods for getting and setting clock bias, rate, and acceleration values have changed as well.
The values returned from `.getBiasDriver` will depend on the clock instance that is used.
However, getting the value for how far the clock is off of the nominal time is provided through
`.getOffset`, which will take in a given time and return the `ClockOffset` class.

There is also a `FieldClockModel` interface and there are field versions of all clock models.
The non-field models have a `toField` method to convert them to field, and field versions
has a `toNonField` method. The `getFieldOffset` in `ClockModel` has therefore been removed,
now users should retrieve a `FieldClockModel` and call its field version of `getOffset`.
...
remote.getBiasDriver().setReferenceDate(AbsoluteDate.ARBITRARY_EPOCH);
remote.getBiasDriver().setValue(1.0e-16);
remote.getDriftDriver().setReferenceDate(AbsoluteDate.ARBITRARY_EPOCH);
remote.getDriftDriver().setValue(0);
remote.getAccelerationDriver().setReferenceDate(AbsoluteDate.ARBITRARY_EPOCH);
remote.getAccelerationDriver().setValue(0);
```

was replaced by:
```java
// The clock is created explicitly and passed into the constructor of the satellite class
PolynomialClockModel remoteClockModel = new PolynomialClockModel(AbsoluteDate.ARBITRARY_EPOCH, prefix, 1e-16);
...
Users should follow the naming conventions for the clock parameters and should ensure
the reference date is set properly when using polynomials models with degree 1 or higher.
...
In order to prevent these errors the formatter now has an additional flag set up at construction to either allow or
forbid exceeding the prescribed width. When this flag is set, this means the width is not a minimum width to be blank
or zero padded, but is rather a mandatory width, so the string produced would always have exactly the prescribed width.
An exception is thrown when this width is exceeded.
...
properly sorted history once built. The `EOPEntry` entries also contain `EOPOrigin` records for X, Y, UT1-UTC
and LOD (they share a common origin), nutation (it may be another origin), and Celestial Intermediate Pole (it
may be yet another origin). These origins store both the publication date and the filename from which the
corresponding data was loaded. It is *not* recommended anymore to use a `SortedSet` to the `EOPHistory`
...
now takes a `DataSource` argument instead of an already opened `InputStream` and name.

A side effect of allowing scattered, unsorted, and incomplete EOP data to be provided to `EOPHistory` is
that the `fillHistory` method in the `EopHistoryLoader` interface changed; it now uses a `Collection<EOPEntry>`
rather than a `SortedSet<EOPEntry>` argument (and has explained above, it is *not* recommended anymore to use
a `SortedSet` here).
...
If users built `EOPEntry` directly, they must now provide the origins for the various components in the constructor.
...
The enum `OrbitType` has been renamed to `OrbitParamsType` as it refers to the type of coordinates of a two-body orbit
rather than the shape (elliptic, hyperbolic).

### how to adapt existing source code
Replace all occurrences of `OrbitType` by `OrbitParamsType`. The same goes with getters and setters.

## Added field version of ParameterDriver

### overview of the change
Since its inception, `ParameterDriver` only dealt with primitive double value. There are also many features
in Orekit that have both a regular primitive and a field version. This `ParameterDriver` limitation made
it harder to create a field version of a feature when the primitive double version relied on `ParameterDriver`.

[Issue 2000](https://gitlab.orekit.org/orekit/orekit/-/work_items/2000) added a `FieldParameterDriver` class.
This class still uses primitive double for the minimum value, maximum value and scale, but uses fields for value,
reference value and reference date. A corresponding `FieldParameterObserver` interface has been created.
Both `ParameterDriver` and `FieldParameterDriver` extends a new `BaseParameterDriver` abstract generic class that
manages the common parts, i.e. name, minimum value, maximum value, scale, selection status, validity interval and
observers. The `ParameterObserver` and `FieldParameterObserver` interfaces also extend a new `BaseParameterObserver`
generic interface. The `ParameterDriversProvider` and `FieldParameterDriversProvider` interfaces also extend a new
`BaseParameterDriversProvider` generic interface. This `BaseParameterDriver` generic class, the `BaseParameterObserver`
and `BaseParameterDriversProvider` generic interfaces are all parameterized over the driver and observer classes,
hence avoiding mixing regular primitive double drivers with field observers.

### how to adapt existing source code

Most existing user code won't need any change, as `ParameterDriver` and `ParameterObserver` API still exist
and their API did not change. The only change is that the static method `ParameterDriversProvider.findByName`
is now defined in the base class and should be referred to as `BaseParameterDriversProvider.findByName`.

## Removed time spans in ParameterDriver

### overview of the change
Since 12.0, `ParameterDriver` supported setting several values for adjacent time spans. This was introduced at the
time in order to deal with daily estimation of drag coefficients or tropospheric effects. In practice, despite it
worked quite well, it introduced a seldom used and complicated feature in a class that is used in many places with
just a single date.

As part of issue [issue 2001](https://gitlab.orekit.org/orekit/orekit/-/work_items/2001) (adding a field version of
`ParameterDriver`), it seemed legit to remove this feature from `ParameterDriver` (and the upcoming
`FieldParameterDriver`) and move it upward in a dedicated `ParameterDriversSequence`. This means that instead of
having one `ParameterDriver` holding a sequence of names and values, one has a sequence of `ParameterDriver`.
The `IsotropicDrag` was changed to use this (see next section). In order to build a `ParameterDriversSequence`,
one has to use a `ParameterDriversSequenceBuilder`, adding the reference values progressively with their associated
time range, and then calling the `build` method once all reference values have been set up. The reference values
can be added in arbitrary order, they will be sorted chronologically when the `build` method is called.

The naming convention used in `ParameterDriversSequenceBuilder` and in `ParameterDriversSequenceBuilder` is slightly
different from the one used previously in `ParameterDriver` time spans. The builder starts from a base name (say
`base` for example). If only one reference value is added to the instance, then the unique `ParameterDriver` in the
sequence will be named `base`. If several reference values have been added, then a names will be built from the
`span-` prefix, the base name, a singe "-" separator, and an integer index counting from 0. the names will therefore
have the form `span-base-0`,`span-base-1` `span-base-2`… The previous convention used `Span` (without a dash), the
index started at 1, and the name was changed even when only one value was used (which was confusing as most uses
required only one value, so users were often required to truncate the names the library messed up).

As the `ParameterDriver` instances in a sequence are independent of each other, it is now possible to select only a
subset. This was not possible with the previous design.

The main consequence of the change is that all `ParameterDriver` methods that were related to time spans have been
removed, making the class API much sleeker. The removed methods include the constructor with a `TimeSpanMap` argument,
`getNamesSpanMap`, `getValueSpanMap`, `setValueSpanMap`, `getNbOfValues`, `getTransitionDates`, `getValues`,
`addSpans`, `addSpanAtDate`, and all the getters and setters having a date argument. A side effect of this change is
that the `getMu(date)` method in the `NewtonianAttraction` force model does not have a date argument anymore.

Another small side effect of the change is that when registering a `ParameterObserver` to a `ParameterDriver`,
its `valueChanged` method is not called once when `addObserver` is called.

As part of the change, all the classes related to `ParameterDriver` have been moved into a new dedicated
`org.orekit.utils.drivers` package.

### how to adapt existing source code

User code that relied on time spans within `ParameterDriver` should be changed to use `ParameterDriversSequence`
instead, and take care of the slightly different naming convention.

If some of the getters with a date were used (most often `getValue(date)`), then the date argument must simply
be removed.

If some users relied on the fact the `valueChanged` method was called once when a `ParameterObserver` was registered,
then they should call the method by themselves alongside the call to `addObserver`.

## Revamped IsotropicDrag

### overview of the change

`IsotropicDrag` used a single `ParameterDriver` instance to manage several time-dependent drag coefficients.
As this `ParameterDriver` feature was phased out, the model had to be adapted to use as many `ParameterDriver`
instances as there are drag coefficients. A new `IsotropicDragBuilder` has been created to set up these drivers
properly, and it is now the only way to build an `IsotropicDrag` model.

The built `IsotropicDrag` model now exposes one `ParameterDriver` instance (the first one in the list) with
the default name `DragSensitive.GLOBAL_DRAG_FACTOR` that is a single global factor applied to all coefficients.
This instance is followed by one `ParameterDriver` instance for each individual drag coefficients, sorted chronologically.
If there is only one drag coefficient, its default name is `DragSensitive.DRAG_COEFFICIENT`. If there are several
drag coefficients their default names are the concatenation of `ParameterDriversSequenceBuilder.SPAN_PREFIX`,
`DragSensitive.DRAG_COEFFICIENT`, a single "-", and an index counting from 0. Of course, these names can be changed
using `ParameterDriver.setName()`. As each drag coefficient is managed by a separate `ParameterDriver` instance,
one can estimate only a subset of the drag coefficients in orbit determination. Previously, estimation applied only
to all coefficients or none of them.

### how to adapt existing source code

As most uses will be to have a single drag coefficient, this is done as follows.

Code before the change:
```java
  IsotropicDrag drag = new IsotropicDrag(crossSection, dragCoeff);
```

Code after the change:
```java
  IsotropicDrag drag = new IsotropicDragBuilder(crossSection).add(dragCoeff).build();
```

If several coefficients are used, then the code becomes:
```java
  IsotropicDrag drag = new IsotropicDragBuilder(crossSection).
                       add(dragCoeff1, AbsoluteDate.PAST_INFINITY, date1).
                       add(dragCoeff2, date1, date2).
                       add(dragCoeff3, date2, AbsoluteDate.FUTURE_INFINITY).
                       build();
```

It is not required to start at past infinity or end at future infinity. If time simulation limits are
already known at build time, one can use these limits instead. The global time interval should be continuous,
though, otherwise an exception will be triggered when a sub-interval with no associated drag coefficient is
detected. Coefficients may be added to the builder in random order, they will be sorted chronologically
internally when the `build()` method is called.

As instead of having a single parameter driver named `drag coefficient`, one now gets several parameters
with names of the form `span-drag coefficient-0`, `span-drag coefficient-1`, and so on. This implies that when
performing orbit determination one should select these parameters one-by-one for estimation; selecting only one
propagation parameter named `drag coefficient` would not work. So code like:
