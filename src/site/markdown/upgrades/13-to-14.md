...
| [Building TimeInterval With Non Chronological Arguments](#Building_TimeInterval_With_Non_Chronological_Arguments)                                                      | [issue 2005](https://gitlab.orekit.org/orekit/orekit/-/work_items/2005)                                                                  |
...
## Building TimeInterval With Non Chronological Arguments

### overview of the change

The `TimeInterval.of` builder method allowed its two argument dates to be provided in either chronological
on non-chronological order. The method reordered its arguments as needed so  `getStartDate` returned the
earliest date and `getEndDate` returned the latest date.

There was therefore no way to detect dates given in a wrong order. An `allowNonChronological` boolean
parameter has therefore been added so users can choose to rely on the existing reordering feature if
is suits them (setting the boolean to `true`), or generating an exception if ordering was wrong
(setting the boolean to `false`).

### how to adapt existing source code

Existing code that relied on the reordering should just add `true` as a third arguments to
`TimeInterval.of` to mimic the previous behavior.
