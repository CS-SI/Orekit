...
So the exception has been removed from the interface signature.

### how to adapt existing source code

If users called `DataFilter.filter(dataSource)`, then this method will not throw any `IOException`
anymore, to the calling method signature may need to be adapted and `IOException` may need to be
removed.

If users implemented the `DataFilter` interface by themselves and in the unlikely case they did
