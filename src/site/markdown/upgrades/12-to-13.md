...
| [introduced impulse provider for maneuvers](#Introduced impulse provider for maneuvers)                                                                    | [issue 1619](https://gitlab.orekit.org/orekit/-/issues/1619/)                                                                      |
...
so they have been generalized as `ThrustSegment`.
Moreover, the latter implements `ParameterDriversProvider`, opening up the door for partial derivative computations.

### how to adapt existing source code
The original constructor of `ProfileThrustPropulsionModel` has been replaced by an static `of` method, 
since now a `TimeSpanMap` of `ThrustSegment` is expected as argument. 
