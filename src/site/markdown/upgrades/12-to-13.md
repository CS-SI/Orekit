...
value back between $O$ and $10^{18}$ is straightforward). The workhorse of the new implementation
...
more importantly more robust and decimal-friendly.
...
were changed from seconds per day to nanoseconds per UTC second, as looking at the values shows
these slopes were in fact simple numbers (only different three slopes were used between 1961
and 1972: 15ns/s, 13ns/s and 30ns/s). The offset has also been changed from double to `SplitTime`
to allow representing exactly the microseconds offsets used in linear models before 1972. As the
...
is defined as `long SLOPE_FACTOR = 86400L * 1000000000L`; and then build the offset by using this
integer slope in nanoseconds per UTC seconds. Using `SplitTime.parse` instead of `Double.parsedouble`
