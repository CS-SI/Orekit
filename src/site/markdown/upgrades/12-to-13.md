...
In order to work around this problem, dates handling has been thoroughly revamped and the
fractional part within the second is not represented as a primitive long with fixed precision
and a resolution of one attosecond ($10^{-18}s$). The choice of attoseconds allows te present
exactly the offset between TT and TAI (because it is a whole number of milliseconds), as well
as all times converted from standard java `Instant`, `Date` or `TimeUnit` classes. The workhorse
of the new implementation is the `SplitTime` class that contains a time split into seconds
and attoseconds. This new class is therefore much more accurate than the previous one (attoseconds
rather than femtoseconds) and more importantly more robust and decimal-friendly.
...
the API of classes `AbsoluteDate`, `TimeComponents`, `DateTimeComponents`, `GNSSDate`, and their
field counterparts, as well as the `TimeScale` interface. In most cases, the methods taking a
primitive double as an argument have been kept and they delegate to a new method that creates
a `SplitTime` instance on the fly from the double. Methods that returned a primitive double
have sometimes been kept (for example `durationFrom` in `AbsoluteDate` is still there) but a
sister method has been created to take advantage of the new implementation with increased
accuracy (so there is now a`splitDurationFrom` method in `AbsoluteDate`). Some methods that
returned a primitive double have been changed and now return `SplitTime` instances, this is
in particular the case of all the methods in the `TimeScale` interface.
...
the wrapping between primitive double and `SplitTime`, but rather take the opportunity to
remove the primitive double and generalize use of `SplitTime` everywhere. This will increase
both accuracy of computation and robustness.

The static method `TimeComponents.fromSecond` method intended to finely tune construction
of dates within a lea seconds occurrent has been replaced by a public constructor
taking a `SplitTime` instead of its first two arguments.

The units for slopes in UTC-TAI linear models used prior to 1972 were changed from seconds per
day to attoseconds per second, as looking at the values shows these slopes were in fact simple
numbers (only three slopes were used between 1961 and 1972: 15ns/s, 13ns/s and 30ns/s). As the
`UTCTAIOffset` class is mainly intended to implement UTC-TAI loaders and Orekit already provides
loaders for the major formats, this should not affect many users. For those users that did
implement custom loaders that take old slopes into account, they should scale the slopes by
changing there parsing code from `double slope = Double.parsedouble(field)` to
