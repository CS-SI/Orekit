...
`object.someMethod(new SplitTime(offset))`. Custom implementations of `TimeShiftable` could
be updated to take advantage of the new `shiftedBy(SplitTime)` method, but it is not required
as there is a default implementation that delegates to the original `shiftedBy(double)` method.
