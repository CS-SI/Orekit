...
| [introduced AdditionalDataProvider](#Introduced_additional_data_provider)                                                                                                       | [issue 1645](https://gitlab.orekit.org/orekit/orekit/-/issues/1645/)                                                                             |
...
## Introduced AdditionalDataProvider

### overview of the change
A new interface `AdditionalDataProvider` has been introduced to generalize the mechanism of additional states to any object.
In order to have a smoother upgrade, the `AdditionalStateProvider` interface has been kept. It extends `AdditionalDataProvider`
and is adapted to `double[]`.
...
by
```java
double[] getAdditionalData(SpacecraftState state);
```
It is strongly recommended to use now the new `AdditionalDataProvider` interface.

### how to adapt existing source code (Propagator)

In `Propagator`, the calls of the following methods
```java
...
In `SpacecraftState`, the calls of the following methods
```java
