...
or microseconds) prevented safe rounbdtrip operations. Another type of problems occurred when
standard java `Instant`, `Date` or `TimeUnit` were used as  they refer to milliseconds,
microseconds or nanoseconds. Yeat another problem was when TT (Terrestrial Time) scale was used,
...
interface and all its implementations. In most cases, the methods taking a primitive double as an
argument have been kept and they delegate to a new method that creates a `SplitTime` instance on
the fly from the double. Methods that returned a primitive double have sometimes been kept (for
example `durationFrom` in `AbsoluteDate` is still there) but a sister method has been created to
take advantage of the new implementation with increased accuracy (so there is now a`splitDurationFrom`
...
is the `TimeScale` interface, which only uses `SplitTime` instances. Most users just
...
within a leap seconds occurrence has been replaced by a public constructor taking a single
...
were changed from seconds per day to nanoseconds per second, as looking at the values shows
...
account, they should scale the slopes by changing their parsing code from
